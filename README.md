...
- 빈 상태에서는 해당 메모와 관련된 안내만 짧게 표시하고, `읽음`/`중요` 안내를 상단에 한꺼번에 늘어놓지 않는다.
