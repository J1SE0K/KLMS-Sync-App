...
대상 저장소: `J1SE0K/KLMS-Sync-App` (commit `09f54670`, tracked 418 files)
...
좌표와 선으로 보는 편이 빠르다. 다만 402개 파일을 한 화면에 뿌리면 오히려 잃는다.
...
실측: node 528, edge 1216 (contains 527, invokes 163, imports 91, packages 173, tests 239, communicates 23). contains 제외 confidence: high 478, medium 68, low 143.

### UI 구조

- 상단: breadcrumb (System overview › Components › <component>), source commit, tracked 수, 생성일
- 좌측: 레벨 전환, Overview/zoom, 검색, 언어·관계·confidence 필터, legend, 변경 강도 범례
- 중앙 SVG: L0 격자(14 component, 주요 흐름만) / L1(모든 component 관계, `type ×N` 라벨) / L2(중첩 디렉터리·파일 박스 + 외부 component stub)
- 우측: node/edge 상세. hover 없이 모든 정보 노출
...
- 분석기 unit test 31개: 합성 저장소(온갖 패턴을 일부러 심음) 22개 + glob/private 규칙 3개 + 실제 저장소 무결성 6개
- 실제 저장소 검사: 모든 file node가 `git ls-files`에 존재, 모든 edge가 존재하는 node 참조, evidence 비어 있지 않음, `source_commit == HEAD`, root wrapper 12개 전부 `bin/`으로 위임하는 edge 존재, `Uncategorized` 없음
...
1. 분석기와 데이터 모델 (근거·confidence를 스키마에 강제)
...
| 시간 | 행동 | 보이는 것 | 말할 것 |
| --- | --- | --- | --- |
| 0–10s | 브라우저에서 열기 | L0: 14개 component 격자, 상단에 commit `09f54670`, 418 files | "이 화면은 저장소의 tracked 파일 418개를 14개 덩어리로 나눈 것이다. 문서가 아니라 코드에서 뽑았다." |
...
| 50–60s | 검색창에 `worker.mjs` → Enter, 그 다음 Esc 두 번 | Relay Infrastructure L2로 점프, Esc로 L1 → L0 복귀 | "어디에 있든 검색으로 파일에 가고 Esc로 전체 그림으로 돌아온다. 이게 파일 트리와 README만으로는 안 되는 부분이다." |

예비 질문 대응:
- "confidence는 어떻게 정했나" → README 표. 코드에서 직접 확인이면 high, 경로 문자열·인자 전달·경로 3개 이상 공유면 medium, 이름만·경로 1~2개 공유·비실행 파일 경로 문자열이면 low.
- "왜 force graph가 아닌가" → 위치가 매번 바뀌면 공간 기억이 쌓이지 않는다. 격자는 고정이고 필터는 opacity만 바꾼다.
