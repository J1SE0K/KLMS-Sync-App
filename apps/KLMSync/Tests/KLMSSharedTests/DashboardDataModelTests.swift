...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacFileDashboardAvoidsPerRowFilesystemTasks() throws {
        ...
        XCTAssertTrue(detail.contains("courses: { DashboardCourseFilter.options(for: kind, snapshot: snapshot) }"))
        XCTAssertTrue(detail.contains("years: { DashboardTermFilter.yearOptions(for: kind, snapshot: snapshot) }"))
        XCTAssertTrue(detail.contains("semesters: { DashboardTermFilter.semesterOptions(for: kind, snapshot: snapshot) }"))
        ...
        XCTAssertTrue(detail.contains("private var fileDataRenderSignature: DashboardFileData.Signature?"))
        XCTAssertTrue(detail.contains("self.fileDataRenderSignature = kind.requiresFileData ? DashboardFileData.Signature(snapshot: resolvedSnapshot) : nil"))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(statusScreen.contains("CompanionScreenContainer(title: horizontalSizeClass == .regular ? \"대시보드\" : \"상태\""))
        ...
        XCTAssertTrue(remoteSettingsPanel.contains(".transition(.opacity)"))
        XCTAssertTrue(remoteSettingsPanel.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: 12))"))
        ...
        XCTAssertTrue(relayConnectionPanel.contains(".transition(.opacity)"))
        ...
        XCTAssertTrue(remotePrivacyNote.contains(".transition(.opacity)"))
        ...
        XCTAssertTrue(immediateSettingRow.contains("@State private var isExpanded = false"))
        XCTAssertTrue(immediateSettingRow.contains("Text(statusText)"))
        XCTAssertTrue(immediateSettingRow.contains("CompanionExpansionBadge(isExpanded: isExpanded, compact: true)"))
        ...
        XCTAssertTrue(remoteCommandPanel.contains("return \"전체 동기화\""))
        XCTAssertFalse(remoteCommandPanel.contains("return \"Mac 연결 필요\""))
        XCTAssertFalse(remoteCommandPanel.contains("return \"잠시 대기\""))
        XCTAssertTrue(remoteCommandPanel.contains("isDisabled ? Color.klmsSecondaryText.opacity(0.76) : Color.klmsPrimaryCommandButtonForeground"))
        XCTAssertTrue(remoteCommandPanel.contains("if isDisabled { return Color.klmsSubtleCardBackground.opacity(0.86) }"))
        XCTAssertTrue(remoteCommandPanel.contains("if isDisabled { return Color.klmsCommandButtonBorder.opacity(0.64) }"))
        XCTAssertTrue(remoteCommandPanel.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: 12, disabledOpacity: 0.78))\n        .disabled(isDisabled)"))
        ...
        XCTAssertTrue(remoteCommandPanel.contains("let isDisabled = commandDisabled(for: kind)"))
        XCTAssertTrue(remoteCommandPanel.contains("secondaryCommandSystemImage(isRunning: isRunning, isDisabled: isDisabled)"))
        XCTAssertTrue(remoteCommandPanel.contains(".buttonStyle(KLMSCardButtonStyle(disabledOpacity: 1.0))"))
        XCTAssertFalse(remoteCommandPanel.contains(".opacity(commandDisabled(for: kind) ? 0.48"))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertFalse(iosDashboardStatusStrip.contains("Color.klmsPrimaryCommandButtonBackground"))
        XCTAssertFalse(iosDashboardStatusStrip.contains("Color.klmsPrimaryCommandButtonForeground"))
        XCTAssertTrue(iosDashboardStatusStrip.contains("return Color.klmsCommandButtonBackground"))
        XCTAssertTrue(iosDashboardStatusStrip.contains("return Color.klmsPrimaryText"))
        ...
        XCTAssertTrue(designSpec.contains("사이드바: 대시보드, 파일, 과제/시험, 공지, 캘린더, 로그, 설정"))
        XCTAssertFalse(designSpec.contains("사이드바: 대시보드, 파일, 공지, 과제/시험, 캘린더, 로그, 설정"))
        ...
        XCTAssertTrue(iosWorkstationOverviewPanel.contains("MetricSummary(title: \"파일\", value: status.fileTotal, systemImage: \"folder\", tint: Color.klmsCommandAccent)"))
        XCTAssertTrue(iosWorkstationOverviewPanel.contains("MetricSummary(title: \"공지\", value: status.notices, systemImage: \"note.text\", tint: Color.klmsCommandAccent)"))
        XCTAssertTrue(iosWorkstationOverviewPanel.contains("MetricSummary(title: \"시험\", value: status.exams, systemImage: \"calendar.badge.clock\", tint: Color.klmsWarningBorder)"))
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertFalse(ios.contains("private struct WorkstationDashboardDetailPanel"))
        XCTAssertTrue(categoryDetail.contains("var itemsRevision: Int"))
        XCTAssertTrue(categoryDetail.contains("itemsRevision: itemsRevision"))
        XCTAssertFalse(categoryDetail.contains("companionItemsFingerprint(items)"))
        XCTAssertTrue(categoryDetail.contains("Section(\"검색과 필터\")"))
        XCTAssertTrue(categoryDetail.contains("TextField(\"\\(category.title) 검색\", text: $query)"))
        XCTAssertFalse(categoryDetail.contains("Section(\"검색\")"))
        XCTAssertFalse(categoryDetail.contains("Section(\"보기\")"))
        XCTAssertTrue(categoryDetail.contains("CompanionItemListControlsPlaceholder()"))
        XCTAssertTrue(inlineDetail.contains("CompanionControlBox(title: \"검색과 필터\", systemImage: \"magnifyingglass\")"))
        ...
        XCTAssertFalse(categoryDetail.contains(".searchable(text: $query"))
        XCTAssertTrue(syncDataPanel.contains("TextField(\"동기화 데이터 검색\", text: $query)"))
        XCTAssertTrue(syncDataPanel.contains("CompanionControlBox(title: \"검색과 필터\", systemImage: \"magnifyingglass\")"))
        ...
        XCTAssertTrue(categoryDetail.contains("await rebuildCachedListDataAfterInputSettles()"))
        XCTAssertTrue(categoryDetail.contains("try? await Task.sleep(nanoseconds: CompanionLargeList.filterRebuildDelayNanoseconds)"))
        ...
        XCTAssertTrue(workstationCategory.contains("deferExternalDetail(item.id)"))
        XCTAssertTrue(workstationCategory.contains("await Task.yield()"))
        XCTAssertTrue(workstationCategory.contains("clearStaleExternalSelectionIfNeeded()"))
        XCTAssertTrue(workstationCategory.contains("WorkstationExternalDetailPanel"))
        XCTAssertTrue(workstationCategory.contains("displayedSelectedItemID = itemID"))
        ...
        XCTAssertTrue(workstationTasks.contains("deferExternalDetail(item.id)"))
    ...
    func testIOSCalendarDetailHasMailPasteAnalyzerAndSharedCalendarActionLabels() throws {
        ...
        XCTAssertTrue(statusScreen.contains("if horizontalSizeClass != .regular, selectedChangeSummary != nil || displayedChangeSummary != nil"))
