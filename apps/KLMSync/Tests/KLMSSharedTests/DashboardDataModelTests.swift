...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacRelayRealtimeUsesSessionAndOperationOwnershipWithoutBlockingClaims() throws {
        ...
        let terminalWrite = try XCTUnwrap(remoteTail.range(of: "try await store.update(completed)")?.lowerBound)
        ...
        XCTAssertLessThan(terminalWrite, currentSessionUI, "Captured old store must receive terminal status before current-session UI writes.")
        ...
        XCTAssertTrue(remoteTail.contains("persistServerRelayTerminalState"))
