...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacAndIOSUseWholeScreenVerticalScroll() throws {
        ...
        XCTAssertTrue(macRootBody.contains("MacWorkspaceRenderedAccessibilityMarker(section: renderedSection)"))
    ...
    func testMacWorkspaceAccessibilitySmokeScriptTargetsNavigation() throws {
        ...
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"dashboard\", title: \"대시보드\")"))
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"files\", title: \"파일\")"))
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"tasks\", title: \"과제/시험\")"))
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"notices\", title: \"공지\")"))
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"calendar\", title: \"캘린더\")"))
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"activityLogs\", title: \"로그\")"))
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"diagnostics\", title: \"진단\")"))
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"settings\", title: \"설정\")"))
        XCTAssertTrue(script.contains("for target in workspaceTargets"))
        XCTAssertTrue(script.contains("var contentIdentifier: String { \"workspace-scroll-\\(rawValue)\" }"))
        XCTAssertTrue(script.contains("waitForSelectedValue(on: button, timeout: timeout)"))
        XCTAssertTrue(script.contains("waitForElement(withIdentifier: target.contentIdentifier, in: appElement, timeout: timeout)"))
        ...
        XCTAssertTrue(probe.contains("var selectionIdentifier: String { \"workspace-content-\\(rawValue)\" }"))
        XCTAssertTrue(probe.contains("var contentIdentifier: String { \"workspace-rendered-section-\\(rawValue)\" }"))
        XCTAssertTrue(probe.contains("waitForElement(withIdentifier: target.selectionIdentifier, in: appElement, timeout: timeout)"))
        XCTAssertTrue(probe.contains("waitForElement(withIdentifier: target.contentIdentifier, in: appElement, timeout: timeout)"))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(ios.contains("shouldShowRunningStatus: model.hasInFlightRequest || model.status.phase == \"running\""))
        ...
        XCTAssertTrue(immediateSettingsPanel.contains("CompanionAppearanceModeSelector(model: model)"))
        ...
        XCTAssertTrue(ios.contains("동기화 범위와 캘린더 반영 기준을 정합니다."))
        ...
        XCTAssertTrue(remoteSettingGroupSection.contains("groupHeader\n                .accessibilityElement(children: .combine)"))
        XCTAssertFalse(ios.contains("var isCollapsible: Bool"))
        XCTAssertFalse(remoteSettingGroupSection.contains("if group.isCollapsible"))
        XCTAssertFalse(remoteSettingGroupSection.contains("if !group.isCollapsible || isExpanded"))
        XCTAssertFalse(remoteSettingGroupSection.contains("(!group.isCollapsible || isExpanded) ? Color.klmsSelectedBorder.opacity(0.48) : Color.klmsBorder.opacity(0.86)"))
        XCTAssertTrue(remoteSettingGroupSection.contains("ForEach(group.settings)"))
        XCTAssertTrue(remoteSettingGroupSection.contains(".stroke(Color.klmsSelectedBorder.opacity(0.48), lineWidth: 1)"))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(macAlertBannerSnapshot.contains("selectedSection == .dashboard"))
        ...
        XCTAssertTrue(taskWorkspaceBody.contains(".frame(minHeight: 44)"))
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertTrue(serverSyncDataRow.contains("let summary = rowSummary"))
        ...
        XCTAssertTrue(serverSyncDataRow.contains(".accessibilityLabel(summary.accessibilityLabel)"))
        XCTAssertTrue(ios.contains("private struct ServerSyncRowSummary: Equatable"))
        ...
        XCTAssertTrue(serverSyncDataRow.contains("private var rowSummary: ServerSyncRowSummary"))
