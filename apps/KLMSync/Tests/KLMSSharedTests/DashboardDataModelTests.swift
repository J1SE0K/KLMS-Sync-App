...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacFileDashboardAvoidsPerRowFilesystemTasks() throws {
        ...
        XCTAssertTrue(macWorkstationLayoutView.contains("workspaceContentMarker\n            DashboardFileDataPrewarmView(snapshot: model.snapshot, signature: model.dashboardFileRenderSignature)"))
        let prewarmIndex = try XCTUnwrap(macWorkstationLayoutView.range(of: "DashboardFileDataPrewarmView(snapshot: model.snapshot, signature: model.dashboardFileRenderSignature)")?.lowerBound)
        let switchIndex = try XCTUnwrap(macWorkstationLayoutView.range(of: "switch selectedSection")?.lowerBound)
        XCTAssertLessThan(prewarmIndex, switchIndex)
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertTrue(ios.contains("static let detailRenderDelayNanoseconds: UInt64 = 4_000_000"))
        XCTAssertTrue(ios.contains("static let prewarmDelayNanoseconds: UInt64 = 120_000_000"))
    ...
    func testIOSCompanionUsesAdaptiveIPadNavigation() throws {
        ...
        XCTAssertTrue(ios.contains("private let sectionRenderDelayNanoseconds: UInt64 = 4_000_000"))
