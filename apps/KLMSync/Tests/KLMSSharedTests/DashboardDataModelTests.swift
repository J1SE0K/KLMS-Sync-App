...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(macDesignWindow.contains("navigationButton(\"대시보드\", selected: selectedWorkspace == .dashboard && selectedMetric != .logs)"))
        XCTAssertTrue(macDesignWindow.contains("navigationButton(\"로그\", selected: selectedWorkspace == .dashboard && selectedMetric == .logs)"))
        XCTAssertTrue(macDesignWindow.contains("navigationButton(\"진단\", selected: selectedWorkspace == .diagnostics)"))
        XCTAssertTrue(macDesignWindow.contains("navigationButton(\"설정\", selected: selectedWorkspace == .settings)"))
