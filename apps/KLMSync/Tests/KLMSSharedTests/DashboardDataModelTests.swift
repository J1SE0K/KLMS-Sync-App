...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(mac.contains("let fileCount = summary.serverFileCount > 0 ? summary.serverFileCount : snapshot.courseFileManifest.count"))
        XCTAssertTrue(mac.contains("let assignmentCount = summary.serverAssignmentCount > 0 ? summary.serverAssignmentCount : counts.assignments + summary.mailAssignmentCount"))
        XCTAssertTrue(mac.contains("let noticeCount = summary.serverNoticeCount > 0 ? summary.serverNoticeCount : counts.notices"))
        XCTAssertTrue(mac.contains("let examCount = summary.serverExamCount > 0 ? summary.serverExamCount : counts.exams + summary.mailExamCount"))
        XCTAssertTrue(mac.contains("let helpDeskCount = summary.serverHelpDeskCount > 0 ? summary.serverHelpDeskCount : counts.helpDesk"))
