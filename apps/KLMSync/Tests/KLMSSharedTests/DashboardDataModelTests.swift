...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacAndIOSUseSeparatedLightAndDarkThemeTokens() throws {
        ...
        XCTAssertFalse(mac.contains(".foregroundStyle(.secondary)"))
        XCTAssertFalse(mac.contains("return .secondary"))
        XCTAssertFalse(mac.contains("Color.secondary"))
        XCTAssertFalse(mac.contains("Color.primary"))
        XCTAssertFalse(mac.contains(".accentColor"))
        XCTAssertFalse(mac.contains("Color(nsColor: .controlBackgroundColor)"))
        XCTAssertFalse(mac.contains("Color(nsColor: .textBackgroundColor)"))
        XCTAssertFalse(mac.contains(".background(.quinary"))
        XCTAssertFalse(mac.contains(".background(.background"))
        XCTAssertNil(mac.range(of: #"foregroundStyle\([^\n]*(\.secondary|\.primary|Color\.secondary|Color\.primary|\.accentColor)"#, options: .regularExpression))
        XCTAssertNil(mac.range(of: #"tint\([^\n]*(\.secondary|\.accentColor)"#, options: .regularExpression))
        XCTAssertNil(mac.range(of: #"stroke\(\.quaternary"#, options: .regularExpression))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(compactRoot.contains("ForEach(CompanionAppSection.compactTabs)"))
        ...
        XCTAssertTrue(statusScreen.contains("RemoteDashboardStatusStrip"))
        XCTAssertTrue(statusScreen.contains("shouldShowCompactStatusStrip"))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(iosSplitRoot.contains("CompanionAppSection.workstationSections"))
        XCTAssertTrue(iosSplitRoot.contains("showsIcon: false"))
        XCTAssertTrue(iosSplitRoot.contains("showsArrow: false"))
    ...
    func testIOSCompanionUsesAdaptiveIPadNavigation() throws {
        ...
        XCTAssertTrue(ios.contains(".navigationSplitViewColumnWidth(min: 176, ideal: 198, max: 230)"))
        ...
        XCTAssertTrue(ios.contains("Mac 앱에 실행 요청을 보냅니다."))
