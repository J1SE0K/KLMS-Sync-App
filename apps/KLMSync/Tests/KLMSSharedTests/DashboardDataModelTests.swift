...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacAndIOSServerRelayUseWebSocketInsteadOfPeriodicPolling() throws {
        ...
        XCTAssertTrue(macModel.contains("private var serverRelayDashboardWatchdogTask: Task<Void, Never>?"))
        XCTAssertTrue(macModel.contains("private static let serverRelayDashboardWatchdogIntervalNanoseconds: UInt64 = 2_000_000_000"))
        XCTAssertTrue(macModel.contains("private func runServerRelayDashboardWatchdog"))
        XCTAssertTrue(macModel.contains("await refreshServerRelayDashboardNow(silent: true)"))
        XCTAssertTrue(ios.contains("private var serverRelayDashboardWatchdogTask: Task<Void, Never>?"))
        XCTAssertTrue(ios.contains("private static let dashboardWatchdogIntervalNanoseconds: UInt64 = 2_000_000_000"))
        XCTAssertTrue(ios.contains("private func runServerRelayDashboardWatchdog"))
        XCTAssertTrue(ios.contains("scope: .syncData"))
