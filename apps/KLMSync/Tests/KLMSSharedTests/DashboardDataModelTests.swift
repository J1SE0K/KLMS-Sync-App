...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(
            immediateSettingsPanel
                .replacingOccurrences(of: "\\u{00A0}", with: " ")
                .contains("화면 모드는 이 기기에 바로 적용되고, 공지 메모 설정은 서버에 저장됩니다.")
        )
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertTrue(tabRoot.contains("@ObservedObject var model: CompanionModel"))
        XCTAssertTrue(splitRoot.contains("@ObservedObject var model: CompanionModel"))
        ...
        XCTAssertFalse(tabRoot.contains("let model: CompanionModel"))
        XCTAssertFalse(splitRoot.contains("let model: CompanionModel"))
