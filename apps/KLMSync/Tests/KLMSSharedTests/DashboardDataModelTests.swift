...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testIOSLargeDashboardListsAreProgressivelyRendered() throws {
        let packageRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        ...
        XCTAssertTrue(selectableRows.contains("@State private var visibleLimit"))
        ...
        XCTAssertTrue(inlineDetail.contains("CompanionInlineItemRowsView(category: category, items: filtered, model: model)"))
        XCTAssertTrue(inlineRows.contains("accessorySystemImage: selectedItemID == item.id ? \"chevron.up\" : \"chevron.down\""))
