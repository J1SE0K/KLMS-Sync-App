...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacAndIOSUseWholeScreenVerticalScroll() throws {
        ...
        XCTAssertTrue(mac.contains(".accessibilityIdentifier(\"workspace-panel-workspace-\\(section.rawValue)\")"))
        XCTAssertTrue(mac.contains(".accessibilityIdentifier(\"workspace-container-\\(section.rawValue)\")"))
    ...
    func testMacWorkspaceAccessibilitySmokeScriptTargetsNavigation() throws {
        ...
        XCTAssertTrue(script.contains("for identifier in [target.panelIdentifier, target.renderedIdentifier]"))
        ...
        XCTAssertTrue(script.contains("waitForElement(withIdentifier: identifier, in: appElement, timeout: timeout)"))
        ...
        XCTAssertTrue(probe.contains("var selectionIdentifier: String { \"workspace-container-\\(rawValue)\" }"))
        XCTAssertTrue(probe.contains("var contentIdentifier: String { \"workspace-panel-workspace-\\(rawValue)\" }"))
        XCTAssertTrue(probe.contains("let requiredIdentifiers = [target.selectionIdentifier, target.contentIdentifier]"))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(deferredInlineItemDetail.contains("onChange(of: item.id)"))
        ...
        XCTAssertTrue(deferredInlineItemDetail.contains("try? await Task.sleep(nanoseconds: CompanionLargeList.detailRenderDelayNanoseconds)"))
        XCTAssertTrue(deferredInlineItemDetail.contains("CompanionInlineDetailPreparingView()"))
        ...
        XCTAssertTrue(remoteSettingRow.contains("로컬에 같은 파일이 있어도 다시 받습니다. 파일이 꼬였을 때만 잠깐 켜세요."))
    ...
    func testSettingsPanelsAreGroupedAndUseNaturalKoreanCopy() throws {
        ...
        XCTAssertTrue(macModel.contains("파일 강제 다시 받기"))
        ...
        XCTAssertTrue(macModel.contains("\"NOTICE_NATIVE_PREFORMATTED_PASTE_ONLY\": runtimeBoolConfigValue(.noticePreformattedPasteOnly, default: false)"))
        XCTAssertTrue(macModel.contains("\"FILE_FORCE_DOWNLOAD\": runtimeBoolConfigValue(.fileForceDownload, default: false)"))
        ...
        XCTAssertTrue(ios.contains("!$0.isHidden && !hiddenByActionItemIDs.contains($0.id)"))
        ...
        XCTAssertTrue(companionDashboardCategoryScreen.contains("CompanionDashboardDataLoadingCard(isServerConfigured: model.serverRelayConfigured)"))
        ...
        XCTAssertTrue(companionTasksScreen.contains("CompanionDashboardDataLoadingCard(isServerConfigured: model.serverRelayConfigured)"))
        ...
        XCTAssertTrue(workstationTasksWorkspace.contains("CompanionDashboardDataLoadingCard(isServerConfigured: model.serverRelayConfigured)"))
        ...
        XCTAssertTrue(dashboardCategoryInlineDetailPanel.contains("CompanionDashboardDataLoadingCard(isServerConfigured: model.serverRelayConfigured)"))
        XCTAssertTrue(ios.contains("\"KLMS_LOGIN_ASSIST_ENABLED\", \"KLMS_LOGIN_ASSIST_MODE\", \"KLMS_LOGIN_ASSIST_ALLOW_NONINTERACTIVE\""))
        XCTAssertTrue(ios.contains("\"FILE_FORCE_DOWNLOAD\""))
        ...
        XCTAssertTrue(ios.contains("\"NOTICE_NATIVE_PREFORMATTED_PASTE_ONLY\""))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(deferredMacWorkspacePanel.contains("try? await Task.sleep(nanoseconds: MacWorkspacePanelTiming.deferredContentDelayNanoseconds)"))
        ...
        XCTAssertTrue(ios.contains("CompanionInlineDetailPreparingView"))
        ...
        XCTAssertTrue(iosWorkstationOverviewPanel.contains("CompanionDashboardDataLoadingCard(isServerConfigured: data.isServerConfigured)"))
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertTrue(ios.contains("static let detailRenderDelayNanoseconds: UInt64 = 45_000_000"))
        XCTAssertTrue(deferredInlineItemDetail.contains("try? await Task.sleep(nanoseconds: CompanionLargeList.detailRenderDelayNanoseconds)"))
        ...
        XCTAssertTrue(ios.contains("private struct CompanionInlineDetailPreparingView"))
        XCTAssertTrue(deferredInlineItemDetail.contains("@State private var isReady = false"))
        XCTAssertTrue(deferredInlineItemDetail.contains("@State private var detailTask: Task<Void, Never>?"))
        XCTAssertTrue(deferredInlineItemDetail.contains("CompanionInlineDetailPreparingView()"))
        XCTAssertTrue(deferredInlineItemDetail.contains("ServerSyncItemInlineDetailPanel(item: item, model: model)"))
        XCTAssertTrue(deferredInlineItemDetail.contains("await Task.yield()"))
        XCTAssertTrue(deferredInlineItemDetail.contains("try? await Task.sleep(nanoseconds: CompanionLargeList.detailRenderDelayNanoseconds)"))
