...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacWorkspaceAccessibilitySmokeScriptTargetsNavigation() throws {
        ...
        let resizeProbeRoot = repoRoot.appendingPathComponent("tools/smoke_klms_mac_resize_hit_area.swift")
        ...
        let resizeProbe = try String(contentsOf: resizeProbeRoot, encoding: .utf8)
        ...
        XCTAssertTrue(script.contains("WorkspaceSmokeTarget(rawValue: \"settings\", title: \"설정\", expectedTexts: [\"이 기기에 바로 적용\"])"))
        XCTAssertTrue(mac.contains(".accessibilityIdentifier(\"workspace-compact-\\(section.rawValue)\")"))
        ...
        XCTAssertTrue(script.contains("var renderedIdentifier: String { contentRootIdentifier }"))
        ...
        XCTAssertTrue(script.contains("private func verifyCompactWorkspaceMenuOptions(appElement: AXUIElement) throws"))
        XCTAssertTrue(script.contains("\"workspace-compact-\\(target.rawValue)\""))
        XCTAssertTrue(script.contains("(759, \"medium\", nil, 0)"))
        XCTAssertTrue(script.contains("(760, \"compact\", \"rail\", 65)"))
        XCTAssertTrue(script.contains("(1_199, \"wide\", \"rail\", 65)"))
        XCTAssertTrue(script.contains("(1_200, \"wide\", \"sidebar\", 185)"))
        ...
        XCTAssertTrue(script.contains("var requiredIdentifiers = [target.renderedIdentifier]"))
        XCTAssertTrue(script.contains("if target.rawValue == \"dashboard\""))
        XCTAssertTrue(script.contains("requiredIdentifiers.append(primarySyncActionIdentifier)"))
        XCTAssertTrue(script.contains("let expectedPrimaryActionCount = target.rawValue == \"dashboard\" ? 1 : 0"))
        ...
        XCTAssertTrue(script.contains("withIdentifier: \"dashboard-command-column\""))
        XCTAssertTrue(script.contains("withIdentifier: \"dashboard-summary-column\""))
        XCTAssertTrue(script.contains("commandColumnFrame.maxY <= summaryColumnFrame.minY + 4"))
        XCTAssertTrue(script.contains("The sync section is not above the dashboard summary"))
        ...
        XCTAssertTrue(script.contains("SettingsSmokeTarget(rawValue: \"app\", expectedText: \"이 기기에 바로 적용\")"))
        ...
        XCTAssertTrue(script.contains("private func launchKLMSApplicationIfNeeded() -> NSRunningApplication?"))
        XCTAssertTrue(script.contains("private func runningKLMSApplication() -> NSRunningApplication?"))
        XCTAssertTrue(script.contains("private func matchesConfiguredAppPath(_ app: NSRunningApplication) -> Bool"))
        XCTAssertTrue(script.contains("guard let app = launchKLMSApplicationIfNeeded() else"))
        XCTAssertTrue(script.contains("private let appPath = environment[\"KLMS_MAC_APP_PATH\"]?.trimmingCharacters(in: .whitespacesAndNewlines)"))
        ...
        XCTAssertTrue(script.contains("let isFrontmost = NSWorkspace.shared.frontmostApplication?.processIdentifier == app.processIdentifier"))
        XCTAssertTrue(script.contains("if isFrontmost, hasUsableAccessibilityWindow(in: appElement)"))
        XCTAssertTrue(script.contains("let deadline = Date().addingTimeInterval(timeout)"))
        XCTAssertTrue(script.contains("timeout: min(0.15, timeout)"))
        XCTAssertTrue(script.contains("private func ensureAccessibilityWindowFits("))
        XCTAssertTrue(script.contains("try ensureAccessibilityWindowFits(requestedSize, window: window)"))
        XCTAssertTrue(resizeProbe.contains("private func waitUntilFrontmost("))
        XCTAssertTrue(resizeProbe.contains("guard waitUntilFrontmost(app: app, appElement: appElement, window: window)"))
        XCTAssertTrue(resizeProbe.contains("var frontmostStableSince: Date?"))
        XCTAssertTrue(resizeProbe.contains("now.timeIntervalSince(frontmostStableSince) >= 0.25"))
        ...
        XCTAssertTrue(basicActionsScript.contains("sendCommandQ(app: app)"))
        XCTAssertTrue(basicActionsScript.contains("private func matchesConfiguredAppPath(_ app: NSRunningApplication) -> Bool"))
        XCTAssertTrue(basicActionsScript.contains("whose unix id is \\(app.processIdentifier)"))
        ...
        XCTAssertFalse(basicActionsScript.contains("\"전체 기록 지우기\""))
        XCTAssertTrue(basicActionsScript.contains("\"실행 로그 지우기\""))
        XCTAssertTrue(basicActionsScript.contains("\"서버 로그 지우기\""))
        XCTAssertTrue(basicActionsScript.contains("KLMS_MAC_SMOKE_ALLOW_DESTRUCTIVE_ACTIONS"))
        XCTAssertTrue(basicActionsScript.contains("try verifyControl(label, appElement: appElement, press: allowDestructiveLogActions)"))
        XCTAssertTrue(basicActionsScript.contains("AXUIElementCopyActionNames(button, &actionNames)"))
        XCTAssertTrue(basicActionsScript.contains("guard press else { return }"))
        XCTAssertFalse(basicActionsScript.contains("try pressControl(label, appElement: appElement)"))
        XCTAssertTrue(basicActionsScript.contains("private func waitForButton("))
        XCTAssertTrue(basicActionsScript.contains("Expected Mac action control could not be pressed"))
        ...
        XCTAssertTrue(probe.contains("private func launchKLMSApplicationIfNeeded() -> NSRunningApplication?"))
        XCTAssertTrue(probe.contains("private func matchesConfiguredAppPath(_ app: NSRunningApplication) -> Bool"))
        XCTAssertTrue(probe.contains("guard let app = launchKLMSApplicationIfNeeded() else"))
        ...
        XCTAssertTrue(probe.contains("ProbeTarget(rawValue: \"activityLogs\", renderedTexts: [\"실행 로그 지우기\", \"서버 로그 지우기\"])"))
        XCTAssertTrue(probe.contains("ProbeTarget(rawValue: \"diagnostics\", renderedTexts: [\"상태 검사\", \"권한/환경 진단\"])"))
        XCTAssertTrue(probe.contains("var renderedTexts: [String]"))
        XCTAssertTrue(probe.contains("var renderIdentifier: String { \"workspace-content-root-\\(rawValue)\" }"))
        XCTAssertTrue(probe.contains("ProbeTarget(rawValue: \"files\", renderedTexts: [\"파일 목록\", \"필터와 검색\"])"))
        XCTAssertTrue(probe.contains("ProbeTarget(rawValue: \"tasks\", renderedTexts: [\"과제\", \"시험\", \"필터와 검색\"])"))
        XCTAssertTrue(probe.contains("ProbeTarget(rawValue: \"notices\", renderedTexts: [\"공지 분류\"])"))
        XCTAssertTrue(probe.contains("ProbeTarget(rawValue: \"calendar\", renderedTexts: [\"캘린더 일정\", \"KLMS 기준 반영\"])"))
        XCTAssertFalse(probe.contains("var contentIdentifier: String"))
        XCTAssertTrue(probe.contains("let requiredIdentifiers = [target.renderIdentifier]"))
        XCTAssertTrue(probe.contains("for renderedText in target.renderedTexts"))
        XCTAssertTrue(probe.contains("waitForText(renderedText, in: appElement, timeout: timeout)"))
        ...
        XCTAssertLessThan(
            try XCTUnwrap(probe.range(of: "let end = DispatchTime.now()")?.lowerBound),
            try XCTUnwrap(probe.range(of: "for renderedText in target.renderedTexts")?.lowerBound)
        )
        ...
        XCTAssertTrue(probe.contains("let isFrontmost = NSWorkspace.shared.frontmostApplication?.processIdentifier == app.processIdentifier"))
        XCTAssertTrue(probe.contains("if isFrontmost, hasUsableAccessibilityWindow(in: appElement)"))
    ...
    func testMacDashboardWindowFollowsApprovedWorkstationMockup() throws {
        ...
        displayedDetail = nil
        """))
        XCTAssertTrue(metricGrid.contains("GridItem(.adaptive(minimum: 118, maximum: 180), spacing: 8)"))
        ...
        XCTAssertTrue(primarySyncAction.contains(".padding(.horizontal, 14)"))
        ...
        XCTAssertTrue(primarySyncAction.contains(".contentShape(RoundedRectangle(cornerRadius: 12))"))
        XCTAssertTrue(commandPanel.contains(".contentShape(RoundedRectangle(cornerRadius: 10))"))
        XCTAssertTrue(primarySyncAction.contains(".buttonStyle(MacPressFeedbackButtonStyle(cornerRadius: 12, disabledOpacity: 1.0))"))
    ...
    func testMacAndIOSDirectInteractionAnimationsStayFast() throws {
        ...
        XCTAssertTrue(iosTaskCategorySelector.contains(".companionStableTap(cornerRadius: 10)"))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(compactTabBar.contains(".background(Color.klmsSubtleCardBackground.opacity(0.54), in: RoundedRectangle(cornerRadius: 12))"))
        ...
        XCTAssertTrue(compactSelectedRow.contains(".companionStableTap(cornerRadius: 14)"))
        ...
        XCTAssertTrue(remoteChangeSummaryDetail.contains(".companionStableTap(cornerRadius: 8)"))
        ...
        XCTAssertTrue(remoteSettingsPanelContent.contains("GridItem(.flexible(minimum: 0), spacing: 10, alignment: .top)"))
        ...
        XCTAssertTrue(ios.contains(".frame(minHeight: 44)"))
        ...
        XCTAssertFalse(companionItemListControls.contains("HStack(spacing: 8) {\n                        if yearOptions.count > 1"))
        ...
        XCTAssertTrue(dashboardSyncSurface.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: 12, disabledOpacity: 0.78))"))
        ...
        XCTAssertTrue(flowChipLayout.contains(".companionStableTap(cornerRadius: 8)"))
    ...
    func testSettingsPanelsAreGroupedAndUseNaturalKoreanCopy() throws {
        ...
        XCTAssertTrue(macSettings.contains("VStack(alignment: .trailing, spacing: 2)"))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(dashboardTopBarStatusContent.contains("HStack(alignment: snapshot.runningProgress == nil ? .center : .top, spacing: 12)"))
        ...
        XCTAssertTrue(iosCompactTrashButtonStyle.contains(".frame(width: 44, height: 44)"))
        ...
        XCTAssertTrue(iosSidebarButton.contains(".background(Color.klmsSubtleCardBackground.opacity(0.30), in: RoundedRectangle(cornerRadius: 12))"))
        ...
        XCTAssertTrue(iosMetricTile.contains(".companionStableTap(cornerRadius: 14, action: action)"))
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertTrue(adaptiveTwoColumnLayout.contains("let isHorizontal = availableWidth >= minimumHorizontalWidth"))
    ...
    func testIOSCalendarDetailHasMailPasteAnalyzerAndSharedCalendarActionLabels() throws {
        ...
        XCTAssertGreaterThanOrEqual(mailPastePanel.components(separatedBy: ".frame(maxWidth: .infinity, minHeight: 44)").count - 1, 3)
        ...
        XCTAssertTrue(mailPasteResultContent.contains(".companionStableTap(cornerRadius: 8)"))
    ...
    func testIOSAdaptiveNavigationUsesWideRailCompactStagesAndSafeKoreanCopy() throws {
        ...
        XCTAssertTrue(rail.contains(".frame(width: 44, height: 44)"))
    ...
    func testIOSServerDisplayItemActionsUpdateDashboardImmediately() throws {
        ...
        XCTAssertTrue(remoteItemToggleButton.contains(".companionStableTap(cornerRadius: 8)"))
    ...
    func testIOSCachesServerSummaryForFirstLaunchDashboard() throws {
        ...
        let resetRemoteSession = try sourceBody(
            after: "private func resetRemoteSessionForConnectionChange()",
            in: ios,
            description: "iOS remote session reset"
        )
        ...
        XCTAssertTrue(applyBody.contains("authoritativeIncoming: true"))
        XCTAssertTrue(ios.contains("private func applySharedSettings("))
        let applySharedSettings = try sourceBody(
            after: "private func applySharedSettings(",
            in: ios,
            description: "iOS shared settings apply"
        )
        XCTAssertTrue(ios.contains("private static let hiddenSharedSettingKeys: Set<String>"))
        XCTAssertTrue(applySharedSettings.contains("visibleIncomingSettings = incomingSettings.filter"))
        XCTAssertTrue(applySharedSettings.contains("visiblePreviousSettings = sharedSettings.filter"))
        XCTAssertTrue(applySharedSettings.contains("!Self.hiddenSharedSettingKeys.contains($0.key)"))
        XCTAssertTrue(applySharedSettings.contains("for setting in visibleIncomingSettings"))
        XCTAssertTrue(applySharedSettings.contains("nextByKey = Dictionary(uniqueKeysWithValues: visibleIncomingSettings.map"))
        XCTAssertTrue(applySharedSettings.contains("for (key, pendingSetting) in pendingSharedSettingsByKey"))
        XCTAssertTrue(applySharedSettings.contains("nextByKey[key] = pendingSetting"))
        XCTAssertTrue(applySharedSettings.contains("if authoritativeIncoming, !pendingSharedSettingsByKey.isEmpty"))
        XCTAssertTrue(applySharedSettings.contains("sharedSettingCommittedBaselines.recordCommittedValue("))
        XCTAssertTrue(applySharedSettings.contains("Self.serverRelaySetting(incoming, isNotOlderThan: committed)"))
        XCTAssertFalse(ios.contains("case \"KLMS_LOGIN_ASSIST_MODE\":"))
        XCTAssertTrue(applySharedSettings.contains("let previousByKey = Dictionary(uniqueKeysWithValues: visiblePreviousSettings.map { ($0.key, $0) })"))
        XCTAssertTrue(applySharedSettings.contains("nextByKey = previousByKey"))
        ...
        XCTAssertTrue(resetRemoteSession.contains("sharedSettingMutationQueue.cancelAll()"))
        XCTAssertTrue(resetRemoteSession.contains("sharedSettingCommittedBaselines.reset()"))
