...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacDashboardWindowFollowsApprovedWorkstationMockup() throws {
        ...
        XCTAssertTrue(dashboardFilterBar.contains("@State private var isExpanded = false"))
        XCTAssertTrue(dashboardFilterBar.contains("filterHeader"))
        XCTAssertTrue(dashboardFilterBar.contains("if !isExpanded, hasActiveFilter"))
        XCTAssertTrue(dashboardFilterBar.contains("collapsedActiveFilterSummary"))
        XCTAssertTrue(detail.contains("private struct DashboardFilterExpansionBadge"))
        ...
        XCTAssertTrue(noticeListView.contains("NoticeDashboardInputSignature(category: category, filters: filters, snapshot: snapshot)"))
        ...
        XCTAssertTrue(detail.contains("private struct NoticeDashboardPresentation"))
    ...
    func testMacFileDashboardAvoidsPerRowFilesystemTasks() throws {
        ...
        XCTAssertTrue(detail.contains("private struct DashboardFileListPresentation"))
        ...
        XCTAssertTrue(detail.contains("private struct DashboardStateItemListPresentation"))
        ...
        XCTAssertTrue(detail.contains("presentation = DashboardStateItemListPresentation(items: items, editor: editor, filters: filters, snapshot: snapshot)"))
        ...
        XCTAssertTrue(detail.contains("private var courseOptions: [String]"))
        XCTAssertTrue(detail.contains("private var yearOptions: [String]"))
        XCTAssertTrue(detail.contains("private var semesterOptions: [String]"))
        XCTAssertTrue(detail.contains("private var hiddenCount: Int"))
        XCTAssertTrue(detail.contains("self.courseOptions = DashboardCourseFilter.options(for: kind, snapshot: resolvedSnapshot)"))
        XCTAssertTrue(detail.contains("self.yearOptions = DashboardTermFilter.yearOptions(for: kind, snapshot: resolvedSnapshot)"))
        XCTAssertTrue(detail.contains("self.semesterOptions = DashboardTermFilter.semesterOptions(for: kind, snapshot: resolvedSnapshot)"))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(immediateSettingsPanel.contains(".transition(.opacity)"))
        XCTAssertTrue(immediateSettingsPanel.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: 12))"))
        ...
        XCTAssertTrue(dashboardSyncCard.contains("isDisabled ? Color.klmsPrimaryText.opacity(0.68) : Color.klmsPrimaryCommandButtonForeground"))
        XCTAssertTrue(dashboardSyncCard.contains("if isDisabled { return Color.klmsCommandButtonBackground }"))
        XCTAssertTrue(dashboardSyncCard.contains("if isDisabled { return Color.klmsCommandButtonBorder }"))
        ...
        XCTAssertTrue(remoteCommandPanel.contains("isDisabled ? Color.klmsPrimaryText.opacity(0.68) : Color.klmsPrimaryCommandButtonForeground"))
        XCTAssertTrue(remoteCommandPanel.contains("if isDisabled { return Color.klmsCommandButtonBackground }"))
        XCTAssertTrue(remoteCommandPanel.contains("if isDisabled { return Color.klmsCommandButtonBorder }"))
    ...
    func testSettingsPanelsAreGroupedAndUseNaturalKoreanCopy() throws {
        ...
        XCTAssertTrue(macSettings.contains("Text(\"필요한 묶음만 펼쳐서 바꿔 주세요.\")"))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(macSidebarView.contains("CommandPanelView(model: model)"))
        ...
        let sidebarCommandRange = try XCTUnwrap(macSidebarView.range(of: "CommandPanelView(model: model)"))
        ...
        XCTAssertLessThan(sidebarCommandRange.lowerBound, sidebarNavigationRange.lowerBound)
        ...
        XCTAssertTrue(taskWorkspaceBody.contains("GridItem(.adaptive(minimum: 260), spacing: 12)"))
        ...
        XCTAssertTrue(iosWorkstationOverviewPanel.contains("Text(\"대시보드 상세\")"))
        XCTAssertTrue(iosWorkstationOverviewPanel.contains("왼쪽 카드나 변경 요약을 누르면 이 패널에서 목록과 처리 버튼이 열립니다."))
        XCTAssertTrue(iosWorkstationOverviewPanel.contains("WorkstationDashboardSelectionGuide()"))
        ...
        XCTAssertTrue(ios.contains("private struct WorkstationDashboardSelectionGuide"))
        XCTAssertTrue(ios.contains("Text(\"선택하면 할 수 있는 일\")"))
        XCTAssertTrue(ios.contains("\"강의자료와 공지 첨부 파일을 같은 목록에서 확인합니다.\""))
