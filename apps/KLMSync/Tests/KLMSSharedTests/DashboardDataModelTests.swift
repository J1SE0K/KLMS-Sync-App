...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacAndIOSDirectInteractionAnimationsStayFast() throws {
        ...
        let iosPressPreviewModifier = try sourceBody(
            after: "private struct CompanionPressPreviewModifier: ViewModifier",
            in: ios,
            description: "iOS press preview modifier"
        )
        ...
        XCTAssertTrue(iosPressPreviewModifier.contains("DragGesture(minimumDistance: 0)"))
        XCTAssertTrue(iosPressPreviewModifier.contains(".simultaneousGesture("))
        XCTAssertTrue(iosPressPreviewModifier.contains("transaction.disablesAnimations = true"))
        XCTAssertTrue(iosItemListControls.contains("@State private var pressedChoiceKey: String?"))
        XCTAssertTrue(iosItemListControls.contains("let isVisuallySelected = isSelected || pressedChoiceKey == title"))
        XCTAssertTrue(iosItemListControls.contains(".companionPressPreview { isPressed in"))
        XCTAssertTrue(iosItemListControls.contains(".buttonStyle(KLMSStableSelectionButtonStyle(cornerRadius: 999))"))
        XCTAssertTrue(iosInlineItemRows.contains("@State private var pressedItemID: String?"))
        XCTAssertTrue(iosInlineItemRows.contains("private var visualSelectedItemID: String?"))
        XCTAssertTrue(iosInlineItemRows.contains("let isSelected = visualSelectedItemID == item.id"))
        XCTAssertTrue(iosInlineItemRows.contains(".companionPressPreview { isPressed in"))
        XCTAssertTrue(iosInlineItemRows.contains(".buttonStyle(KLMSStableSelectionButtonStyle())"))
        XCTAssertTrue(iosSelectableItemRows.contains("@State private var pressedItemID: String?"))
        XCTAssertTrue(iosSelectableItemRows.contains("private var visualSelectedItemID: String?"))
        XCTAssertTrue(iosSelectableItemRows.contains(".companionPressPreview { isPressed in"))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(mailAnalysisResult.contains(".accessibilityValue((pressedItemID ?? selectedItemID) == item.id ? \"펼쳐짐\" : \"접힘\")"))
        XCTAssertTrue(mailAnalysisResult.contains(".accessibilityHint((pressedItemID ?? selectedItemID) == item.id ? \"관련 KLMS 항목 상세와 처리 버튼을 접습니다.\" : \"관련 KLMS 항목 상세와 처리 버튼을 펼칩니다.\")"))
        ...
        XCTAssertTrue(remoteChangeSummaryDetail.contains(".accessibilityValue((pressedItemID ?? selectedItemID) == item.id ? \"펼쳐짐\" : \"접힘\")"))
        XCTAssertTrue(remoteChangeSummaryDetail.contains(".accessibilityHint((pressedItemID ?? selectedItemID) == item.id ? \"변경 항목 상세와 처리 버튼을 접습니다.\" : \"변경 항목 상세와 처리 버튼을 펼칩니다.\")"))
        ...
        XCTAssertTrue(companionItemListControls.contains(".buttonStyle(KLMSStableSelectionButtonStyle(cornerRadius: 8))"))
        ...
        XCTAssertTrue(flowChipLayout.contains(".buttonStyle(KLMSStableSelectionButtonStyle(cornerRadius: 8))"))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(iosMetricTile.contains(".buttonStyle(KLMSStableSelectionButtonStyle(cornerRadius: 14))"))
        ...
        XCTAssertTrue(iosWorkstationMetricCard.contains(".buttonStyle(KLMSStableSelectionButtonStyle(cornerRadius: 13))"))
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertTrue(selectableRows.contains(".accessibilityValue(visualSelectedItemID == item.id ? \"선택됨\" : \"선택 안 됨\")"))
        ...
        XCTAssertTrue(selectableRows.contains("ServerSyncDataRow(item: item, isSelected: visualSelectedItemID == item.id)"))
