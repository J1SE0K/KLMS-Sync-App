...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testSameTitleAssignmentsRequireUrlOrDeadlineSpecificOverride() throws {
        let first = try decodeStateItem(
            url: "",
            title: "Weekly Report",
            course: "Algorithms",
        ...
        let broadSnapshot = ManualOverridesSnapshot(assignments: [legacyKey: "completed"])

        XCTAssertEqual(broadSnapshot.assignmentStatus(for: first), "")
        XCTAssertEqual(broadSnapshot.assignmentStatus(for: second), "")
        XCTAssertFalse(
            ManualOverridesSnapshot.assignmentOverrideCandidateKeys(for: first).contains(legacyKey)
        )

        let firstCanonicalKey = ManualOverridesSnapshot.preferredAssignmentOverrideKey(for: first)
        let preciseSnapshot = ManualOverridesSnapshot(assignments: [firstCanonicalKey: "completed"])
        XCTAssertEqual(preciseSnapshot.assignmentStatus(for: first), "completed")
        XCTAssertEqual(preciseSnapshot.assignmentStatus(for: second), "")

        let updated = LegacySyncState(
            content: .init(assignments: [first, second])
        ).applyingManualOverrides(preciseSnapshot)
        XCTAssertEqual(updated.content.completedAssignments.map(\.syncDue), [first.syncDue])
        XCTAssertEqual(updated.content.assignments.map(\.syncDue), [second.syncDue])
    }

    func testLegacyCourseTitleAssignmentOverrideMigratesOnlyWhenItemIsExplicitlySelected() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("klms-dashboard-legacy-assignment-key-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }

        let selected = try decodeStateItem(
            url: "",
            title: "Weekly Report",
            course: "Algorithms",
            syncDue: "2026-07-20T23:59:00+09:00"
        )
        let later = try decodeStateItem(
            url: "",
            title: "Weekly Report",
            course: "Algorithms",
        ...
        let legacyKey = try XCTUnwrap(
            ManualOverridesSnapshot.legacyBroadAssignmentOverrideKey(for: selected)
        )
        let url = directory.appendingPathComponent("manual_assignment_overrides.json")
        try """
        {"assignments":{"\(legacyKey)":"completed"}}
        """.write(to: url, atomically: true, encoding: .utf8)

        let store = ManualOverrideStore(url: url)
        XCTAssertEqual(try store.load().assignmentStatus(for: selected), "")
        XCTAssertTrue(try store.migrateLegacyAssignmentOverride(for: selected))

        let migrated = try store.load()
        XCTAssertEqual(migrated.assignmentStatus(for: selected), "completed")
        XCTAssertEqual(migrated.assignmentStatus(for: later), "")
        XCTAssertNil(migrated.assignments[legacyKey])
        let raw = try XCTUnwrap(
            JSONSerialization.jsonObject(with: Data(contentsOf: url)) as? [String: Any]
        )
        XCTAssertNil(raw[legacyKey])
        XCTAssertFalse(try store.migrateLegacyAssignmentOverride(for: later))
    }
    ...
    func testMacAndIOSUseWholeScreenVerticalScroll() throws {
        ...
        XCTAssertTrue(mac.contains("WholeScreenVerticalScrollView("))
        XCTAssertTrue(mac.contains("resetID: MacWorkspaceScrollResetKey(section: selectedSection, nonce: scrollResetNonce)"))
        ...
        XCTAssertTrue(mac.contains("recentRunFailureDetail: model.recentRunFailureDetail"))
        XCTAssertTrue(mac.contains("return \"\\(recentRunFailureDetail) · 실행 로그에서 원인을 바로 확인할 수 있습니다.\""))
        XCTAssertTrue(mac.contains(".accessibilityHint(snapshot.detail)"))
        XCTAssertFalse(mac.contains("AccessibilityMarker"))
        XCTAssertFalse(mac.contains("WorkspaceNavigationSelectionMarker"))
        XCTAssertFalse(mac.contains(".font(.system(size: 1))"))
        XCTAssertTrue(mac.contains(".accessibilityIdentifier(\"workspace-layout-mode-\\(metrics.contentMode.rawValue)\")"))
        XCTAssertTrue(mac.contains(".accessibilityIdentifier(\"workspace-container-\\(selectedSection.rawValue)\")"))
        XCTAssertTrue(mac.contains(".accessibilityIdentifier(\"workspace-panel-\\(id)\")"))
        XCTAssertTrue(mac.contains("var accessibilitySummary: String"))
        XCTAssertTrue(mac.contains("\"대시보드 화면 · 전체 동기화 · 변경 요약\""))
        XCTAssertTrue(macRootBody.contains(".frame(width: metrics.navigationColumnWidth, alignment: .topLeading)"))
        ...
        XCTAssertTrue(macRootBody.contains(".frame(width: metrics.workspaceWidth, alignment: .topLeading)"))
        XCTAssertTrue(mac.contains(".fill(Color.klmsMacBorder.opacity(0.76))"))
    ...
    func testMacWorkspaceRuntimeProbeScriptsTypeCheck() throws {
        let packageRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let repoRoot = packageRoot
            ...
            .deletingLastPathComponent()
        let scripts = [
            "smoke_klms_mac_accessibility.swift",
            "smoke_klms_mac_basic_actions.swift",
            "smoke_klms_mac_resize_hit_area.swift",
            "probe_klms_mac_tab_response.swift",
        ].map { repoRoot.appendingPathComponent("tools/\($0)") }

        for script in scripts {
            let process = Process()
            process.executableURL = URL(fileURLWithPath: "/usr/bin/xcrun")
            process.arguments = ["swiftc", "-typecheck", script.path]
            ...
            XCTAssertEqual(
                ...
                0,
                "\(script.lastPathComponent) did not type-check:\n\(diagnosticText)"
            )
        }
    }
    func testIOSDeviceInstallHelperWaitsForUnavailableDevices() throws {
        ...
        let readinessWorkflowRoot = repoRoot.appendingPathComponent(".github/workflows/ui-readiness.yml")
        ...
        let readinessWorkflow = try String(contentsOf: readinessWorkflowRoot, encoding: .utf8)
        ...
        XCTAssertTrue(readinessScript.contains("record_step \"git-metadata\""))
        XCTAssertTrue(readinessScript.contains("rev-parse --verify 'HEAD^{commit}'"))
        XCTAssertTrue(readinessScript.contains("GIT_METADATA_STATE=\"invalid\""))
        XCTAssertFalse(readinessScript.contains("print -r -- unknown"))
        ...
        XCTAssertTrue(readinessScript.contains("mac-build|mac-relaunch|mac-accessibility-smoke|mac-resize-hit-area|mac-basic-actions|mac-tab-response"))
        ...
        XCTAssertTrue(readinessScript.contains("ALLOW_DESTRUCTIVE_ACTIONS=\"${KLMS_READINESS_ALLOW_DESTRUCTIVE_ACTIONS:-0}\""))
        XCTAssertTrue(readinessScript.contains("KLMS_MAC_SMOKE_ALLOW_DESTRUCTIVE_ACTIONS=\"$ALLOW_DESTRUCTIVE_ACTIONS\""))
        XCTAssertTrue(readinessWorkflow.contains("KLMS_READINESS_ALLOW_DESTRUCTIVE_ACTIONS: \"0\""))
    ...
    func testMacDashboardWindowFollowsApprovedWorkstationMockup() throws {
        ...
        let dashboardTopBar = try sourceStructBody(named: "DashboardTopBarView", in: view)
        let primarySyncAction = try sourceStructBody(named: "MacPrimarySyncActionView", in: view)
        let commandCenterLayout = try sourceBody(
            after: "private struct MacDashboardCommandCenterLayout: Layout",
            in: view,
            description: "Mac dashboard command-center layout"
        )
        ...
        XCTAssertFalse(dashboardRuntimePanel.contains("@State private var isExpanded = false"))
        ...
        XCTAssertTrue(dashboardFilterBar.contains("compactFilterLine"))
        ...
        XCTAssertTrue(dashboardFilterBar.contains("@State private var isAdvancedFiltersExpanded = false"))
        ...
        XCTAssertTrue(detail.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(detail.contains("let isInitialLoad = presentationSignature == nil"))
        XCTAssertTrue(detail.contains("let shouldDelay = !isInitialLoad && filters.shouldDebounceComparedTo(renderedFilters)"))
        ...
        XCTAssertTrue(app.contains("static let minWidth: CGFloat = 640"))
        XCTAssertTrue(app.contains("configureApplicationMenu()"))
        XCTAssertTrue(app.contains("NSApp.mainMenu = mainMenu"))
        XCTAssertTrue(app.contains("NSMenuItem(title: \"새로고침\", action: #selector(refreshStatusFromMenu), keyEquivalent: \"r\")"))
        XCTAssertTrue(app.contains("refreshItem.keyEquivalentModifierMask = [.command]"))
        XCTAssertTrue(app.contains("NSMenuItem(title: \"상태 검사\", action: #selector(runVerifyFromMenu), keyEquivalent: \"v\")"))
        XCTAssertTrue(app.contains("verifyItem.keyEquivalentModifierMask = [.command, .shift]"))
        XCTAssertTrue(app.contains("NSMenuItem(title: \"권한/환경 진단\", action: #selector(runDoctorFromMenu), keyEquivalent: \"d\")"))
        XCTAssertTrue(app.contains("doctorItem.keyEquivalentModifierMask = [.command, .shift]"))
        ...
        XCTAssertTrue(app.contains("await model.refreshVisibleStateFromShortcut()"))
        XCTAssertTrue(app.contains("@objc private func runVerifyFromMenu"))
        XCTAssertTrue(app.contains("await model.run(.verify)"))
        XCTAssertTrue(app.contains("@objc private func runDoctorFromMenu"))
        XCTAssertTrue(app.contains("await model.run(.doctor)"))
        XCTAssertTrue(app.contains("private var shortcutKeyMonitor: Any?"))
        ...
        let bootstrap = try sourceBody(
            ...
            in: macModel,
            description: "Mac startup bootstrap"
        )
        let initialReload = try XCTUnwrap(bootstrap.range(of: "await reloadEngineState()"))
        let engineInstall = try XCTUnwrap(bootstrap.range(of: "await installEngine(force: false, runDoctorAfterInstall: false)"))
        let permissionRequest = try XCTUnwrap(bootstrap.range(of: "await requestAppPermissions(markAutomatic: true)"))
        XCTAssertLessThan(initialReload.lowerBound, engineInstall.lowerBound)
        XCTAssertLessThan(initialReload.lowerBound, permissionRequest.lowerBound)
        ...
        XCTAssertTrue(app.contains("NSMenuItem(title: \"새로고침\""))
        XCTAssertFalse(app.contains("NSMenuItem(title: \"상태 갱신\""))
        XCTAssertTrue(macModel.contains("func refreshVisibleStateFromShortcut() async"))
        XCTAssertTrue(macModel.contains("await reloadEngineState()"))
        XCTAssertTrue(macModel.contains("await refreshServerRelayDashboardNow(silent: true)"))
        XCTAssertFalse(macModel.contains("guard runningCommand == nil else { return }"))
        XCTAssertTrue(app.contains("private enum KLMSMenuBarStatusIcon"))
        XCTAssertTrue(app.contains("image.isTemplate = true"))
        ...
        XCTAssertTrue(app.contains(".frame(minWidth: KLMSControlSize.minimumInteractive, minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(app.contains("panel.level = .statusBar"))
        XCTAssertTrue(app.contains("panel.setAccessibilityIdentifier(\"klms-auth-digits-overlay\")"))
        XCTAssertFalse(app.contains("NSImage(systemSymbolName: model.menuBarSystemImage"))
        XCTAssertFalse(macModel.contains("var menuBarSystemImage"))
        ...
        XCTAssertTrue(actionButtonStyle.contains(".frame(minWidth: KLMSControlSize.minimumInteractive, minHeight: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(iconButtonStyle.contains(".frame(width: KLMSControlSize.minimumInteractive, height: KLMSControlSize.minimumInteractive)"))
        ...
        displayedDetail = nil
        """))
        XCTAssertTrue(metricGrid.contains("GridItem(.adaptive(minimum: 118, maximum: 180), spacing: KLMSSpacing.standard)"))
        XCTAssertFalse(metricGrid.contains("let count = min(max(metrics.count, 1), 4)"))
        ...
        XCTAssertFalse(dashboardFilterBar.contains("disabled: years.count <= 1"))
        XCTAssertFalse(dashboardFilterBar.contains("disabled: semesters.count <= 1"))
        XCTAssertFalse(dashboardFilterBar.contains(".disabled(years.count <= 1)"))
        XCTAssertFalse(dashboardFilterBar.contains(".disabled(semesters.count <= 1)"))
        XCTAssertFalse(dashboardRangeField.contains("var disabled"))
        XCTAssertFalse(dashboardRangeField.contains("opacity(disabled"))
        ...
        XCTAssertFalse(view.contains(".disabled(!model.hasClearableExecutionRunLogs)"))
        ...
        let hitFrame = try XCTUnwrap(
            compactDangerStyle.range(of: ".frame(width: KLMSControlSize.minimumInteractive, height: KLMSControlSize.minimumInteractive)")?.lowerBound
        )
        ...
        XCTAssertTrue(workstationLayout.contains("DeferredMacWorkspacePanel(id: \"workspace-dashboard\", deferContent: false"))
        XCTAssertTrue(workstationLayout.contains("MacDashboardCommandCenterView("))
        XCTAssertTrue(workstationLayout.contains("layoutMode: layoutMode"))
        let macCommandCenter = try sourceStructBody(named: "MacDashboardCommandCenterView", in: view)
        XCTAssertFalse(macCommandCenter.contains("ViewThatFits(in: .horizontal)"))
        XCTAssertTrue(macCommandCenter.contains("MacDashboardCommandCenterLayout(isWide: layoutMode == .wide)"))
        XCTAssertTrue(macCommandCenter.contains(".frame(width: layoutMode == .wide ? 320 : nil, alignment: .topLeading)"))
        XCTAssertFalse(macCommandCenter.contains("if layoutMode == .wide"))
        XCTAssertTrue(macCommandCenter.contains("CommandPanelView(model: model, openRunLog: openRunLog)"))
        XCTAssertTrue(macCommandCenter.contains("DashboardSummaryView(model: model)"))
        XCTAssertTrue(macCommandCenter.contains(".accessibilityIdentifier(\"dashboard-command-column\")"))
        XCTAssertTrue(macCommandCenter.contains(".accessibilityIdentifier(\"dashboard-summary-column\")"))
        XCTAssertTrue(commandCenterLayout.contains("height: commandSize.height + spacing + dashboardSize.height"))
        XCTAssertTrue(commandCenterLayout.contains("at: CGPoint(x: bounds.minX, y: bounds.minY + commandSize.height + spacing)"))
        let stackedPlacementStart = try XCTUnwrap(commandCenterLayout.range(of: "let commandSize = subviews[0].sizeThatFits(\n            ProposedViewSize(width: bounds.width, height: nil)"))
        let stackedPlacement = commandCenterLayout[stackedPlacementStart.lowerBound...]
        let stackedCommandIndex = try XCTUnwrap(stackedPlacement.range(of: "subviews[0].place(\n            at: bounds.origin")?.lowerBound)
        let stackedDashboardIndex = try XCTUnwrap(stackedPlacement.range(of: "subviews[1].place(\n            at: CGPoint(x: bounds.minX, y: bounds.minY + commandSize.height + spacing)")?.lowerBound)
        XCTAssertLessThan(stackedPlacement.distance(from: stackedPlacement.startIndex, to: stackedCommandIndex), stackedPlacement.distance(from: stackedPlacement.startIndex, to: stackedDashboardIndex))
        XCTAssertTrue(view.contains("private struct DashboardCommandCenterStatusCard"))
        ...
        XCTAssertTrue(navigationView.contains("RoundedRectangle(cornerRadius: KLMSRadius.card)"))
        XCTAssertFalse(navigationView.contains("WorkspaceNavigationSelectionMarker"))
        ...
        XCTAssertTrue(navigationView.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertTrue(primarySyncAction.contains(".font(.title3.weight(.black))"))
        XCTAssertTrue(primarySyncAction.contains(".fixedSize(horizontal: false, vertical: true)"))
        XCTAssertTrue(commandPanel.contains("commandStatusStrip"))
        XCTAssertTrue(commandPanel.contains("MacPrimarySyncActionView(model: model)"))
        XCTAssertTrue(commandPanel.contains("var openRunLog: () -> Void"))
        XCTAssertTrue(commandPanel.contains("if canOpenRunLog"))
        XCTAssertTrue(commandPanel.contains("openRunLog()"))
        XCTAssertTrue(commandPanel.contains("private var canOpenRunLog: Bool"))
        XCTAssertTrue(commandPanel.contains("|| !model.commandHistory.records.isEmpty"))
        ...
        XCTAssertFalse(commandPanel.contains(".onTapGesture"))
        XCTAssertFalse(commandPanel.contains(".accessibilityAction"))
        XCTAssertFalse(commandPanel.contains("runOrCancelIfAvailable(command, isDisabled: isDisabled)"))
        XCTAssertFalse(commandPanel.contains("cancelRunningCommandIfAvailable()"))
        XCTAssertTrue(primarySyncAction.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.card))"))
        XCTAssertTrue(commandPanel.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.control))"))
        XCTAssertTrue(primarySyncAction.contains(".buttonStyle(MacPressFeedbackButtonStyle(cornerRadius: KLMSRadius.card, disabledOpacity: 1.0))"))
        XCTAssertTrue(commandPanel.contains(".buttonStyle(MacPressFeedbackButtonStyle(disabledOpacity: 1.0))"))
        XCTAssertTrue(commandPanel.contains(".accessibilityIdentifier(\"command-cancel-current\")"))
        XCTAssertTrue(commandPanel.contains(".accessibilityIdentifier(\"command-\\(command.rawValue)\")"))
        XCTAssertFalse(commandPanel.contains(".accessibilityIdentifier(\"command-cancel\")"))
        XCTAssertTrue(commandPanel.contains(".accessibilityLabel(isRunning ? \"\\(command.displayName) 중단\" : \"\\(command.displayName) 실행\")"))
        ...
        XCTAssertTrue(primarySyncAction.contains("return model.isCancellingCommand ? \"중단 요청 중\" : \"전체 동기화 중단\""))
        XCTAssertTrue(primarySyncAction.contains("runningIdentity: model.runningCommandIdentity"))
        XCTAssertTrue(primarySyncAction.contains("cancelRunningCommand(expectedIdentity: expectedIdentity)"))
        XCTAssertTrue(macModel.contains("private var runningCommandOperationID: UInt64 = 0"))
        XCTAssertTrue(macModel.contains("if let expectedIdentity, expectedIdentity != runningCommandIdentity"))
        XCTAssertTrue(primarySyncAction.contains("Image(systemName: systemImage)"))
        ...
        XCTAssertTrue(primarySyncAction.contains("return Color.klmsMacSubtleCardBackground.opacity(0.86)"))
        XCTAssertTrue(primarySyncAction.contains("return Color.klmsMacCommandButtonBorder.opacity(0.64)"))
        ...
        XCTAssertTrue(pressFeedbackStyle.contains(".scaleEffect(configuration.isPressed && isEnabled ? 0.985 : 1.0)"))
        XCTAssertTrue(pressFeedbackStyle.contains("Color.klmsMacCommandButtonPressedOverlay.opacity(configuration.isPressed && isEnabled ? 0.18 : 0.0)"))
        XCTAssertTrue(pressFeedbackStyle.contains(".animation(.easeOut(duration: 0.08), value: configuration.isPressed)"))
        ...
        XCTAssertTrue(dashboardFilterBar.contains("ViewThatFits(in: .horizontal)"))
        XCTAssertTrue(dashboardFilterBar.contains("searchControl"))
        XCTAssertTrue(dashboardFilterBar.contains("rangeControl"))
        XCTAssertTrue(dashboardFilterBar.contains("displayControl"))
        XCTAssertTrue(view.contains("alpha: 0.105"))
        XCTAssertTrue(view.contains("alpha: 0.140"))
        XCTAssertFalse(rootActionButtonStyle.contains("background(isPressed: configuration.isPressed)"))
        XCTAssertFalse(rootActionButtonStyle.contains("background(isPressed:"))
        XCTAssertFalse(rootActionButtonStyle.contains("Color.klmsMacCommandButtonPressedBackground"))
        XCTAssertFalse(rootActionButtonStyle.contains("Color.klmsMacPrimaryCommandButtonPressedBackground"))
        ...
        XCTAssertTrue(rootActionButtonStyle.contains("Color.klmsMacDangerBorder.opacity(0.84)"))
        XCTAssertFalse(rootActionButtonStyle.contains("Color.klmsMacDangerBackground"))
        XCTAssertFalse(actionButtonStyle.contains("Color.klmsMacCommandButtonPressedBackground"))
        XCTAssertFalse(actionButtonStyle.contains("Color.klmsMacPrimaryCommandButtonPressedBackground"))
        XCTAssertFalse(iconButtonStyle.contains("Color.klmsMacCommandButtonPressedBackground"))
        XCTAssertFalse(iconButtonStyle.contains("Color.klmsMacPrimaryCommandButtonBorder.opacity(0.46)"))
        XCTAssertTrue(iconButtonStyle.contains(".frame(width: KLMSControlSize.minimumInteractive, height: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(commandPanel.contains("Color.klmsMacDangerBorder.opacity(0.10)"))
        XCTAssertTrue(commandPanel.contains("KLMSMacCompactDangerIconButtonStyle"))
        ...
        XCTAssertFalse(commandPanel.contains(".background(Color.klmsMacDangerBackground, in: RoundedRectangle(cornerRadius: KLMSRadius.control))"))
        ...
        XCTAssertTrue(actionButtonStyle.contains("RoundedRectangle(cornerRadius: KLMSRadius.control)"))
        XCTAssertTrue(actionButtonStyle.contains(".padding(.vertical, KLMSSpacing.standard)"))
        ...
        XCTAssertTrue(detailPressFeedbackStyle.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertFalse(actionButtonStyle.contains("background(isPressed: configuration.isPressed)"))
        XCTAssertFalse(actionButtonStyle.contains(".opacity(isEnabled ? (configuration.isPressed ? 0.96 : 1.0)"))
    ...
    func testMacAndIOSUseSeparatedLightAndDarkThemeTokens() throws {
        ...
        XCTAssertTrue(mac.contains(".foregroundStyle(foregroundColor)"))
        ...
        XCTAssertTrue(macSettingsButtonStyle.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
    ...
    func testMacAndIOSDirectInteractionAnimationsStayFast() throws {
        ...
        XCTAssertTrue(sources.contains("func companionStableTap("))
        XCTAssertTrue(ios.contains("companionPerformWithoutAnimation(action)"))
        XCTAssertFalse(sources.contains(".onTapGesture {"))
        XCTAssertFalse(sources.contains("CompanionPressPreviewModifier"))
        XCTAssertFalse(sources.contains("companionPressPreview"))
        XCTAssertFalse(sources.contains("pressedChoiceKey"))
        XCTAssertFalse(sources.contains("pressedItemID"))
        ...
        XCTAssertTrue(ios.contains(".scaleEffect(configuration.isPressed && isEnabled ? 0.97 : 1.0)"))
        XCTAssertTrue(mac.contains(".scaleEffect(configuration.isPressed && isEnabled ? 0.985 : 1.0)"))
        ...
        XCTAssertTrue(sources.contains("private static let authDigitsMinimumVisibleNanoseconds: UInt64 = 6_000_000_000"))
        XCTAssertTrue(sources.contains("private static let authDigitsDisplayTimeoutNanoseconds: UInt64 = 120_000_000_000"))
        XCTAssertTrue(sources.contains("private static let authStatusDisplayTimeoutNanoseconds: UInt64 = 120_000_000_000"))
        XCTAssertTrue(sources.contains("private var authDigitsClearTask: Task<Void, Never>?"))
        XCTAssertTrue(sources.contains("private var lastAuthDigitsRecordedAt: Date?"))
        ...
        XCTAssertTrue(sources.contains("liveAuthDigits != digits || !authDigitsSeenForCurrentRun"))
        XCTAssertTrue(sources.contains("await recordAuthDigits(digits)\n            return"))
        XCTAssertTrue(sources.contains("respectMinimumVisibleDuration: Bool = true"))
        XCTAssertTrue(sources.contains("await clearAuthDigitsState(showAuthenticatedMessage: false, respectMinimumVisibleDuration: false)"))
        ...
        XCTAssertFalse(sources.contains("runningSnapshotRefreshIntervalNanoseconds"))
        XCTAssertTrue(sources.contains("private static let fileSystemEventDebounceNanoseconds: UInt64 = 150_000_000"))
        XCTAssertTrue(sources.contains("KLMSFileSystemEventWatcher"))
        XCTAssertTrue(logTextBlock.contains("@State private var highlights: [KLMSLogHighlight]"))
        XCTAssertTrue(logTextBlock.contains(".task(id: highlightSourceText)"))
        ...
        XCTAssertTrue(iosTaskCategorySelector.contains(".companionStableTap(cornerRadius: KLMSRadius.control)"))
        XCTAssertFalse(iosDeferredSelectionDetail.contains("@State private var renderTask: Task<Void, Never>?"))
        XCTAssertFalse(iosDeferredSelectionDetail.contains("try? await Task.sleep"))
        XCTAssertTrue(iosDeferredSelectionDetail.contains("shouldRender = expanded"))
        XCTAssertTrue(iosDeferredSelectionDetail.contains("transaction.disablesAnimations = true"))
        XCTAssertFalse(iosInlineItemRows.contains("DeferredSelectionDetail(isExpanded: presentation == .inlineDetail && selectedItemID == item.id)"))
        XCTAssertTrue(iosInlineItemRows.contains("CompanionFloatingItemDetailOverlay(item: selectedItem, model: model)"))
        XCTAssertTrue(iosInlineItemRows.contains(".companionStableTap(action: { select(item) })"))
        XCTAssertTrue(iosControlBox.contains(".clipShape(RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
        XCTAssertTrue(iosSearchFilterPanel.contains(".clipShape(RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
    ...
    func testMacFileDashboardAvoidsPerRowFilesystemTasks() throws {
        ...
        XCTAssertTrue(fileItem.contains("var sourceURL: String"))
        XCTAssertTrue(fileItem.contains("searchBlob = [academicTerm?.displayName ?? \"\", title, course, path, url, sourceURL]"))
        XCTAssertTrue(fileItem.contains("hasher.combine(bucket)"))
        XCTAssertTrue(fileItem.contains("hasher.combine(sourceURL)"))
        XCTAssertTrue(fileItem.contains("hasher.combine(fileKindLabel)"))
        XCTAssertTrue(fileItem.contains("DashboardFileKindStyle(\n            bucket: bucket,\n            title: title,\n            path: sortPath.isEmpty ? path : sortPath,\n            url: url,\n            sourceURL: sourceURL\n        )"))
        XCTAssertTrue(detail.contains("private static func hasAssignmentSignal(in text: String) -> Bool"))
        XCTAssertTrue(detail.contains("private static func hasExamSignal(in text: String) -> Bool"))
        XCTAssertTrue(detail.contains("\"과제 관련\""))
        XCTAssertTrue(detail.contains("\"시험/퀴즈\""))
        XCTAssertTrue(detail.contains("label = \"과제 공지 첨부\""))
        XCTAssertTrue(detail.contains("label = \"시험/퀴즈 공지 첨부\""))
        XCTAssertTrue(detail.contains("case \"resources\":\n            label = \"강의 자료\""))
        XCTAssertTrue(detail.contains("case \"folders\":\n            label = \"폴더 자료\""))
        XCTAssertTrue(detail.contains("containsTokenPrefix(in: text, prefixes: [\"hw\", \"wa\", \"pa\"])"))
        ...
        XCTAssertFalse(detail.contains("recencyText: manifest?.localDownloadedAt ?? \"\""))
        ...
        XCTAssertFalse(fileListContentView.contains(".id(sortOption.rawValue)"))
        XCTAssertTrue(detail.contains("visibleLimit = dashboardAdjustedVisibleLimit(visibleLimit, resultCount: nextPresentation.files.count)"))
        ...
        XCTAssertTrue(detail.contains("visibleLimit = dashboardAdjustedVisibleLimit(visibleLimit, resultCount: nextPresentation.items.count)"))
        ...
        XCTAssertTrue(detail.contains("DashboardFileRenderSignature(\n                snapshot: resolvedSnapshot,\n                serverItems: model.dashboardServerRelayItems(for: .files)\n            )"))
        XCTAssertTrue(detail.contains("init(snapshot: EngineSnapshot, serverItems: [ServerRelaySyncItem] = [])"))
        XCTAssertTrue(detail.contains("for item in serverItems where item.kind == \"file\""))
        ...
        XCTAssertTrue(detail.contains(".accessibilityLabel(\"로컬 파일 열기 또는 미리보기\")"))
        XCTAssertTrue(detail.contains(".accessibilityLabel(\"Finder에서 파일 위치 보기\")"))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        let compactRoot = try sourceStructBody(named: "CompanionAdaptiveRootView", in: ios)
        ...
        XCTAssertLessThan(
            compactRoot.range(of: "CompanionStableSectionPane(section: currentSection, model: model, openSection: select)")?.lowerBound ?? compactRoot.endIndex,
            compactRoot.range(of: "CompanionCompactTabBar(selectedSection: sectionBinding)")?.lowerBound ?? compactRoot.startIndex,
        ...
        XCTAssertTrue(compactTabBar.contains("@Environment(\\.dynamicTypeSize) private var dynamicTypeSize"))
        XCTAssertTrue(compactTabBar.contains("if dynamicTypeSize.isAccessibilitySize"))
        XCTAssertTrue(compactTabBar.contains("Array(tabs.prefix(3))"))
        XCTAssertTrue(compactTabBar.contains("Array(tabs.dropFirst(3).prefix(2))"))
        XCTAssertTrue(compactTabBar.contains("Array(tabs.dropFirst(5))"))
        XCTAssertTrue(compactTabBar.contains("return [Array(tabs.prefix(4)), Array(tabs.dropFirst(4))]"))
        ...
        XCTAssertTrue(compactTabBar.contains(".frame(maxWidth: .infinity, minHeight: 48)"))
        ...
        XCTAssertTrue(sidebarButton.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertFalse(compactTabBar.contains("? Color.klmsSelectedBackground"))
        XCTAssertTrue(compactTabBar.contains(".background(Color.klmsSubtleCardBackground.opacity(0.54), in: RoundedRectangle(cornerRadius: KLMSRadius.card))"))
        XCTAssertTrue(compactTabBar.contains(".fill(isSelected ? Color.klmsSelectedForeground : Color.clear)"))
        XCTAssertTrue(compactTabBar.contains("RoundedRectangle(cornerRadius: KLMSRadius.card)"))
        XCTAssertTrue(compactTabBar.contains(".stroke(Color.klmsBorder.opacity(0.38), lineWidth: 1)"))
        ...
        XCTAssertTrue(remoteDiagnosticPanel.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertTrue(remoteSettingGroupSection.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.control))"))
        ...
        XCTAssertTrue(compactTabBar.contains(".padding(KLMSSpacing.compact)"))
        XCTAssertFalse(compactTabBar.contains(".padding(.horizontal, KLMSSpacing.compact)"))
        XCTAssertFalse(compactTabBar.contains(".padding(7)"))
        XCTAssertFalse(compactTabBar.contains(".frame(height: 56)"))
        XCTAssertTrue(compactTabBar.contains("RoundedRectangle(cornerRadius: KLMSRadius.panel)"))
        XCTAssertTrue(compactTabBar.contains("RoundedRectangle(cornerRadius: KLMSRadius.card)"))
        ...
        XCTAssertTrue(statusScreen.contains("title: \"대시보드\""))
        XCTAssertFalse(statusScreen.contains("showsAttentionStack: false"))
        XCTAssertFalse(statusScreen.contains("CompanionScreenContainer(title: \"상태\""))
        ...
        XCTAssertTrue(statusScreen.contains("if layoutMode != .wide"))
        ...
        XCTAssertTrue(statusScreen.contains("displayedDashboardPreview = category"))
        XCTAssertFalse(statusScreen.contains("deferDashboardPreview(category)"))
        ...
        XCTAssertTrue(quickAccessGrid.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertTrue(authSuccessBanner.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertTrue(errorBanner.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(loginAttentionBanner.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertTrue(remoteRunningStatusBanner.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(statusSummaryColumn.contains("showsLoadingPlaceholder: false"))
        XCTAssertTrue(statusSummaryColumn.contains("CompanionDisconnectedRecoveryPanel(openSettings: onOpenSettings)"))
        XCTAssertLessThan(
            try XCTUnwrap(statusSummaryColumn.range(of: "RemoteDashboardSyncCard(")).lowerBound,
            try XCTUnwrap(statusSummaryColumn.range(of: "CompanionDashboardScopeBar(")).lowerBound
        )
        XCTAssertLessThan(
        ...
        XCTAssertTrue(statusScreen.contains("displayedDashboardPreview = category"))
        XCTAssertFalse(statusScreen.contains("dashboardPreviewTask"))
        ...
        XCTAssertTrue(statusScreen.contains("VStack(alignment: .leading, spacing: KLMSSpacing.section)"))
        ...
        XCTAssertTrue(remoteChangeSummary.contains("if layoutMode != .wide"))
        ...
        XCTAssertTrue(ios.contains(".buttonStyle(KLMSActionButtonStyle(tone: .primary))"))
        ...
        XCTAssertTrue(compactSelectedRow.contains(".companionStableTap(cornerRadius: KLMSRadius.panel)"))
        ...
        XCTAssertTrue(mailAnalysisResult.contains(".companionStableTap(cornerRadius: KLMSRadius.smallSurface)"))
        ...
        XCTAssertTrue(remoteChangeSummaryDetail.contains(".companionStableTap(cornerRadius: KLMSRadius.smallSurface)"))
        XCTAssertTrue(remoteChangeSummaryDetail.contains(".accessibilityValue(selectedItemID == item.id ? \"선택됨\" : \"선택 안 됨\")"))
        XCTAssertTrue(remoteChangeSummaryDetail.contains(".accessibilityHint(\"변경 항목 상세와 처리 버튼을 화면 아래 고정 패널에 표시합니다.\")"))
        ...
        XCTAssertTrue(ios.contains(".frame(width: KLMSControlSize.minimumInteractive, height: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(immediateSettingsPanel.contains("@AppStorage(\"KLMSAppearanceMode\") private var localAppearanceModeRaw"))
        XCTAssertTrue(immediateSettingsPanel.contains("statusText: effectiveAppearanceMode.title"))
        XCTAssertTrue(immediateSettingsPanel.contains("selectedMode: effectiveAppearanceMode"))
        ...
        XCTAssertTrue(appearanceModeSelector.contains("@AppStorage(\"KLMSAppearanceMode\") private var localAppearanceModeRaw"))
        XCTAssertTrue(appearanceModeSelector.contains("localAppearanceModeRaw = mode.rawValue"))
        XCTAssertTrue(appearanceModeSelector.contains("let isSelected = effectiveSelectedMode == mode"))
        XCTAssertTrue(appearanceModeSelector.contains(".opacity(isSelected ? 1.0 : 0.0)"))
        XCTAssertTrue(appearanceModeSelector.contains(".frame(width: 14)"))
        ...
        XCTAssertTrue(appearanceModeSelector.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(appearanceModeSelector.contains(".accessibilityLabel(\"화면 모드 \\(mode.title)\")"))
        XCTAssertTrue(appearanceModeSelector.contains(".accessibilityValue(effectiveSelectedMode == mode ? \"선택됨\" : \"선택 안 됨\")"))
        XCTAssertTrue(appearanceModeSelector.contains(".accessibilityHint(\"KLMS Sync 화면 모드를 \\(mode.title)으로 바꿉니다.\")"))
        XCTAssertTrue(appearanceModeSelector.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: KLMSRadius.card))"))
        XCTAssertFalse(appearanceModeSelector.contains(".disabled(isSubmitting)"))
        XCTAssertTrue(settingsScreen.contains("@Environment(\\.companionLayoutMode)"))
        XCTAssertTrue(settingsScreen.contains("if layoutMode == .wide"))
        ...
        XCTAssertTrue(settingsScreen.contains("noticeNotesUpdatePending: model.isSharedNoticeUpdatePending"))
        ...
        XCTAssertTrue(settingsScreen.contains("RemoteSettingsPanel(model: model, usesWideGrid: layoutMode == .wide)"))
        ...
        XCTAssertTrue(immediateSettingsPanel.contains("화면 모드는 바로 적용됩니다.\\n공지 메모는 서버에 저장됩니다."))
        XCTAssertTrue(immediateSettingsPanel.contains("기기 설정을 따릅니다.\\n또는 앱 화면 모드를 선택합니다."))
        ...
        XCTAssertFalse(immediateSettingsPanel.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: KLMSRadius.card))"))
        ...
        XCTAssertTrue(remoteSettingsPanelContent.contains("private var settingGridColumns: [GridItem]"))
        XCTAssertTrue(remoteSettingsPanelContent.contains("GridItem(.flexible(minimum: 0), spacing: KLMSSpacing.comfortable, alignment: .top)"))
        XCTAssertFalse(remoteSettingsPanelContent.contains("private var settingGroupNavigation: some View"))
        XCTAssertFalse(remoteSettingsPanelContent.contains("Text(\"설정 섹션\")"))
        XCTAssertFalse(remoteSettingsPanelContent.contains("settingGroupNavigationButton("))
        XCTAssertFalse(remoteSettingsPanelContent.contains("private var visibleSettingGroups: [RemoteSettingGroup]"))
        ...
        XCTAssertFalse(remoteSettingRow.contains("Button(settingChoiceTitle(option))"))
        XCTAssertFalse(remoteSettingRow.contains("private func settingChoiceChip(_ option: String) -> some View"))
        XCTAssertTrue(remoteSettingRow.contains("Text(settingValueSummary)"))
        XCTAssertTrue(remoteSettingRow.contains("CompanionNativeChoiceField("))
        ...
        XCTAssertFalse(remoteSettingsPanelContent.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: KLMSRadius.card))"))
        ...
        XCTAssertTrue(remoteSettingGroupSection.contains("Color.klmsSubtleCardBackground.opacity(0.86)"))
        XCTAssertTrue(remoteSettingGroupSection.contains("ForEach(group.settings)"))
        XCTAssertTrue(remoteSettingGroupSection.contains(".stroke(Color.klmsBorder.opacity(0.86), lineWidth: 1)"))
        ...
        XCTAssertTrue(remoteDiagnosticPanel.contains("RoundedRectangle(cornerRadius: KLMSRadius.panel)"))
        ...
        XCTAssertTrue(relayConnectionPanel.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: KLMSRadius.card))"))
        ...
        XCTAssertFalse(remotePrivacyNote.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: KLMSRadius.card))"))
        ...
        XCTAssertTrue(remotePrivacyNote.contains("RoundedRectangle(cornerRadius: KLMSRadius.panel)"))
        XCTAssertTrue(dashboardSyncSurface.contains("primaryCommandTitle(isRunning: isRunning)"))
        XCTAssertTrue(dashboardSyncSurface.contains("primaryCommandSystemImage(isRunning: isRunning, isDisabled: isDisabled)"))
        XCTAssertTrue(dashboardSyncSurface.contains("return \"전체 동기화\""))
        XCTAssertFalse(dashboardSyncSurface.contains("return \"Mac 연결 필요\""))
        XCTAssertFalse(dashboardSyncSurface.contains("return \"잠시 대기\""))
        ...
        XCTAssertTrue(dashboardSyncCard.contains("서버 URL과 클라이언트 토큰을 저장하면 동기화 요청과 대시보드 갱신을 사용할 수 있습니다."))
        XCTAssertTrue(dashboardSyncCard.contains("hasInFlightRequest && !isCommandActive(kind)"))
        XCTAssertFalse(dashboardSyncCard.contains("!isRemoteAvailable || (hasInFlightRequest && !isCommandActive(kind))"))
        XCTAssertTrue(dashboardSyncSurface.contains("isDisabled ? Color.klmsSecondaryText.opacity(0.76) : Color.klmsPrimaryCommandButtonForeground"))
        XCTAssertTrue(dashboardSyncSurface.contains("if isDisabled { return Color.klmsSubtleCardBackground.opacity(0.86) }"))
        XCTAssertTrue(dashboardSyncSurface.contains("if isDisabled { return Color.klmsCommandButtonBorder.opacity(0.64) }"))
        ...
        XCTAssertTrue(ios.contains("let intent = RemoteCommandActionIntent.capture("))
        XCTAssertTrue(ios.contains("cancelRunningCommand(expectedCommandID: commandID)"))
        XCTAssertTrue(dashboardPrimarySyncAction.contains(".font(.system(.title3, design: .rounded, weight: .heavy))"))
        XCTAssertFalse(companionScreenHeader.contains("RemoteDashboardPrimarySyncAction"))
        XCTAssertTrue(dashboardSyncCard.contains("RemoteDashboardPrimarySyncAction(model: model, compact: compact)"))
        XCTAssertTrue(dashboardSyncCard.contains(".accessibilityIdentifier(\"dashboard-sync-section\")"))
        XCTAssertTrue(dashboardPrimarySyncAction.contains(".accessibilityIdentifier(\"dashboard-primary-full-sync\")"))
        XCTAssertFalse(dashboardSyncCard.contains(".fullSync"))
        XCTAssertLessThan(
            try XCTUnwrap(dashboardSyncCard.range(of: "RemoteDashboardPrimarySyncAction(model: model, compact: compact)")).lowerBound,
            try XCTUnwrap(dashboardSyncCard.range(of: "RemoteDashboardSyncCardContent(")).lowerBound
        )
        ...
        let iosYearFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "title: \"연도\"")?.lowerBound)
        let iosSemesterFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "title: \"학기\"")?.lowerBound)
        let iosCourseFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "title: \"과목\"")?.lowerBound)
        let iosStatusFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "title: \"상태\"")?.lowerBound)
        let iosVisibilityFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "title: \"표시 범위\"")?.lowerBound)
        XCTAssertLessThan(companionItemListControls.distance(from: companionItemListControls.startIndex, to: iosSortFieldIndex), companionItemListControls.distance(from: companionItemListControls.startIndex, to: iosYearFieldIndex))
        ...
        XCTAssertLessThan(companionItemListControls.distance(from: companionItemListControls.startIndex, to: iosCourseFieldIndex), companionItemListControls.distance(from: companionItemListControls.startIndex, to: iosStatusFieldIndex))
        XCTAssertLessThan(companionItemListControls.distance(from: companionItemListControls.startIndex, to: iosStatusFieldIndex), companionItemListControls.distance(from: companionItemListControls.startIndex, to: iosVisibilityFieldIndex))
        XCTAssertTrue(companionItemListControls.contains("Label(\"초기화\", systemImage: \"arrow.counterclockwise\")"))
        XCTAssertTrue(companionItemListControls.contains(".buttonStyle(KLMSCompactResetButtonStyle())"))
        XCTAssertTrue(ios.contains("private struct KLMSCompactResetButtonStyle: ButtonStyle"))
        XCTAssertTrue(ios.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
        XCTAssertFalse(companionItemListControls.contains("Label(\"필터 지우기\", systemImage: \"arrow.counterclockwise\")"))
        XCTAssertFalse(companionItemListControls.contains("Picker(\"연도\""))
        XCTAssertFalse(companionItemListControls.contains("Picker(\"학기\""))
        XCTAssertFalse(companionItemListControls.contains("Picker(\"과목\""))
        XCTAssertTrue(companionItemListControls.contains("private struct CompanionMenuPickerField<Option: Hashable>: View"))
        XCTAssertTrue(companionItemListControls.contains("CompanionNativeChoiceField("))
        XCTAssertTrue(companionItemListControls.contains("minHeight: Self.menuHeight"))
        ...
        XCTAssertFalse(ios.contains(".disabled(options.count <= 1)"))
        XCTAssertFalse(ios.contains("DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(45))"))
        XCTAssertFalse(ios.contains(".milliseconds(45)"))
        ...
        XCTAssertFalse(companionItemListControls.contains("HStack(spacing: KLMSSpacing.standard) {\n                        if yearOptions.count > 1"))
        ...
        XCTAssertTrue(compactSelectedRow.contains("RoundedRectangle(cornerRadius: KLMSRadius.panel)"))
        ...
        XCTAssertTrue(actionButtonStyle.contains("RoundedRectangle(cornerRadius: KLMSRadius.control)"))
        XCTAssertTrue(actionButtonStyle.contains(".font(.system(.caption, design: .rounded, weight: .semibold))"))
        XCTAssertTrue(actionButtonStyle.contains(".padding(.horizontal, KLMSSpacing.standard)"))
        XCTAssertTrue(actionButtonStyle.contains(".padding(.vertical, KLMSSpacing.comfortable)"))
        XCTAssertFalse(actionButtonStyle.contains("background(isPressed: configuration.isPressed)"))
        XCTAssertFalse(actionButtonStyle.contains("background(isPressed:"))
        XCTAssertFalse(actionButtonStyle.contains("border(isPressed:"))
        XCTAssertTrue(actionButtonStyle.contains(".stroke(border, lineWidth: isEnabled ? 1.15 : 1)"))
        ...
        XCTAssertTrue(actionButtonStyle.contains("return AnyShapeStyle(Color.klmsSuccessBackground.opacity(isEnabled ? 1.0 : 0.22))"))
        ...
        XCTAssertTrue(mailDashboardItemEditForm.contains("CompanionNativeChoiceField("))
        XCTAssertTrue(mailDashboardItemEditForm.contains("title: \"분류\""))
        XCTAssertFalse(mailDashboardItemEditForm.contains("Picker(\"분류\""))
        ...
        XCTAssertTrue(remoteSettingRow.contains("Text(settingValueSummary)"))
        XCTAssertTrue(remoteSettingRow.contains("Image(systemName: setting.boolValue ? \"checkmark.circle.fill\" : \"circle\")"))
        XCTAssertTrue(remoteSettingRow.contains(".frame(width: 18, height: 18)"))
        XCTAssertTrue(remoteSettingRow.contains("@State private var choiceValue = \"\""))
        XCTAssertTrue(remoteSettingRow.contains("CompanionNativeChoiceField("))
        XCTAssertTrue(remoteSettingRow.contains("selection: settingChoiceBinding"))
        XCTAssertTrue(remoteSettingRow.contains("settingChoiceOptions"))
        XCTAssertTrue(remoteSettingRow.contains("choiceValue = newValue"))
        XCTAssertFalse(remoteSettingRow.contains("@State private var isChoiceExpanded"))
        XCTAssertFalse(remoteSettingRow.contains("isChoiceExpanded"))
        XCTAssertFalse(remoteSettingRow.contains("settingChoiceChip"))
        XCTAssertFalse(remoteSettingRow.contains("private var settingChoiceColumns"))
        XCTAssertFalse(remoteSettingRow.contains("LazyVGrid(columns: settingChoiceColumns"))
        XCTAssertFalse(remoteSettingRow.contains("Menu {"))
        XCTAssertTrue(remoteSettingRow.contains("Button(\"저장\")"))
        XCTAssertTrue(remoteSettingRow.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(companionDiagnosticDisclosure.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(companionDiagnosticDisclosure.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: KLMSRadius.smallSurface))"))
        ...
        XCTAssertFalse(actionButtonStyle.contains(".padding(.horizontal, KLMSSpacing.comfortable)"))
        XCTAssertFalse(actionButtonStyle.contains(".padding(.vertical, KLMSSpacing.standard)"))
        XCTAssertTrue(relayConnectionPanel.contains("RoundedRectangle(cornerRadius: KLMSRadius.panel)"))
        XCTAssertTrue(relayConnectionPanel.contains("RoundedRectangle(cornerRadius: KLMSRadius.card)"))
        XCTAssertTrue(immediateSettingsPanel.contains("RoundedRectangle(cornerRadius: KLMSRadius.panel)"))
        ...
        XCTAssertTrue(immediateSettingsPanel.contains("@AppStorage(\"KLMSShouldUpdateNoticeNotes\")"))
        XCTAssertTrue(immediateSettingsPanel.contains("localNoticeNotesEnabled = enabled"))
        XCTAssertTrue(immediateSettingsPanel.contains("private var effectiveNoticeNotesEnabled: Bool"))
        XCTAssertTrue(immediateSettingsPanel.contains("Label(\n                                \"원격 실행에서 공지 메모도 갱신\""))
        XCTAssertTrue(immediateSettingsPanel.contains("systemImage: effectiveNoticeNotesEnabled ? \"checkmark.circle.fill\" : \"circle\""))
        XCTAssertTrue(immediateSettingsPanel.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(immediateSettingsPanel.contains(".buttonStyle(KLMSActionButtonStyle(tone: effectiveNoticeNotesEnabled ? .success : .soft))"))
        XCTAssertTrue(immediateSettingsPanel.contains(".disabled(noticeNotesUpdatePending)"))
        XCTAssertTrue(immediateSettingsPanel.contains(".accessibilityLabel(\"공지 메모 갱신\")"))
        XCTAssertTrue(immediateSettingsPanel.contains(".accessibilityValue(noticeNotesUpdatePending ? \"저장 중\""))
        ...
        XCTAssertTrue(dashboardSyncSurface.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: KLMSRadius.card, disabledOpacity: 0.78))"))
        XCTAssertFalse(dashboardSyncCard.contains("isRunning ? Color.klmsPrimaryCommandButtonBorder.opacity(0.58)"))
        XCTAssertTrue(dashboardSyncSurface.contains("primaryCommandTitle(isRunning: isRunning)"))
        XCTAssertTrue(dashboardSyncSurface.contains("primaryCommandSystemImage(isRunning: isRunning, isDisabled: isDisabled)"))
        XCTAssertTrue(dashboardSyncSurface.contains("Color.klmsPrimaryCommandButtonForeground"))
        XCTAssertTrue(dashboardSyncSurface.contains("return \"전체 동기화\""))
        XCTAssertFalse(dashboardSyncSurface.contains("return \"Mac 연결 필요\""))
        XCTAssertFalse(dashboardSyncSurface.contains("return \"잠시 대기\""))
        XCTAssertTrue(dashboardSyncCard.contains("서버 URL과 클라이언트 토큰을 저장하면 동기화 요청과 대시보드 갱신을 사용할 수 있습니다."))
        ...
        XCTAssertTrue(dashboardMetricCategory.contains("Color.klmsWarningForeground"))
        XCTAssertTrue(dashboardMetricCategory.contains("Color.klmsSuccessForeground"))
        ...
        XCTAssertTrue(relayConnectionPanel.contains("Color.klmsSuccessForeground"))
        XCTAssertTrue(calendarChangeDetailRow.contains("Color.klmsSuccessForeground"))
        XCTAssertTrue(calendarChangeDetailRow.contains("Color.klmsCommandAccent"))
        XCTAssertTrue(calendarChangeDetailRow.contains("Color.klmsDangerForeground"))
        ...
        XCTAssertTrue(flowChipLayout.contains("isSelected ? Color.klmsSelectedForeground : Color.klmsSecondaryText"))
        XCTAssertTrue(flowChipLayout.contains("let isSelected = selectedKind == entry.kind"))
        XCTAssertFalse(flowChipLayout.contains("? Color.klmsSelectedBackground.opacity(0.96)"))
        ...
        XCTAssertFalse(flowChipLayout.contains("Color.klmsSelectedBorder.opacity(0.92)"))
        XCTAssertTrue(flowChipLayout.contains(".companionStableTap(cornerRadius: KLMSRadius.smallSurface)"))
        XCTAssertTrue(remoteChangeSummaryDetail.contains("@Environment(\\.companionLayoutMode) private var layoutMode"))
        ...
        XCTAssertTrue(remoteChangeSummaryDetail.contains("CompanionLargeList.initialVisibleLimit(layoutMode: layoutMode)"))
        XCTAssertTrue(remoteChangeSummaryDetail.contains("CompanionLargeList.calendarVisibleLimit(layoutMode: layoutMode)"))
        XCTAssertTrue(remoteChangeSummaryDetail.contains("CompanionLargeList.previewVisibleLimit(layoutMode: layoutMode)"))
        XCTAssertTrue(remoteChangeSummaryDetail.contains("private func resetVisibleLimits()"))
        let resetVisibleLimits = try sourceBody(
            after: "private func resetVisibleLimits()",
            in: remoteChangeSummaryDetail,
            description: "remote change summary visible limit reset"
        )
        XCTAssertFalse(resetVisibleLimits.contains("selectedItemID = nil"))
        XCTAssertTrue(resetVisibleLimits.contains("clearStaleSelectedItemIfNeeded()"))
        ...
        XCTAssertFalse(serverSyncDataRow.contains("isSelected ? Color.klmsSelectedBackground.opacity(0.96)"))
        XCTAssertTrue(serverSyncDataRow.contains(".background(Color.klmsSubtleCardBackground)"))
        XCTAssertFalse(serverSyncDataRow.contains("isSelected ? Color.klmsSelectedBorder.opacity(0.92) : Color.klmsBorder"))
        XCTAssertTrue(serverSyncDataRow.contains(".stroke(Color.klmsBorder, lineWidth: 1)"))
        XCTAssertTrue(serverSyncDataRow.contains("isSelected ? Color.klmsSelectedForeground : tint.opacity(0.30)"))
        XCTAssertFalse(serverSyncDataRow.contains(".shadow(color: isSelected ? Color.klmsSelectedBorder.opacity(0.10) : Color.clear"))
        XCTAssertTrue(sharedRunLogRow.contains("Color.klmsWarningForeground"))
        XCTAssertTrue(sharedRunLogRow.contains("Color.klmsSuccessForeground"))
        XCTAssertTrue(remoteCommandRow.contains("Color.klmsCommandAccent"))
        XCTAssertTrue(remoteCommandRow.contains("Color.klmsWarningForeground"))
    ...
    func testSettingsPanelsAreGroupedAndUseNaturalKoreanCopy() throws {
        ...
        XCTAssertTrue(macSettings.contains("Text(\"대부분의 값은 설정 파일에 저장되고, 화면 모드와 원격 실행 옵션은 이 기기에 바로 적용됩니다.\")"))
        XCTAssertTrue(macSettings.contains("\"파일 확인\""))
        XCTAssertTrue(macSettings.contains("\"화면 모드와 원격 실행\""))
        XCTAssertTrue(macSettings.contains("ViewThatFits(in: .horizontal)"))
        XCTAssertTrue(macSettings.contains("VStack(alignment: .trailing, spacing: KLMSSpacing.hairline)"))
        XCTAssertTrue(settingsForm.contains("VStack(alignment: .leading, spacing: KLMSSpacing.section)"))
        ...
        XCTAssertTrue(macSettings.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .center)"))
        ...
        XCTAssertTrue(macSettings.contains("badge: \"즉시 적용\""))
        ...
        XCTAssertTrue(settingsGroupBox.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(settingsFieldRow.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(settingsDisclosureCard.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(settingsActionGroupBox.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertTrue(macSettings.contains("title: \"화면 모드와 원격 실행\""))
        XCTAssertTrue(macSettings.contains("이 Mac의 화면 모드와 원격 실행 옵션입니다."))
        ...
        XCTAssertTrue(macSettings.contains(".fixedSize(horizontal: false, vertical: true)"))
        XCTAssertFalse(macSettings.contains(".minimumScaleFactor(0.88)"))
        ...
        XCTAssertTrue(macSettings.contains("LazyVGrid(columns: settingsActionColumns, spacing: KLMSSpacing.standard)"))
        ...
        XCTAssertFalse(macSettings.contains("\"보조 모드\""))
        XCTAssertFalse(mac.contains("인증 방식:"))

        XCTAssertTrue(macModel.contains("앱이 앞에 없어도 로그인 보조"))
        XCTAssertTrue(macModel.contains("\"KLMS_LOGIN_ASSIST_MODE\": \"manual-digits\""))
        ...
        XCTAssertTrue(ios.contains("\"KLMS_LOGIN_ASSIST_ENABLED\", \"KLMS_LOGIN_ASSIST_ALLOW_NONINTERACTIVE\""))
        ...
        XCTAssertTrue(ios.contains("transaction.disablesAnimations = true"))
        XCTAssertFalse(remoteSettingGroupSection.contains("if isExpanded {\n                    groupSettingsRows"))
        XCTAssertTrue(remoteSettingGroupSection.contains("private var groupSettingsRows: some View"))
        XCTAssertTrue(remoteSettingGroupSection.contains("CompanionExpansionBadge(isExpanded: isExpanded, compact: true)"))
        XCTAssertTrue(remoteSettingGroupSection.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertTrue(companionConnectionInput.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(companionSettingsSubsectionCard.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        let iosHistoryBody = try sourceBody(
            after: "var body: some View",
            in: iosHistoryScreen,
            description: "iPhone compact log hierarchy"
        )
        ...
        let iosHistoryRequestColumn = try sourceBody(
            after: "private var historyRequestColumn",
            in: iosHistoryScreen,
            description: "iPhone/iPad scoped request log column"
        )
        let iosAdaptiveRoot = try sourceStructBody(named: "CompanionAdaptiveRootView", in: ios)
        ...
        let iosCompactTrashButtonStyle = try sourceBody(
            after: "private struct KLMSCompactTrashButtonStyle: ButtonStyle",
            in: ios,
            description: "iOS compact destructive icon style"
        )
        let iosConfirmedClearAction = try sourceStructBody(named: "KLMSConfirmedClearAction", in: ios)
        ...
        XCTAssertTrue(macModel.contains("private func clearRelayLogTailDisplay()"))
        XCTAssertTrue(macModel.contains("replaceSnapshot(nextSnapshot)"))
        XCTAssertTrue(macModel.contains("refreshDashboardPresentationCaches()"))
        XCTAssertFalse(macModel.contains("func clearLocalRelayLogs() {\n        snapshot.relayLogTail = \"\""))
        XCTAssertFalse(macModel.contains("private func clearLocalStoredLogs() {\n        commandHistory = (try? CommandRunHistoryStore(url: paths.appHistoryURL).clear()) ?? CommandRunHistory()\n        snapshot.relayLogTail = \"\""))
        XCTAssertFalse(mac.contains(".disabled(!model.hasClearableVisibleLogs)"))
        ...
        XCTAssertFalse(macRunLogArchivePanel.contains(".disabled(!model.hasClearableExecutionRunLogs)"))
        XCTAssertFalse(macRunLogArchivePanel.contains(".disabled(!model.hasClearableLocalRelayLogs)"))
        ...
        XCTAssertTrue(macRunLogArchiveRow.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
        ...
        XCTAssertTrue(macRemoteActivityPanel.contains("KLMSMacConfirmedClearAction("))
        ...
        XCTAssertTrue(macRootBody.contains("DashboardTopBarView("))
        XCTAssertTrue(macRootBody.contains("layoutMode: metrics.contentMode"))
        XCTAssertTrue(macRootBody.contains("MacAlertBannerView("))
        XCTAssertFalse(macRootBody.contains("CommandPanelView(model: model)"))
        XCTAssertTrue(macRootBody.contains("MacWorkspaceSidebarView("))
        XCTAssertTrue(macRootBody.contains("openRunLog: openRunLog"))
        XCTAssertTrue(mac.contains("private func openRunLog()"))
        XCTAssertTrue(macRootBody.contains("resetCurrentSectionScroll: resetCurrentSectionScroll"))
        XCTAssertTrue(macRootBody.contains(".frame(width: metrics.navigationColumnWidth, alignment: .topLeading)"))
        XCTAssertTrue(mac.contains(".fill(Color.klmsMacBorder.opacity(0.76))"))
        XCTAssertTrue(macRootBody.contains("ZStack(alignment: .topLeading)"))
        XCTAssertFalse(macRootBody.contains("AccessibilityMarker"))
        XCTAssertTrue(macRootBody.contains(".accessibilityIdentifier(\"workspace-layout-mode-\\(metrics.contentMode.rawValue)\")"))
        XCTAssertTrue(macRootBody.contains(".frame(width: metrics.workspaceWidth, alignment: .topLeading)"))
        ...
        XCTAssertFalse(macRootBody.contains(".frame(width: 1, height: 1)"))
        ...
        XCTAssertTrue(macModel.contains("@Published private(set) var dashboardSummaryCache = KLMSMacDashboardSummaryCache()"))
        XCTAssertTrue(macModel.contains("@Published private(set) var dashboardSummaryPresentation = DashboardSummaryPresentation"))
        ...
        XCTAssertTrue(macModel.contains("applyServerRelaySyncData(syncData, source: .startupCache)"))
        XCTAssertTrue(macModel.contains("applyServerRelaySyncData(syncData, source: .serverFetch)"))
        XCTAssertTrue(macModel.contains("serverRelayLastSyncDataFetchAt = nil"))
        XCTAssertTrue(macModel.contains("serverRelayForceSyncDataFetchOnNextWorkerRefresh = true"))
        ...
        XCTAssertTrue(macModel.contains("await publishServerRelayStatusIfNeeded(force: true, publishSyncData: true)"))
        XCTAssertTrue(macModel.contains("await refreshServerRelayDashboardNow(silent: true)"))
        XCTAssertTrue(macModel.contains("func refreshServerRelayDashboardNow("))
        XCTAssertTrue(macModel.contains("sessionGeneration: UInt64? = nil"))
        ...
        XCTAssertTrue(macConnectionBody.contains("let expectedSessionGeneration = serverRelaySessionGeneration"))
        XCTAssertTrue(macConnectionBody.contains("guard serverRelaySessionGeneration == expectedSessionGeneration else { return }"))
        XCTAssertTrue(macConnectionBody.contains("let recentFileRequests = try? await store.fetchRecentFileAccessRequests(limit: 8)\n            guard serverRelaySessionGeneration == expectedSessionGeneration else { return }"))
        XCTAssertTrue(macConnectionBody.contains("let requestLog = try? await store.fetchRecentRequestLog(limit: 20)\n            guard serverRelaySessionGeneration == expectedSessionGeneration else { return }"))
        XCTAssertTrue(macConnectionBody.contains("silent: false,\n                sessionGeneration: expectedSessionGeneration"))
        XCTAssertTrue(macConnectionBody.contains("} catch {\n            guard serverRelaySessionGeneration == expectedSessionGeneration else { return }"))
        let macStatusPublishQueueBody = try sourceBody(
            after: "private func publishServerRelayStatusIfNeeded(force: Bool = false, publishSyncData: Bool = false) async",
            in: macModel,
            description: "Mac server relay status publish queue"
        )
        let macStatusPublishBody = try sourceBody(
            after: "private func performServerRelayStatusPublish(",
            in: macModel,
            description: "Mac serialized server relay status publish"
        )
        let macLogClearBody = try sourceBody(
            after: "func clearServerRelayLogs(scope: ServerRelayLogClearScope = .all) async",
            in: macModel,
            description: "Mac server relay log clear"
        )
        let macSharedSettingUpdateBody = try sourceBody(
            after: "private func updateServerRelaySharedSetting(",
            in: macModel,
            description: "Mac server relay shared setting update"
        )
        XCTAssertTrue(macStatusPublishQueueBody.contains("let predecessor = serverRelayStatusPublishTail"))
        XCTAssertTrue(macStatusPublishQueueBody.contains("serverRelayStatusPublishTail = operation"))
        ...
        XCTAssertTrue(macStatusPublishBody.contains("serverRelaySessionGeneration == expectedSessionGeneration"))
        XCTAssertTrue(macLogClearBody.contains("let expectedSessionGeneration = serverRelaySessionGeneration"))
        XCTAssertTrue(macLogClearBody.contains("serverRelayLogClearOperationID == operationID"))
        XCTAssertTrue(macSharedSettingUpdateBody.contains("serverRelaySharedSettingMutationVersions[key] == mutationVersion"))
        XCTAssertTrue(macSharedSettingUpdateBody.contains("await serverRelaySharedSettingMutationQueue.enqueue(for: key)"))
        XCTAssertTrue(macSharedSettingUpdateBody.contains("serverRelayPendingSharedSettingsByKey[key] = setting"))
        let macBaselineCaptureIndex = try XCTUnwrap(macSharedSettingUpdateBody.range(of: "serverRelaySharedSettingCommittedBaselinesByKey[key] = ServerRelaySharedSettingCommittedBaseline(")?.lowerBound)
        let macOptimisticApplyIndex = try XCTUnwrap(macSharedSettingUpdateBody.range(of: "serverRelayPendingSharedSettingsByKey[key] = setting")?.lowerBound)
        XCTAssertLessThan(macBaselineCaptureIndex, macOptimisticApplyIndex)
        let macSavedIndex = try XCTUnwrap(macSharedSettingUpdateBody.range(of: "let saved = try await store.updateSharedSetting(setting)")?.lowerBound)
        let macAfterSaved = macSharedSettingUpdateBody[macSavedIndex...]
        let macCommittedUpdateIndex = try XCTUnwrap(macAfterSaved.range(of: "baseline.setting = saved")?.lowerBound)
        let macLatestVersionGuardIndex = try XCTUnwrap(macAfterSaved.range(of: "guard self.serverRelaySharedSettingMutationVersions[key] == mutationVersion else { return }")?.lowerBound)
        XCTAssertLessThan(macCommittedUpdateIndex, macLatestVersionGuardIndex)
        XCTAssertTrue(macAfterSaved.contains("RelayMutationBaselinePolicy.shouldAcceptResponse("))
        XCTAssertTrue(macAfterSaved.contains("var savedWasAccepted = false"))
        XCTAssertTrue(macAfterSaved.contains("if savedWasAccepted"))
        XCTAssertTrue(macAfterSaved.contains("self.restoreServerRelaySharedSetting(key: key, committedSetting: committedSetting)"))
        XCTAssertTrue(macSharedSettingUpdateBody.contains("let committedSetting = self.serverRelaySharedSettingCommittedBaselinesByKey[key]?.setting"))
        XCTAssertTrue(macSharedSettingUpdateBody.contains("self.restoreServerRelaySharedSetting(key: key, committedSetting: committedSetting)"))
        XCTAssertTrue(macModel.contains("serverRelaySharedSettingMutationQueue.cancelAll()"))
        XCTAssertTrue(macModel.contains("serverRelaySharedSettingCommittedBaselinesByKey.removeAll()"))
        ...
        XCTAssertFalse(macModel.contains("fetchSyncData(limit: 1)"))
        XCTAssertTrue(macModel.contains("@Published private(set) var dashboardRenderSignature = DashboardRenderSignature"))
        XCTAssertTrue(macModel.contains("@Published private(set) var dashboardFileRenderSignature = DashboardFileRenderSignature(snapshot: EngineSnapshot())"))
        ...
        XCTAssertTrue(macModel.contains("dashboardFileRenderSignature = DashboardFileRenderSignature(\n            snapshot: snapshot,\n            serverItems: dashboardServerRelayItems(for: .files)\n        )"))
        ...
        XCTAssertFalse(deferredMacWorkspacePanel.contains("contentIdentifier"))
        ...
        XCTAssertFalse(deferredMacWorkspacePanel.contains("workspaceContentAccessibilityMarker"))
        XCTAssertFalse(deferredMacWorkspacePanel.contains("Text(\"작업공간 내용\")"))
        ...
        XCTAssertTrue(macWorkstationLayoutView.contains("DeferredMacWorkspacePanel(id: \"workspace-dashboard\", deferContent: false)"))
        XCTAssertTrue(macWorkstationLayoutView.contains("MacDashboardCommandCenterView("))
        XCTAssertTrue(macWorkstationLayoutView.contains("layoutMode: layoutMode"))
        let macCommandCenter = try sourceStructBody(named: "MacDashboardCommandCenterView", in: mac)
        XCTAssertFalse(macCommandCenter.contains("ViewThatFits(in: .horizontal)"))
        XCTAssertTrue(macCommandCenter.contains("MacDashboardCommandCenterLayout(isWide: layoutMode == .wide)"))
        XCTAssertTrue(macCommandCenter.contains(".frame(width: layoutMode == .wide ? 320 : nil, alignment: .topLeading)"))
        XCTAssertFalse(macCommandCenter.contains("if layoutMode == .wide"))
        XCTAssertTrue(macCommandCenter.contains("CommandPanelView(model: model, openRunLog: openRunLog)"))
        XCTAssertTrue(macCommandCenter.contains("DashboardSummaryView(model: model)"))
        XCTAssertTrue(mac.contains("private struct DashboardCommandCenterStatusCard"))
        ...
        XCTAssertFalse(filesPanelBlock.contains("contentIdentifier"))
        ...
        XCTAssertTrue(macWorkstationLayoutView.contains("DeferredMacWorkspacePanel(id: \"workspace-notices\")"))
        XCTAssertTrue(macWorkstationLayoutView.contains("DeferredMacWorkspacePanel(id: \"workspace-calendar\")"))
        ...
        XCTAssertFalse(macWorkstationLayoutView.contains("contentIdentifier"))
        ...
        XCTAssertFalse(macWorkstationLayoutView.contains("workspace-rendered-content"))
        ...
        XCTAssertTrue(taskAndExamWorkspaceView.contains("LazyVGrid(columns: taskKindColumns, spacing: KLMSSpacing.standard)"))
        XCTAssertTrue(taskAndExamWorkspaceView.contains("private var taskKindColumns: [GridItem]"))
        XCTAssertTrue(taskAndExamWorkspaceView.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(logSummaryPanelView.contains("@ObservedObject var model: KLMSMacModel"))
        ...
        XCTAssertLessThan(alertRange.lowerBound, topBarRange.lowerBound)
        XCTAssertLessThan(topBarRange.lowerBound, scrollRange.lowerBound)
        ...
        XCTAssertTrue(macSidebarView.contains("WorkspaceNavigationView("))
        XCTAssertTrue(macSidebarView.contains("selection: $selectedSection"))
        XCTAssertTrue(macSidebarView.contains("iconOnly: iconOnly"))
        XCTAssertFalse(macSidebarView.contains("Spacer(minLength: 10)"))
        let sidebarNavigationRange = try XCTUnwrap(macSidebarView.range(of: "WorkspaceNavigationView("))
        ...
        XCTAssertFalse(macRuntimePanel.contains("@State private var isExpanded = false"))
        XCTAssertFalse(macRuntimePanel.contains("@AppStorage(\"KLMSMacRuntimePanelExpanded\")"))
        XCTAssertFalse(externalIntegrationStatusView.contains("@State private var isExpanded = false"))
        ...
        XCTAssertTrue(macRuntimePanel.contains("private var runtimeSummaryBadgeText: String?"))
        XCTAssertTrue(macRuntimePanel.contains("if let runtimeSummaryBadgeText"))
        XCTAssertTrue(macRuntimePanel.contains("return nil"))
        XCTAssertFalse(macRuntimePanel.contains("return \"검사 전\""))
        ...
        XCTAssertTrue(macNavigationView.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertFalse(macNavigationView.contains("WorkspaceNavigationSelectionMarker"))
        ...
        XCTAssertTrue(dashboardTopBarStatusContent.contains(".frame(maxWidth: .infinity, alignment: .leading)"))
        ...
        XCTAssertTrue(mac.contains("private struct MacRunningProgressSnapshot"))
        XCTAssertTrue(mac.contains("private struct MacRunningProgressBarView"))
        XCTAssertTrue(mac.contains("runningProgress: MacRunningProgressSnapshot(command: command, phaseText: phase)"))
        XCTAssertTrue(mac.contains("return [\"로그인\", \"파일\", \"과제/시험\", \"공지\", \"상태 검사\", \"정리\"]"))
        XCTAssertTrue(mac.contains("ProgressView(value: progress.fraction)"))
        XCTAssertTrue(mac.contains(".accessibilityLabel(\"동기화 진행률\")"))
        XCTAssertTrue(mac.contains(".accessibilityIdentifier(\"running-progress\")"))
        XCTAssertTrue(dashboardTopBarStatusContent.contains("MacRunningProgressBarView(progress: runningProgress)"))
        XCTAssertTrue(macAlertBannerView.contains("MacAlertBannerContent(snapshot: snapshot)"))
        XCTAssertTrue(macAlertBannerView.contains("hasRecentRunFailure: model.hasRecentRunFailure"))
        XCTAssertTrue(macAlertBannerView.contains("if model.hasRecentRunFailure"))
        XCTAssertTrue(macAlertBannerView.contains("expandedLogSummaryKind = .run"))
        XCTAssertTrue(macAlertBannerView.contains("selectedSection = .activityLogs"))
        XCTAssertTrue(macAlertBannerView.contains(".equatable()"))
        XCTAssertTrue(macAlertBannerContent.contains("nonisolated static func =="))
        XCTAssertTrue(macAlertBannerContent.contains("MacRunningProgressBarView(progress: runningProgress)"))
        ...
        XCTAssertTrue(mac.contains("private enum MacFirstRunReadiness"))
        XCTAssertTrue(mac.contains("@AppStorage(MacFirstRunReadiness.storageKey) private var firstRunReadinessCompleted = false"))
        XCTAssertTrue(macRootBody.contains("if model.snapshot.syncReport == nil, (section == .diagnostics || section == .settings)"))
        XCTAssertTrue(macRootBody.contains("firstRunReadinessCompleted = true"))
        XCTAssertTrue(macAlertBannerSnapshot.contains("var firstRunReadinessCompleted: Bool"))
        XCTAssertTrue(macAlertBannerSnapshot.contains("private var shouldShowFirstRunReadiness: Bool"))
        XCTAssertTrue(macAlertBannerSnapshot.contains("|| shouldShowFirstRunReadiness"))
        XCTAssertFalse(macAlertBannerSnapshot.contains("|| !hasSyncReport"))
        XCTAssertTrue(macAlertBannerView.contains("firstRunReadinessCompleted = true"))
        ...
        XCTAssertTrue(macAlertBannerSnapshot.contains("if hasRecentRunFailure"))
        XCTAssertTrue(macAlertBannerSnapshot.contains("return recentRunFailureTitle"))
        XCTAssertTrue(macAlertBannerSnapshot.contains("return \"로그\""))
        XCTAssertTrue(macAlertBannerSnapshot.contains("if needsAttention"))
        XCTAssertTrue(macAlertBannerSnapshot.contains("로그 탭에서 실패 흐름과 마지막 메시지를 확인할 수 있습니다."))
        XCTAssertFalse(macAlertBannerSnapshot.contains("진단 보기에서 실패 원인"))
        XCTAssertTrue(macAlertBannerSnapshot.contains("return currentPhaseText ?? \"LOG\""))
        XCTAssertTrue(nextActionPanelView.contains("if model.hasRecentRunFailure"))
        XCTAssertTrue(nextActionPanelView.contains("kind: .showRunningLog"))
        XCTAssertTrue(nextActionPanelView.contains("title: model.recentRunFailureTitle"))
        XCTAssertTrue(nextActionPanelView.contains("detail: \"실행 로그에서 마지막 오류를 확인하세요.\""))
        XCTAssertTrue(nextActionPanelView.contains("detail: \"로그 탭에서 실패 흐름과 마지막 오류를 먼저 확인하세요.\""))
        XCTAssertTrue(nextActionPanelView.contains("buttonTitle: \"로그 보기\""))
        XCTAssertFalse(nextActionPanelView.contains("openDiagnostics"))
        XCTAssertFalse(mac.contains("진단 탭에서 실패 원인"))
        XCTAssertFalse(mac.contains("마지막 오류는 진단 탭"))
        ...
        XCTAssertTrue(mac.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(macDetail.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertFalse(workstationBody.contains("HStack(alignment: .top, spacing: KLMSSpacing.roomy)"))
        ...
        XCTAssertFalse(workstationBody.contains("workspaceContentMarker"))
        ...
        XCTAssertFalse(workstationBody.contains("workspace-rendered-content"))
        ...
        XCTAssertTrue(workstationBody.contains("DeferredMacWorkspacePanel(id: \"workspace-dashboard\", deferContent: false"))
        XCTAssertTrue(workstationBody.contains("MacDashboardCommandCenterView("))
        XCTAssertTrue(workstationBody.contains("layoutMode: layoutMode"))
        ...
        XCTAssertTrue(taskWorkspaceBody.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertFalse(taskWorkspaceBody.contains("GridItem(.flexible(minimum: 280), spacing: KLMSSpacing.section, alignment: .top)"))
        XCTAssertFalse(taskWorkspaceBody.contains(".gridCellColumns(2)"))
        XCTAssertFalse(taskWorkspaceBody.contains("LazyVGrid(columns: columns, alignment: .leading, spacing: KLMSSpacing.section)"))
        ...
        XCTAssertFalse(taskWorkspaceBody.contains("HStack(alignment: .top, spacing: KLMSSpacing.section)"))
        ...
        XCTAssertTrue(dashboardBody.contains("MacDashboardCommandCenterView"))
        ...
        XCTAssertTrue(mac.contains("let fileCount = DashboardFileMetricCounter.visibleCourseFileCount("))
        XCTAssertTrue(mac.contains("fallback: summary.serverDashboardItemsLoaded ? summary.serverFileCount : nil"))
        XCTAssertTrue(mac.contains("files = DashboardFileMetricCounter.visibleCourseFileCount("))
        XCTAssertTrue(mac.contains("let assignmentCount = summary.serverDashboardItemsLoaded ? summary.serverAssignmentCount : counts.assignments + summary.mailAssignmentCount"))
        XCTAssertTrue(mac.contains("let noticeCount = summary.serverDashboardItemsLoaded ? summary.serverNoticeCount : counts.notices"))
        XCTAssertTrue(mac.contains("let examCount = summary.serverDashboardItemsLoaded ? summary.serverExamCount : counts.exams + summary.mailExamCount"))
        XCTAssertTrue(mac.contains("let helpDeskCount = summary.serverDashboardItemsLoaded ? summary.serverHelpDeskCount : counts.helpDesk"))
        XCTAssertTrue(mac.contains("await model.refreshServerRelayDashboardNow(silent: true)"))
        XCTAssertTrue(mac.contains("func visibleMetrics(archiveExpanded: Bool) -> [Metric]"))
        XCTAssertTrue(ios.contains(".sorted(by: CompanionRecentSortEntry.areInIncreasingOrder)"))
        XCTAssertFalse(ios.contains("let leftKLMSTimestamp = lhs.dashboardTimestampEpoch"))
        XCTAssertFalse(ios.contains("return leftKLMSTimestamp > rightKLMSTimestamp"))
        XCTAssertTrue(macDetail.contains("DashboardFileItem"))
        XCTAssertTrue(macDetail.contains("fileprivate var klmsSortEpoch: Int = Int.min"))
        XCTAssertTrue(macDetail.contains("klmsSortEpoch = klmsTimestampEpoch ?? ServerRelaySyncItem.dashboardTimestampEpoch(from: trimmedRecency) ?? Int.min"))
        XCTAssertTrue(macDetail.contains("let leftKLMSTimestamp = lhs.klmsSortEpoch"))
        ...
        XCTAssertLessThan(
            ...
            try XCTUnwrap(logsBody.range(of: "LogSummaryPanelView(")).lowerBound
        )
        XCTAssertLessThan(
            try XCTUnwrap(logsBody.range(of: "LogSummaryPanelView(")).lowerBound,
        ...
        XCTAssertTrue(logSummaryPanelView.contains("private let tileColumns = [GridItem(.adaptive(minimum: 176), spacing: KLMSSpacing.standard)]"))
        ...
        XCTAssertTrue(logSummaryPanelView.contains("LazyVGrid(columns: tileColumns, alignment: .leading, spacing: KLMSSpacing.standard)"))
        XCTAssertFalse(logSummaryPanelView.contains("HStack(alignment: .top, spacing: KLMSSpacing.standard)"))
        XCTAssertTrue(logSummaryPanelView.contains(".background(Color.klmsMacCardBackground, in: RoundedRectangle(cornerRadius: KLMSRadius.panel))"))
        ...
        XCTAssertTrue(macServerRequestLogActivityRow.contains(".padding(KLMSSpacing.standard)"))
        XCTAssertTrue(macServerRequestLogActivityRow.contains(".background(statusColor.opacity(0.08), in: RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
        XCTAssertTrue(macServerRequestLogActivityRow.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
        ...
        XCTAssertTrue(macRemoteCommandActivityRow.contains(".padding(KLMSSpacing.standard)"))
        XCTAssertTrue(macRemoteCommandActivityRow.contains(".background(statusColor.opacity(0.08), in: RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
        XCTAssertTrue(macRemoteCommandActivityRow.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
        ...
        XCTAssertTrue(verifyPanelView.contains("정상 항목은 필요할 때만 펼쳐서 확인합니다."))
        ...
        XCTAssertTrue(doctorPanelView.contains("정상 항목은 필요할 때만 펼쳐서 확인합니다."))
        ...
        XCTAssertTrue(collapsibleSectionBox.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(collapsibleSectionBox.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.control))"))
        ...
        XCTAssertTrue(appDiagnosticsPanel.contains("private let permissionActionColumns = [GridItem(.adaptive(minimum: 136), spacing: KLMSSpacing.standard)]"))
        XCTAssertTrue(appDiagnosticsPanel.contains("LazyVGrid(columns: permissionActionColumns, alignment: .leading, spacing: KLMSSpacing.standard)"))
        ...
        let diagnosticsBody = try sectionBody(in: workstationBody, from: "case .diagnostics:", to: ".padding(.vertical, KLMSSpacing.tight)")
        XCTAssertTrue(diagnosticsBody.contains("id: \"workspace-diagnostics\""))
        XCTAssertFalse(diagnosticsBody.contains("contentIdentifier"))
        XCTAssertTrue(diagnosticsBody.contains("contentDelayNanoseconds: MacWorkspacePanelTiming.heavyListContentDelayNanoseconds"))
        XCTAssertTrue(diagnosticsBody.contains("VStack(alignment: .leading, spacing: KLMSSpacing.spacious)"))
        ...
        XCTAssertTrue(iosStatusScreen.contains("RemoteDashboardSyncCard(model: model, compact: layoutMode != .wide)"))
        ...
        XCTAssertTrue(iosHistoryScreen.contains("@Environment(\\.companionLayoutMode)"))
        XCTAssertTrue(iosHistoryScreen.contains("if layoutMode == .wide"))
        ...
        XCTAssertTrue(iosHistoryScreen.contains("VStack(alignment: .leading, spacing: KLMSSpacing.section)"))
        ...
        let compactHistorySummaryIndex = try XCTUnwrap(iosHistoryBody.range(of: "historySummaryColumn")?.lowerBound)
        let compactHistoryStageIndex = try XCTUnwrap(iosHistoryBody.range(of: "historyStageColumn")?.lowerBound)
        let compactHistoryRequestIndex = try XCTUnwrap(iosHistoryBody.range(of: "historyRequestColumn")?.lowerBound)
        XCTAssertLessThan(compactHistorySummaryIndex, compactHistoryStageIndex)
        XCTAssertLessThan(compactHistoryStageIndex, compactHistoryRequestIndex)
        XCTAssertFalse(iosHistoryBody.contains("selectedHistoryDetailPanel"))
        ...
        XCTAssertTrue(iosHistoryScreen.contains("showsInlineDetail: layoutMode != .wide"))
        XCTAssertTrue(iosHistoryScreen.contains("selectedKind: layoutMode == .wide ? $selectedLogSummaryKind : nil"))
        XCTAssertFalse(iosHistorySummaryColumn.contains("SharedRunLogsView"))
        XCTAssertTrue(iosHistorySummaryColumn.contains("await model.clearRemoteLogs()"))
        XCTAssertTrue(iosHistoryStageColumn.contains("SharedRunLogsView"))
        XCTAssertTrue(iosHistoryStageColumn.contains("await model.clearSharedRunLogs()"))
        ...
        XCTAssertTrue(iosHistoryRequestColumn.contains("await model.clearRemoteLogs(scope: .requestLog)"))
        XCTAssertTrue(iosHistoryRequestColumn.contains("await model.clearRemoteLogs(scope: .fileAccess)"))
        XCTAssertTrue(iosHistoryRequestColumn.contains("await model.clearRemoteLogs(scope: .command)"))
        ...
        XCTAssertTrue(iosRemoteLogSummaryPanel.contains("private var secondaryExpandedKind"))
        XCTAssertTrue(iosRemoteLogSummaryPanel.contains("isExpanded: true"))
        XCTAssertTrue(iosRemoteLogSummaryPanel.contains("inlineDetail(.status)"))
        XCTAssertTrue(iosRemoteLogSummaryPanel.contains("Color.klmsCardBackground, in: RoundedRectangle(cornerRadius: KLMSRadius.panel)"))
        ...
        XCTAssertFalse(iosRemoteLogSummaryPanel.contains("행을 누르면 펼쳐집니다."))
        ...
        XCTAssertFalse(iosRemoteLogSummaryPanel.contains(".clipShape(RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
        XCTAssertTrue(iosRemoteLogSummaryPanel.contains("KLMSConfirmedClearAction("))
        XCTAssertTrue(iosRemoteLogSummaryPanel.contains("accessibilityIdentifier: \"history-clear-all\""))
        XCTAssertTrue(iosRemoteLogDetailPanel.contains("RoundedRectangle(cornerRadius: KLMSRadius.card)"))
        ...
        XCTAssertTrue(iosCompanionEmptyDetailPanel.contains("Color.klmsCardBackground, in: RoundedRectangle(cornerRadius: KLMSRadius.panel)"))
        ...
        XCTAssertTrue(iosRemoteLogSummaryRow.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .topLeading)"))
        XCTAssertTrue(iosRemoteLogSummaryRow.contains("KLMSCardButtonStyle(cornerRadius: KLMSRadius.card)"))
        ...
        XCTAssertFalse(iosRemoteLogSummaryRow.contains("KLMSCardButtonStyle(cornerRadius: KLMSRadius.smallSurface)"))
        ...
        XCTAssertTrue(iosSharedRunLogsView.contains("accessibilityIdentifier: \"history-clear-shared-run-logs\""))
        ...
        XCTAssertTrue(iosRemoteLogSummaryPanel.contains("isEnabled: !snapshot.clearDisabled"))
        XCTAssertTrue(iosHistoryScreen.contains("!model.hasClearableRemoteLogs"))
        XCTAssertTrue(iosCompactTrashButtonStyle.contains(".frame(width: KLMSControlSize.minimumInteractive, height: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(iosCompactTrashButtonStyle.contains("Color.klmsDangerForeground"))
        XCTAssertTrue(iosCompactTrashButtonStyle.contains("Color.klmsDangerBackground.opacity(configuration.isPressed ? 0.20 : 0.08)"))
        XCTAssertTrue(iosCompactTrashButtonStyle.contains("Color.klmsDangerBorder.opacity(configuration.isPressed ? 0.58 : 0.34)"))
        XCTAssertFalse(iosCompactTrashButtonStyle.contains("LinearGradient("))
        XCTAssertFalse(iosCompactTrashButtonStyle.contains("Color.klmsDangerCommandButtonForeground"))
        XCTAssertTrue(iosConfirmedClearAction.contains("Button(role: .destructive)"))
        XCTAssertTrue(iosConfirmedClearAction.contains("Image(systemName: \"trash\")"))
        XCTAssertTrue(iosConfirmedClearAction.contains(".buttonStyle(KLMSCompactTrashButtonStyle())"))
        XCTAssertTrue(iosConfirmedClearAction.contains(".alert(confirmationTitle, isPresented: $showsConfirmation)"))
        XCTAssertTrue(iosConfirmedClearAction.contains("Button(confirmationButtonTitle, role: .destructive, action: action)"))
        XCTAssertTrue(iosConfirmedClearAction.contains("Button(\"취소\", role: .cancel)"))
        XCTAssertTrue(iosConfirmedClearAction.contains(".accessibilityLabel(accessibilityLabel)"))
        XCTAssertTrue(iosConfirmedClearAction.contains(".accessibilityHint(\"삭제 전에 확인합니다.\")"))
        XCTAssertTrue(iosConfirmedClearAction.contains(".accessibilityIdentifier(accessibilityIdentifier)"))
        ...
        XCTAssertTrue(iosAdaptiveRoot.contains("WorkstationSidebar(selectedSection: $selectedSection)"))
        XCTAssertTrue(iosAdaptiveRoot.contains("minWidth: CompanionWorkstationMetrics.sidebarWidth"))
        XCTAssertTrue(iosAdaptiveRoot.contains("idealWidth: CompanionWorkstationMetrics.sidebarWidth"))
        XCTAssertTrue(iosAdaptiveRoot.contains("maxWidth: CompanionWorkstationMetrics.sidebarWidth"))
        XCTAssertTrue(iosAdaptiveRoot.contains(".fixedSize(horizontal: true, vertical: false)"))
        XCTAssertFalse(iosAdaptiveRoot.contains(".frame(width: 154)"))
        XCTAssertTrue(iosAdaptiveRoot.contains("HStack(spacing: 0)"))
        ...
        XCTAssertTrue(iosSidebar.contains("VStack(alignment: .leading, spacing: KLMSSpacing.standard)"))
        ...
        XCTAssertTrue(iosSidebar.contains(".padding(.horizontal, KLMSSpacing.roomy)"))
        XCTAssertTrue(iosSidebar.contains(".padding(.top, KLMSSpacing.workstationColumn)"))
        ...
        XCTAssertTrue(iosSidebarButton.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(iosSidebarButton.contains("ZStack"))
        XCTAssertFalse(iosSidebarButton.contains("Color.klmsSelectedBorder.opacity(0.24)"))
        ...
        XCTAssertTrue(iosSidebarButton.contains(".frame(width: 4)"))
        XCTAssertTrue(iosSidebarButton.contains("RoundedRectangle(cornerRadius: KLMSRadius.card)"))
        XCTAssertTrue(iosSidebarButton.contains(".fill(isSelected ? Color.klmsSelectedForeground : Color.clear)"))
        XCTAssertTrue(iosSidebarButton.contains(".stroke(Color.klmsBorder.opacity(0.40), lineWidth: 1)"))
        XCTAssertFalse(ios.contains("weight: isSelected ? .bold : .semibold"))
        XCTAssertFalse(ios.contains("weight(isSelected ? .bold : .semibold)"))
        XCTAssertFalse(ios.contains("lineWidth: isSelected ? 1.2 : 1"))
        XCTAssertFalse(ios.contains("lineWidth: isSelected ? 1.35 : 1"))
        XCTAssertFalse(ios.contains("lineWidth: isSelected ? 1.6 : 1"))
        XCTAssertFalse(ios.contains(".frame(width: isSelected ? 4 : 0)"))
        ...
        XCTAssertTrue(iosSidebarButton.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.card))"))
        ...
        XCTAssertTrue(iosCardButtonStyle.contains(".frame(minWidth: KLMSControlSize.minimumInteractive, minHeight: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(iosCardButtonStyle.contains(".contentShape(RoundedRectangle(cornerRadius: cornerRadius))"))
        XCTAssertTrue(actionButtonStyle.contains(".frame(minWidth: KLMSControlSize.minimumInteractive, minHeight: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(toolbarButtonStyle.contains(".frame(minWidth: KLMSControlSize.minimumInteractive, minHeight: KLMSControlSize.minimumInteractive)"))
        XCTAssertFalse(ios.contains(".frame(minHeight: 36)"))
        XCTAssertFalse(ios.contains(".frame(height: 36)"))
        XCTAssertFalse(ios.contains(".frame(maxWidth: .infinity, minHeight: 36)"))
        XCTAssertFalse(iosCardButtonStyle.contains("Color.klmsCommandButtonPressedOverlay.opacity(configuration.isPressed ? 1.0 : 0.0)"))
        XCTAssertTrue(iosCardButtonStyle.contains("Color.klmsCommandButtonPressedOverlay.opacity(0.0)"))
        ...
        XCTAssertTrue(iosAdaptiveRoot.contains("currentSection"))
        XCTAssertFalse(iosAdaptiveRoot.contains("deferDisplayedSection(newSection ?? .status)"))
        ...
        XCTAssertFalse(iosAdaptiveRoot.contains("await Task.yield()"))
        XCTAssertFalse(iosAdaptiveRoot.contains("CompanionInlineDetailPreparingView"))
        ...
        XCTAssertTrue(iosScreenContainer.contains(".safeAreaInset(edge: .top, spacing: 0)"))
        XCTAssertTrue(iosScreenContainer.contains("private var attentionInset: some View"))
        XCTAssertFalse(iosScreenContainer.contains(".overlay(alignment: .top)"))
        XCTAssertFalse(iosScreenContainer.contains(".allowsHitTesting(attentionSnapshot.hasAttention)"))
        ...
        XCTAssertTrue(iosHeader.contains("@Environment(\\.companionLayoutMode)"))
        XCTAssertTrue(iosHeader.contains("compactHeader"))
        XCTAssertTrue(iosHeader.contains("regularHeader"))
        XCTAssertFalse(iosHeader.contains("Text(\"KLMS Sync\")"))
        ...
        XCTAssertTrue(iosStatusScreen.contains("VStack(alignment: .leading, spacing: KLMSSpacing.section)"))
        ...
        XCTAssertTrue(iosMetricOverview.contains("@Environment(\\.companionLayoutMode)"))
        XCTAssertTrue(iosMetricOverview.contains("@Environment(\\.dynamicTypeSize) private var dynamicTypeSize"))
        XCTAssertTrue(iosMetricOverview.contains("count: dynamicTypeSize.isAccessibilitySize ? 1 : 2"))
        XCTAssertTrue(iosMetricOverview.contains("LazyVGrid(columns: workstationColumns, alignment: .leading, spacing: KLMSSpacing.standard)"))
        XCTAssertTrue(iosMetricOverview.contains("LazyVGrid(columns: compactColumns, alignment: .leading, spacing: KLMSSpacing.standard)"))
        XCTAssertFalse(iosMetricOverview.contains("if layoutMode == .wide {\n                    VStack(spacing: KLMSSpacing.standard)"))
        XCTAssertFalse(iosMetricOverview.contains("layoutMode == .wide && !primaryMetricCategories.isEmpty"))
        ...
        XCTAssertTrue(iosMetricOverview.contains("layoutMode != .wide"))
        ...
        XCTAssertFalse(iosMetricOverview.contains("layoutMode == .wide ? [.quarantine, .calendar] : [.quarantine]"))
        ...
        XCTAssertTrue(iosMetricTile.contains(".font(.system(.title2, design: .rounded, weight: .bold).monospacedDigit())"))
        XCTAssertFalse(iosMetricTile.contains("Color.klmsSelectedBackground.opacity(0.96)"))
        XCTAssertTrue(iosMetricTile.contains(".background(Color.klmsCardBackground)"))
        XCTAssertTrue(iosMetricTile.contains("Color.klmsSelectedForeground"))
        XCTAssertTrue(iosMetricTile.contains(".foregroundStyle(Color.klmsSecondaryText)"))
        XCTAssertTrue(iosMetricTile.contains(".fill(isSelected ? Color.klmsSelectedForeground : Color.klmsCommandAccent.opacity(0.28))"))
        XCTAssertFalse(iosMetricTile.contains("Color.klmsSelectedBorder.opacity(0.92)"))
        ...
        XCTAssertTrue(iosWorkstationMetricCard.contains(".font(.system(.subheadline, design: .rounded, weight: .bold).monospacedDigit())"))
        ...
        XCTAssertTrue(iosWorkstationOverviewPanel.contains(".background(Color.klmsCardBackground, in: RoundedRectangle(cornerRadius: KLMSRadius.panel))"))
        ...
        XCTAssertTrue(iosWorkstationOverviewPanel.contains(".background(metric.tint.opacity(0.12), in: RoundedRectangle(cornerRadius: KLMSRadius.smallSurface))"))
        ...
        XCTAssertTrue(iosWorkstationOverviewPanel.contains("MetricSummary(category: .exams, title: \"시험\", value: status.exams, systemImage: \"calendar.badge.clock\", tint: Color.klmsWarningForeground)"))
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        let iosRoot = packageRoot.appendingPathComponent("Sources/KLMSiOS/KLMSiOSApp.swift")
        let ios = try String(contentsOf: iosRoot, encoding: .utf8)
        let adaptiveTwoColumnLayout = try sourceBody(
            after: "private struct CompanionAdaptiveTwoColumnLayout: Layout",
            in: ios,
            description: "iPad adaptive list/detail layout"
        )
        let workstationMetrics = try sourceBody(
            after: "private enum CompanionWorkstationMetrics",
            in: ios,
            description: "iPad workstation width thresholds"
        )
        let adaptiveRoot = try sourceStructBody(named: "CompanionAdaptiveRootView", in: ios)
        let sectionContent = try sourceStructBody(named: "CompanionSectionContent", in: ios)
        let statusScreen = try sourceStructBody(named: "CompanionStatusScreen", in: ios)
        let categoryScreen = try sourceStructBody(named: "CompanionDashboardCategoryScreen", in: ios)
        let tasksScreen = try sourceStructBody(named: "CompanionTasksScreen", in: ios)
        ...
        XCTAssertTrue(inlineRows.contains("return isSelected ? \"checkmark.circle.fill\" : \"info.circle\""))
        XCTAssertTrue(inlineRows.contains(".accessibilityValue(isSelected ? \"선택됨\" : \"선택 안 됨\")"))
        ...
        XCTAssertTrue(longDetailFieldRow.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        XCTAssertTrue(longDetailFieldRow.contains(".contentShape(RoundedRectangle(cornerRadius: KLMSRadius.control))"))
        XCTAssertTrue(longDetailFieldRow.contains(".buttonStyle(KLMSCardButtonStyle(cornerRadius: KLMSRadius.control))"))
        ...
        XCTAssertTrue(ios.contains("private struct KLMSCompactTrashButtonStyle: ButtonStyle"))
        XCTAssertTrue(ios.contains(".frame(width: KLMSControlSize.minimumInteractive, height: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(recentFileRequests.contains("KLMSConfirmedClearAction("))
        XCTAssertTrue(recentFileRequests.contains("accessibilityIdentifier: \"history-clear-file-access\""))
        XCTAssertTrue(recentServerRequests.contains("KLMSConfirmedClearAction("))
        XCTAssertTrue(recentServerRequests.contains("accessibilityIdentifier: \"history-clear-request-log\""))
        XCTAssertTrue(recentRemoteCommands.contains("KLMSConfirmedClearAction("))
        XCTAssertTrue(recentRemoteCommands.contains("accessibilityIdentifier: \"history-clear-commands\""))
        XCTAssertTrue(ios.contains("private struct KLMSCardButtonStyle: ButtonStyle"))
        XCTAssertTrue(ios.contains(".frame(minWidth: KLMSControlSize.minimumInteractive, minHeight: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(ios.contains(".frame(minWidth: KLMSControlSize.minimumInteractive, minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertFalse(tasksScreen.contains(".id(selectedCompactTaskCategory.rawValue)"))
        ...
        XCTAssertTrue(searchFilterPanel.contains(".frame(minHeight: KLMSControlSize.minimumInteractive)"))
        ...
        XCTAssertTrue(ios.contains("static func initialVisibleLimit(layoutMode: AdaptiveLayoutMode) -> Int"))
        XCTAssertTrue(ios.contains("layoutMode == .wide ? regularInitialVisibleLimit : initialVisibleLimit"))
        ...
        XCTAssertTrue(ios.contains("static func previewVisibleLimit(layoutMode: AdaptiveLayoutMode) -> Int"))
        XCTAssertTrue(ios.contains("static func calendarVisibleLimit(layoutMode: AdaptiveLayoutMode) -> Int"))
        XCTAssertFalse(ios.contains(".onChange(of: layoutMode)"))
        XCTAssertFalse(ios.contains("filterRebuildDelayNanoseconds"))
        ...
        XCTAssertTrue(ios.contains("guard let previous, query != previous.query else { return false }"))
        XCTAssertTrue(ios.contains("itemsRevision == previous.itemsRevision"))
        XCTAssertTrue(ios.contains("category == previous.category"))
        XCTAssertTrue(ios.contains("transaction.animation = nil"))
        XCTAssertTrue(ios.contains("withTransaction(transaction)"))
        XCTAssertTrue(adaptiveRoot.contains("@ObservedObject var model: CompanionModel"))
        XCTAssertTrue(sectionContent.contains("@ObservedObject var model: CompanionModel"))
        XCTAssertTrue(statusScreen.contains("@ObservedObject var model: CompanionModel"))
        XCTAssertTrue(categoryScreen.contains("@ObservedObject var model: CompanionModel"))
        XCTAssertTrue(tasksScreen.contains("@ObservedObject var model: CompanionModel"))
        XCTAssertTrue(settingsScreen.contains("@ObservedObject var model: CompanionModel"))
        XCTAssertTrue(iosHistoryScreen.contains("@ObservedObject var model: CompanionModel"))
        XCTAssertFalse(adaptiveRoot.contains("let model: CompanionModel"))
        XCTAssertFalse(sectionContent.contains("let model: CompanionModel"))
        XCTAssertFalse(statusScreen.contains("let model: CompanionModel"))
        XCTAssertFalse(categoryScreen.contains("let model: CompanionModel"))
        XCTAssertFalse(tasksScreen.contains("let model: CompanionModel"))
        XCTAssertFalse(settingsScreen.contains("let model: CompanionModel"))
        XCTAssertFalse(iosHistoryScreen.contains("let model: CompanionModel"))
        XCTAssertTrue(selectableRows.contains("@State private var selectedItemID"))
        XCTAssertTrue(selectableRows.contains("@Environment(\\.companionLayoutMode) private var layoutMode"))
        ...
        XCTAssertTrue(selectableRows.contains("LazyVStack(alignment: .leading, spacing: KLMSSpacing.standard)"))
        ...
        XCTAssertTrue(ios.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertTrue(selectableRows.contains(".companionStableTap(action: { select(item) })"))
        ...
        XCTAssertTrue(syncDataPanel.contains("try? await Task.sleep(nanoseconds: 180_000_000)"))
        XCTAssertFalse(syncDataPanel.contains("try? await Task.sleep(nanoseconds: CompanionLargeList.filterRebuildDelayNanoseconds)"))
        ...
        XCTAssertTrue(inlineDetail.contains("try? await Task.sleep(nanoseconds: 180_000_000)"))
        XCTAssertFalse(inlineDetail.contains("try? await Task.sleep(nanoseconds: CompanionLargeList.filterRebuildDelayNanoseconds)"))
        ...
        XCTAssertTrue(inlineRows.contains(".companionStableTap(action: { select(item) })"))
        ...
        XCTAssertTrue(inlineRows.contains("@Environment(\\.companionLayoutMode) private var layoutMode"))
        XCTAssertTrue(inlineRows.contains("CompanionLargeList.initialVisibleLimit(layoutMode: layoutMode)"))
        XCTAssertTrue(inlineRows.contains("LazyVStack(alignment: .leading, spacing: KLMSSpacing.standard)"))
        ...
        XCTAssertTrue(inlineDetail.contains("@Environment(\\.companionLayoutMode) private var layoutMode"))
        XCTAssertTrue(inlineDetail.contains("CompanionLargeList.calendarVisibleLimit(layoutMode: layoutMode)"))
        ...
        XCTAssertFalse(inlineRows.contains("presentation == .inlineDetail && selectedItemID == item.id"))
        XCTAssertTrue(inlineRows.contains("CompanionFloatingItemDetailOverlay(item: selectedItem, model: model)"))
        ...
        XCTAssertFalse(workstationCategoryRegularWorkspace.contains("VStack(alignment: .leading, spacing: KLMSSpacing.section)"))
        XCTAssertTrue(workstationMetrics.contains("twoColumnMinimumContentWidth = listColumnMinWidth + columnSpacing + detailColumnMinWidth"))
        XCTAssertTrue(workstationMetrics.contains("static func supportsTwoColumns(for layoutWidth: CGFloat) -> Bool"))
        XCTAssertTrue(workstationMetrics.contains("- (horizontalPadding * 2)"))
        XCTAssertTrue(adaptiveTwoColumnLayout.contains("proposal: ProposedViewSize"))
        XCTAssertTrue(adaptiveTwoColumnLayout.contains("let availableWidth = max(proposal.width ?? minimumHorizontalWidth, 0)"))
        XCTAssertTrue(adaptiveTwoColumnLayout.contains("let measurement = measure(availableWidth: bounds.width, subviews: subviews)"))
        XCTAssertTrue(adaptiveTwoColumnLayout.contains("let isHorizontal = allowsHorizontalColumns && availableWidth >= minimumHorizontalWidth"))
        XCTAssertTrue(adaptiveTwoColumnLayout.contains("primaryWidth = availableWidth"))
        XCTAssertTrue(adaptiveTwoColumnLayout.contains("detailWidth = availableWidth"))
        ...
        XCTAssertTrue(workstationCategoryRegularWorkspace.contains("CompanionAdaptiveTwoColumnLayout"))
        XCTAssertLessThan(
            try XCTUnwrap(workstationCategoryRegularWorkspace.range(of: "categoryListPanel")?.lowerBound),
            try XCTUnwrap(workstationCategoryRegularWorkspace.range(of: "categoryDetailPanel")?.lowerBound)
        )
        ...
        XCTAssertFalse(workstationTasksRegularWorkspace.contains("VStack(alignment: .leading, spacing: KLMSSpacing.section)"))
        ...
        XCTAssertTrue(workstationTasksRegularWorkspace.contains("CompanionAdaptiveTwoColumnLayout"))
        XCTAssertLessThan(
            try XCTUnwrap(workstationTasksRegularWorkspace.range(of: "tasksListPanel")?.lowerBound),
            try XCTUnwrap(workstationTasksRegularWorkspace.range(of: "tasksDetailPanel")?.lowerBound)
        )
        ...
        XCTAssertTrue(categoryScreen.contains("if layoutMode == .wide && category == .calendar"))
        ...
        XCTAssertFalse(workstationCalendarRegularWorkspace.contains("VStack(alignment: .leading, spacing: KLMSSpacing.section)"))
        XCTAssertTrue(workstationCalendar.contains("model.visibleCalendarChanges()"))
        XCTAssertTrue(workstationCalendarRegularWorkspace.contains("CompanionAdaptiveTwoColumnLayout"))
        ...
        XCTAssertTrue(workstationCalendarRegularWorkspace.contains("calendarListPanel"))
        XCTAssertTrue(workstationCalendarRegularWorkspace.contains("calendarDetailPanel"))
        XCTAssertFalse(workstationCalendarRegularWorkspace.contains("HStack(alignment: .top"))
        XCTAssertFalse(workstationCalendarRegularWorkspace.contains("minWidth: CompanionWorkstationMetrics.listColumnMinWidth"))
        XCTAssertFalse(workstationCalendarRegularWorkspace.contains("minWidth: CompanionWorkstationMetrics.detailColumnMinWidth"))
        ...
        XCTAssertFalse(workstationCalendar.contains("activeAction: model.activeCalendarAction(for: selectedChange)"))
        XCTAssertTrue(workstationCalendar.contains("await model.createCalendarEventOnDevice(change: selectedChange, edit: edit)"))
        XCTAssertTrue(workstationCalendar.contains("await model.editCalendarEventOnDevice(change: selectedChange, edit: edit)"))
        XCTAssertTrue(workstationCalendar.contains("await model.deleteCalendarEventOnDevice(change: selectedChange)"))
        ...
        XCTAssertTrue(workstationPreviewSection.contains(".companionStableTap {\n                        onOpenCategory(category)"))
        XCTAssertFalse(workstationPreviewSection.contains(".buttonStyle(KLMSStableSelectionButtonStyle())"))
        ...
        XCTAssertTrue(recentFileRequests.contains("LazyVStack(spacing: KLMSSpacing.standard)"))
        ...
        XCTAssertTrue(recentRemoteCommands.contains("LazyVStack(spacing: KLMSSpacing.standard)"))
    ...
    func testIOSCalendarDetailHasMailPasteAnalyzerAndSharedCalendarActionLabels() throws {
        ...
        XCTAssertTrue(mailPastePanel.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
        ...
        XCTAssertFalse(statusScreen.contains("@State private var dashboardPreviewTask"))
        XCTAssertFalse(statusScreen.contains("if layoutMode != .wide, selectedChangeSummary != nil || displayedChangeSummary != nil"))
        XCTAssertFalse(statusScreen.contains("if layoutMode != .wide {\n                statusDetailColumn"))
        ...
        XCTAssertFalse(ios.contains("private struct RemoteCommandPanel"))
        XCTAssertFalse(dashboardSyncCard.contains(".fullSync"))
        XCTAssertTrue(dashboardPrimarySyncAction.contains("model.runOrCancelRemoteCommand(.fullSync)"))
        XCTAssertTrue(dashboardPrimarySyncAction.contains(".accessibilityIdentifier(\"dashboard-primary-full-sync\")"))
        XCTAssertFalse(companionScreenHeader.contains("RemoteDashboardPrimarySyncAction"))
        XCTAssertTrue(dashboardSyncCard.contains("RemoteDashboardPrimarySyncAction(model: model, compact: compact)"))
        ...
        XCTAssertFalse(mailPastePanel.contains("HStack(spacing: KLMSSpacing.standard) {\n                        Button {\n                            pasteFromClipboard()"))
        XCTAssertTrue(mailPastePanel.contains("Label(\"입력 비우기\", systemImage: \"xmark.circle\")"))
        XCTAssertGreaterThanOrEqual(mailPastePanel.components(separatedBy: ".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive)").count - 1, 3)
        XCTAssertTrue(mailPastePanel.contains(".accessibilityHint(\"붙여넣은 본문에서 과제, 시험, 일정, 공지 후보를 찾습니다.\")"))
        ...
        XCTAssertTrue(mailPasteResultContent.contains(".companionStableTap(cornerRadius: KLMSRadius.smallSurface)"))
        XCTAssertFalse(mailPasteResultContent.contains("DeferredSelectionDetail(isExpanded: selectedItemID == item.id)"))
        XCTAssertTrue(mailPasteResultContent.contains("CompanionFloatingMailDetailOverlay(item: selectedMatchedItem, detailPanel: detailPanel)"))
        XCTAssertGreaterThanOrEqual(
            mailPasteResultContent.components(
                separatedBy: "minHeight: KLMSControlSize.minimumInteractive"
            ).count - 1,
            5
        )
        ...
        XCTAssertTrue(ios.contains("createCalendarEventOnDevice(change: CalendarChange, edit: CalendarEventEdit)"))
        XCTAssertTrue(ios.contains("editCalendarEventOnDevice(change: CalendarChange, edit: CalendarEventEdit)"))
        XCTAssertTrue(ios.contains("deleteCalendarEventOnDevice(change: CalendarChange)"))
        XCTAssertTrue(ios.contains("private func publishCompletedCalendarAction("))
        XCTAssertTrue(ios.contains("store: ServerRelayCommandStore?,\n        generation: UInt64"))
        XCTAssertTrue(ios.contains("status: .completed"))
        XCTAssertTrue(ios.contains("completedAction: .calendarCreate"))
        XCTAssertTrue(ios.contains("let originalStore = serverRelayStore"))
        XCTAssertTrue(ios.contains("store: originalStore,\n                    generation: generation"))
        XCTAssertTrue(ios.contains("guard isCurrentRelaySession(generation: generation) else { return true }"))
        XCTAssertTrue(ios.contains("let savedAction = try await store.createItemAction(action)"))
        XCTAssertTrue(ios.contains("ensureLocalCalendarAccess()"))
        ...
        XCTAssertTrue(ios.contains("let mutation = calendarResolutionMutation(for: change, key: mutationKey)"))
        ...
        XCTAssertTrue(ios.contains("rollbackCalendarResolutionMutation("))
        XCTAssertFalse(ios.contains("resolvedCalendarChangeIDs = previousResolvedCalendarChangeIDs"))
        XCTAssertTrue(ios.contains("replaceRecentItemAction(action) { candidateIDs.contains($0.itemID) }"))
        XCTAssertTrue(ios.contains("recordResolvedCalendarChanges(overlaidItemActions)"))
        XCTAssertTrue(ios.contains("action.action.resolvesCalendarChange"))
        XCTAssertFalse(ios.contains("activeAction: model.activeCalendarAction(for: change)"))
    ...
    func testMacCalendarDashboardHasMailPasteCalendarRegistration() throws {
        ...
        XCTAssertTrue(dashboardSummary.contains("VStack(alignment: .leading, spacing: KLMSSpacing.section)"))
        ...
        let macPrimarySyncIndex = try XCTUnwrap(commandPanel.range(of: "MacPrimarySyncActionView(model: model)")?.lowerBound)
        let macMailPanelIndex = try XCTUnwrap(commandPanel.range(of: "MacMailPasteAnalyzerPanel")?.lowerBound)
        let macSecondarySyncIndex = try XCTUnwrap(commandPanel.range(of: "LazyVGrid(columns: secondaryCommandColumns")?.lowerBound)
        XCTAssertTrue(macPrimarySyncIndex < macSecondarySyncIndex)
        XCTAssertTrue(macMailPanelIndex > macSecondarySyncIndex)
        XCTAssertFalse(commandPanel.contains(".fullSync"))
        XCTAssertTrue(primarySyncAction.contains("MacCommandActionIntent.capture("))
        XCTAssertTrue(primarySyncAction.contains("await model.run(command)"))
        XCTAssertTrue(primarySyncAction.contains("cancelRunningCommand(expectedIdentity: expectedIdentity)"))
        XCTAssertTrue(primarySyncAction.contains(".accessibilityIdentifier(\"command-fullSync-primary\")"))
        ...
        XCTAssertTrue(model.contains("let previousResolvedCalendarChangeIDs = resolvedCalendarChangeIDs"))
        XCTAssertTrue(model.contains("resolvedCalendarChangeIDs = previousResolvedCalendarChangeIDs"))
    ...
    func testAuthStatusDoesNotPromoteAlreadyLoggedInToAuthCompleted() throws {
        ...
        XCTAssertTrue(mac.contains("if let liveAuthDigits = currentAuthDigits"))
        XCTAssertFalse(mac.contains("if phase == \"running\", let liveAuthDigits"))
        XCTAssertTrue(mac.contains("lastAuthDigitsRecordedAt = Date()"))
    ...
    func testMacMailDashboardDerivedDataIsCachedOffRenderPath() throws {
        ...
        XCTAssertTrue(model.contains("private struct SnapshotSourceSignature: Sendable, Equatable"))
        ...
        XCTAssertTrue(
            model.contains("paths.overridesURL"),
            "An external manual override edit must invalidate the Mac snapshot and refresh the open dashboard."
        )
        XCTAssertTrue(model.contains("await self.loadEngineSnapshotOffMain(force: forceSnapshotReload)"))
        XCTAssertTrue(model.contains("self.applySnapshot(loadedSnapshot, showLoginTransition: true)"))
        XCTAssertFalse(model.contains("guard self.runningCommand == nil else { return }"))
        ...
        XCTAssertTrue(model.contains("@Published private(set) var dashboardFilterOptionsByKind: [DashboardDetailKind: DashboardFilterOptions] = [:]"))
    ...
    func testIOSAdaptiveNavigationUsesWideRailCompactStagesAndSafeKoreanCopy() throws {
        let packageRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        ...
        let ios = try String(contentsOf: iosRoot, encoding: .utf8)
        let adaptiveRoot = try sourceStructBody(named: "CompanionAdaptiveRootView", in: ios)
        let metrics = try sourceBody(
            after: "private enum CompanionWorkstationMetrics",
            in: ios,
            description: "iOS adaptive navigation metrics"
        )
        let rail = try sourceStructBody(named: "WorkstationNavigationRail", in: ios)

        XCTAssertTrue(metrics.contains("static let navigationRailWidth: CGFloat = 65"))
        XCTAssertFalse(metrics.contains("minimumSplitWidth"))
        XCTAssertTrue(adaptiveRoot.contains("case .compact"))
        XCTAssertTrue(adaptiveRoot.contains("case .medium"))
        XCTAssertTrue(adaptiveRoot.contains("case .wide"))
        XCTAssertTrue(adaptiveRoot.contains("CompanionWorkstationMetrics.navigationRailWidth + 1"))
        XCTAssertTrue(adaptiveRoot.contains("WorkstationNavigationRail(selectedSection: $selectedSection)"))
        XCTAssertTrue(adaptiveRoot.contains("if layoutMode == .compact"))
        XCTAssertTrue(rail.contains("CompanionAppSection.workstationSections"))
        XCTAssertTrue(rail.contains(".frame(width: KLMSControlSize.minimumInteractive, height: KLMSControlSize.minimumInteractive)"))
        XCTAssertTrue(rail.contains(".accessibilityLabel(section.title)"))
        XCTAssertTrue(rail.contains(".accessibilityValue(selectedSection == section ? \"선택됨\" : \"선택 안 됨\")"))
        XCTAssertTrue(rail.contains(".accessibilityIdentifier(\"companion-rail-\\(section.rawValue)\")"))
        XCTAssertFalse(rail.contains(".accessibilityIdentifier(\"companion-navigation-rail\")"))
        XCTAssertFalse(ios.contains("private struct CompanionTabRootView"))
        XCTAssertFalse(ios.contains("private struct CompanionSplitRootView"))

        XCTAssertTrue(ios.contains("Text(\"화면 모드는 바로 적용됩니다.\\n공지 메모는 서버에 저장됩니다.\")"))
        XCTAssertTrue(ios.contains("detail: \"기기 설정을 따릅니다.\\n또는 앱 화면 모드를 선택합니다.\""))
        XCTAssertTrue(ios.contains("Text(\"일정 변경은 아래에서 처리합니다.\\n전체 검사는 진단 화면에서 실행합니다.\")"))
        XCTAssertTrue(ios.contains(".accessibilityIdentifier(\"immediate-settings-summary-copy\")"))
        XCTAssertTrue(ios.contains(".accessibilityIdentifier(\"calendar-action-help-copy\")"))
        XCTAssertFalse(ios.contains("화면 모드는 이 기기에 바로 적용되고"))
        XCTAssertFalse(ios.contains("전체\\u{00A0}상태\\u{00A0}검사는"))
    }
    ...
    func testIOSCompanionUsesAdaptiveIPadNavigation() throws {
        ...
        let workstationTasks = try sourceStructBody(named: "WorkstationTasksWorkspace", in: ios)
        let workstationCalendar = try sourceStructBody(named: "WorkstationCalendarWorkspace", in: ios)
        let stableSectionPane = try sourceStructBody(named: "CompanionStableSectionPane", in: ios)
        let deferredSectionContent = try sourceStructBody(named: "CompanionDeferredSectionContent", in: ios)
        ...
        XCTAssertTrue(ios.contains("@Environment(\\.companionLayoutMode)"))
        XCTAssertFalse(ios.contains("private struct CompanionSplitRootView"))
        XCTAssertTrue(ios.contains("private struct WorkstationNavigationRail"))
        XCTAssertTrue(ios.contains("private struct WorkstationSidebar"))
        XCTAssertTrue(ios.contains("private struct CompanionStableSectionPane"))
        ...
        XCTAssertTrue(ios.contains("CompanionAdaptiveRootView("))
        XCTAssertTrue(ios.contains("private struct CompanionAdaptiveRootView"))
        XCTAssertTrue(ios.contains("CompanionStableSectionPane(section: currentSection, model: model, openSection: select)"))
        XCTAssertFalse(ios.contains("CompanionStableSectionPane(section: currentSection, model: model)"))
        ...
        XCTAssertTrue(stableSectionPane.contains("CompanionDeferredSectionContent(section: section, model: model, openSection: openSection)"))
        XCTAssertTrue(stableSectionPane.contains(".clipped()"))
        XCTAssertTrue(stableSectionPane.contains("transaction.disablesAnimations = true"))
        XCTAssertFalse(deferredSectionContent.contains(".id(section.rawValue)"))
        XCTAssertTrue(deferredSectionContent.contains(".background(Color.klmsScreenBackground)"))
        XCTAssertTrue(deferredSectionContent.contains("transaction.disablesAnimations = true"))
        ...
        XCTAssertTrue(ios.contains("layoutMode == .wide"))
        XCTAssertTrue(ios.contains(".frame(maxWidth: .infinity, minHeight: KLMSControlSize.minimumInteractive, alignment: .leading)"))
    ...
    func testMacAndIOSServerRelayUseWebSocketInsteadOfPeriodicPolling() throws {
        ...
        XCTAssertTrue(ios.contains("private func handleRelayEvent("))
        XCTAssertTrue(ios.contains("let decision = relayEventCursor.decision(for: event)"))
        XCTAssertTrue(ios.contains("private static func relayRefreshScope(for scopes: Set<RelayEventScope>)"))
        XCTAssertTrue(ios.contains("scheduleRelayEventBatch(generation: generation)"))
        ...
        XCTAssertTrue(ios.contains("static let commandUpdate = RelayRefreshScope(\n            fetchesCommands: true,\n            fetchesSyncData: true"))
        ...
        XCTAssertTrue(macModel.contains("serverRelayForceSyncDataFetchOnNextWorkerRefresh = true"))
        XCTAssertTrue(macModel.contains("let succeeded = await self.processServerRelayCommands("))
        XCTAssertTrue(macModel.contains("func processServerRelayCommands("))
        XCTAssertTrue(macModel.contains("forceSyncDataFetch: Bool = false"))
        XCTAssertTrue(macModel.contains("let shouldForceSyncDataFetch = forceSyncDataFetch || serverRelayForceSyncDataFetchOnNextWorkerRefresh"))
        XCTAssertTrue(macModel.contains("shouldFetchServerRelaySyncData(force: shouldForceSyncDataFetch)"))
        XCTAssertTrue(macModel.contains("reason == \"state\"\n            || reason == \"updated\""))
        XCTAssertTrue(macModel.contains("reason.hasPrefix(\"commands:\")"))
        XCTAssertTrue(macModel.contains("if reason == \"sync-data\" || reason.hasPrefix(\"sync-data:\")"))
        XCTAssertTrue(macModel.contains("|| (reason.hasPrefix(\"commands:\") && reason != \"commands:pending\")"))
        ...
        XCTAssertTrue(macModel.contains("reason.hasPrefix(\"file-access:\")"))
        XCTAssertTrue(macModel.contains("reason.hasPrefix(\"logs-display:\")"))
        XCTAssertFalse(macModel.contains("serverRelayFallbackPollIntervalNanoseconds"))
        XCTAssertFalse(macModel.contains("runningSnapshotRefreshIntervalNanoseconds"))
    ...
    func testMacRetriesOneShotRemoteCancelFetchFailureWithoutAnotherRelayEvent() throws {
        let packageRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let macModelRoot = packageRoot.appendingPathComponent("Sources/KLMSMac/KLMSMacModel.swift")
        let macModel = try String(contentsOf: macModelRoot, encoding: .utf8)
        let eventBatch = try sourceBody(
            after: "private func scheduleServerRelayEventBatch(",
            in: macModel,
            description: "Mac relay event batch"
        )
        let commandProcessor = try sourceBody(
            after: "func processServerRelayCommands(",
            in: macModel,
            description: "Mac relay command processor"
        )
        let cancelProcessor = try sourceBody(
            after: "private func processServerRelayCancelRequest(",
            in: macModel,
            description: "Mac relay cancel processor"
        )
        ...
        XCTAssertTrue(eventBatch.contains("RelayReconnectBackoff.nanoseconds("))
        XCTAssertTrue(eventBatch.contains("self.scheduleServerRelayEventBatch(\n                        generation: generation,\n                        delayNanoseconds: retryDelay"))
    }

    func testMacRelayRealtimeUsesSessionAndOperationOwnershipWithoutBlockingClaims() throws {
        let packageRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let macModelRoot = packageRoot.appendingPathComponent("Sources/KLMSMac/KLMSMacModel.swift")
        let macModel = try String(contentsOf: macModelRoot, encoding: .utf8)
        let eventStream = try sourceBody(
            after: "private func runServerRelayEventStream(key: String) async",
            in: macModel,
            description: "Mac relay event stream"
        )
        let eventBatch = try sourceBody(
            after: "private func scheduleServerRelayEventBatch(",
            in: macModel,
        ...
        let resetSession = try sourceBody(
            after: "private func resetServerRelaySessionForConnectionChange()",
            in: macModel,
            description: "Mac relay session reset"
        )

        XCTAssertEqual(
            macModel.components(separatedBy: "serverRelaySessionGeneration &+= 1").count - 1,
            1,
            "Only relay URL/token session reset may advance the account generation."
        )
        XCTAssertTrue(resetSession.contains("serverRelaySessionGeneration &+= 1"))
        XCTAssertTrue(eventStream.contains("let generation = serverRelayEventCursor.beginConnection()"))
        XCTAssertFalse(eventStream.contains("await refreshServerRelayDashboardNow"))
        XCTAssertTrue(eventStream.contains("scheduleServerRelayEventBatch(generation: generation)"))
        XCTAssertTrue(eventStream.contains("let message = try await task.receive()"))

        let workerIndex = try XCTUnwrap(eventBatch.range(of: "let workerSucceeded = await self.processServerRelayCommands(")?.lowerBound)
        let dashboardIndex = try XCTUnwrap(eventBatch.range(of: "let dashboardSucceeded = await self.refreshServerRelayDashboardNow(")?.lowerBound)
        XCTAssertLessThan(workerIndex, dashboardIndex, "Worker inbox/claims must precede dashboard snapshot I/O.")
        XCTAssertTrue(eventBatch.contains("serverRelayEventBatchOperationID == operationID"))
        XCTAssertTrue(eventBatch.contains("serverRelayEventBatchPrioritizesRunningCancel"))
        ...
        XCTAssertTrue(commandProcessor.contains("serverRelayForceSyncDataFetchOnNextWorkerRefresh = true"))
        XCTAssertTrue(commandProcessor.contains("activeServerRelayCommandCheckOperationID == commandCheckOperationID"))
        let busyOwner = try sourceBody(
            after: "guard activeServerRelayCommandCheckOperationID == nil else",
            in: commandProcessor,
        ...
        XCTAssertTrue(immediateFollowUp.contains("if !succeeded"))
        XCTAssertTrue(immediateFollowUp.contains("self.serverRelayDirtyScopes.insert("))
        XCTAssertTrue(immediateFollowUp.contains("RelayReconnectBackoff.nanoseconds("))

        let remoteCompletion = try XCTUnwrap(commandProcessor.range(of: "let completionMessage = \"최근 서버 요청:")?.lowerBound)
        let remoteTail = commandProcessor[remoteCompletion...]
        let terminalWrite = try XCTUnwrap(remoteTail.range(of: "try await persistServerRelayTerminalCommand(")?.lowerBound)
        let currentSessionUI = try XCTUnwrap(remoteTail.range(of: "let terminalDisposition = RelayClaimedOperationPolicy.terminalDisposition(")?.lowerBound)
        XCTAssertLessThan(terminalWrite, currentSessionUI, "Captured old relay result must enter the durable outbox before current-session UI writes.")
        XCTAssertTrue(remoteTail.contains("if terminalDisposition.appliesVisibleState"))
        XCTAssertTrue(remoteTail.contains("} else if serverRelayEnabled, serverRelayConfigured {"))
        XCTAssertTrue(remoteTail.contains("serverRelayDirtyScopes.insert(\n                    RelayEventScope.allCases,\n                    requiresSnapshot: true"))
        XCTAssertTrue(remoteTail.contains("generation: serverRelayEventCursor.connectionGeneration"))
        let terminalOutboxPersistence = try sourceBody(
            after: "private func persistServerRelayTerminalCommand(",
            in: macModel,
        ...
        XCTAssertTrue(macModel.contains("func clearServerRelaySharedRunLogs() async {\n        let expectedSessionGeneration = serverRelaySessionGeneration\n        serverRelayLogClearOperationID &+= 1"))
        XCTAssertTrue(macModel.contains("func clearServerRelayActivityLogs() async {\n        let expectedSessionGeneration = serverRelaySessionGeneration"))
        XCTAssertTrue(macModel.contains("await clearServerRelaySharedRunLogs()\n            guard serverRelaySessionGeneration == expectedSessionGeneration else { return }"))
        XCTAssertTrue(macModel.contains("await clearServerRelayLogs(scope: .all)\n        guard serverRelaySessionGeneration == expectedSessionGeneration else { return }"))
    }
    ...
    func testMacRuntimeUsesServerSyncSettingsBeforeConfigEnv() throws {
        ...
        XCTAssertTrue(applySyncData.contains("let incomingSettings = syncData.settings + syncData.sharedSettings"))
        XCTAssertTrue(applySyncData.contains("applyServerRelaySharedSettings(incomingSettings, merge: false)"))
        XCTAssertTrue(applySyncData.contains("updateServerRelaySharedSettingCommittedBaselines(from: incomingSettings)"))
    ...
    func testIOSSettingActionsUpdateRemoteSettingsImmediately() throws {
        ...
        XCTAssertTrue(createSettingAction.contains("removeRecentSettingActions { $0.key == setting.key }"))
        ...
        XCTAssertTrue(ios.contains("application.didChange = applyRemoteSettingActionsFromServer(settingActions)"))
    ...
    func testIOSServerDisplayItemActionsUpdateDashboardImmediately() throws {
        ...
        XCTAssertTrue(createItemAction.contains("let suppressSuccessFeedback = actionKind.suppressesImmediateSuccessFeedback"))
        ...
        XCTAssertTrue(inlineItemDetail.contains("GridItem(.flexible(minimum: 0), spacing: KLMSSpacing.standard)"))
        XCTAssertTrue(remoteItemToggleButton.contains("private var isDisabled: Bool"))
        ...
        XCTAssertTrue(ios.contains("var suppressesImmediateSuccessFeedback: Bool"))
        XCTAssertTrue(ios.contains("case .assignmentComplete,\n             .assignmentHide,\n             .examIgnore,\n             .noticeHide,\n             .fileHide,\n             .fileTrash,\n             .mailDashboardRemove:"))
        ...
        XCTAssertTrue(ios.contains("guard serverRelayEventWebSocketTask?.state != .running else"))
        XCTAssertTrue(ios.contains("private func replaceRecentItemAction("))
        XCTAssertTrue(ios.contains("private func removeRecentItemActions(where shouldRemove:"))
        ...
        XCTAssertTrue(ios.contains("private func rollbackItemActionMutation(_ overlay: PendingItemActionOverlay)"))
        XCTAssertTrue(ios.contains("itemActionMutationVersions.owns(key: key, version: version)"))
        XCTAssertTrue(ios.contains("OptimisticRollbackPolicy.shouldRestore("))
        XCTAssertFalse(ios.contains("syncItems = previousSyncItems"))
        XCTAssertFalse(ios.contains("mailDashboardItems = previousMailDashboardItems"))
        XCTAssertTrue(localApply.contains("rebuildDashboardDerivedState()"))
        XCTAssertTrue(ios.contains("rollbackItemActionMutation(overlay)"))
        XCTAssertTrue(ios.contains("rebuildDashboardDerivedState()\n        persistCachedServerSyncData(ServerRelaySyncData("))
        XCTAssertTrue(ios.contains("let incomingSyncItems = syncItemsOverlayingRecentDisplayActions(syncData.items)"))
        XCTAssertTrue(ios.contains("private func syncItemsOverlayingRecentDisplayActions(_ incomingItems: [ServerRelaySyncItem]) -> [ServerRelaySyncItem]"))
        XCTAssertTrue(ios.contains("let pendingActions = pendingItemActionOverlaysByID.values.map(\\.action)"))
        XCTAssertTrue(ios.contains("(recentItemActions + pendingActions).map { ($0.id, $0) }"))
        XCTAssertTrue(ios.contains("&& !$0.status.isFailedLike"))
    ...
    func testIOSServerActionRefreshesUseNarrowScopes() throws {
        ...
        let cancelCommand = try sourceBody(
            after: "private func cancelRunningCommand(expectedCommandID: UUID?) async",
            in: ios,
        ...
        XCTAssertTrue(updateSharedSetting.contains("sharedSettingMutationVersionsByKey[key] = mutationVersion"))
        XCTAssertTrue(updateSharedSetting.contains("pendingSharedSettingsByKey[key] = setting"))
        XCTAssertTrue(updateSharedSetting.contains("sharedSettingMutationVersionsByKey[key] == mutationVersion"))
        XCTAssertTrue(updateSharedSetting.contains("await sharedSettingMutationQueue.enqueue(for: key)"))
        XCTAssertTrue(updateSharedSetting.contains("sharedSettingCommittedBaselines.beginMutation("))
        XCTAssertTrue(updateSharedSetting.contains("sharedSettingCommittedBaselines.recordCommittedValue("))
        XCTAssertTrue(updateSharedSetting.contains("restoreSharedSetting(key: key, previousSetting: committedSetting)"))
        let iosBaselineCaptureIndex = try XCTUnwrap(updateSharedSetting.range(of: "sharedSettingCommittedBaselines.beginMutation(")?.lowerBound)
        let iosOptimisticApplyIndex = try XCTUnwrap(updateSharedSetting.range(of: "pendingSharedSettingsByKey[key] = setting")?.lowerBound)
        XCTAssertLessThan(iosBaselineCaptureIndex, iosOptimisticApplyIndex)
        let iosSavedIndex = try XCTUnwrap(updateSharedSetting.range(of: "let saved = try await serverRelayStore.updateSharedSetting(setting)")?.lowerBound)
        let iosAfterSaved = updateSharedSetting[iosSavedIndex...]
        let iosCommittedUpdateIndex = try XCTUnwrap(iosAfterSaved.range(of: "self.sharedSettingCommittedBaselines.recordCommittedValue(")?.lowerBound)
        let iosLatestVersionGuardIndex = try XCTUnwrap(iosAfterSaved.range(of: "guard self.sharedSettingMutationVersionsByKey[key] == mutationVersion else { return }")?.lowerBound)
        XCTAssertLessThan(iosCommittedUpdateIndex, iosLatestVersionGuardIndex)
        XCTAssertFalse(createManualCalendarAction.contains("isSubmitting = true"))
        XCTAssertTrue(createManualCalendarAction.contains("await createCalendarEventOnDevice(title: title, edit: edit, change: nil, completedAction: nil)"))
        XCTAssertFalse(createManualCalendarAction.contains("let requestItemID = \"mail-calendar-\\(UUID().uuidString)\""))
        XCTAssertFalse(createManualCalendarAction.contains("replaceRecentItemAction(action) { $0.id == action.id }"))
        XCTAssertFalse(createManualCalendarAction.contains("schedulePostActionRefresh(scope: .itemActions)"))
        ...
        XCTAssertTrue(createCommand.contains("isSubmitting = true"))
        XCTAssertTrue(createCommand.contains("commandSubmissionOperationID &+= 1"))
        XCTAssertTrue(createCommand.contains("commandSubmissionOperationID == submissionOperationID"))
        ...
        XCTAssertTrue(cancelCommand.contains("중단 요청을 화면에 먼저 반영했습니다. 서버로 보내는 중입니다."))
        XCTAssertTrue(cancelCommand.contains("중단 요청 전송 실패"))
        XCTAssertFalse(cancelCommand.contains("isSubmitting = true"))
        XCTAssertTrue(createFileAccess.contains("replaceRecentFileAccessRequest(request) { $0.id == request.id }"))
        XCTAssertTrue(createFileAccess.contains("pendingFileAccessRequestsByID.removeValue(forKey: request.id)"))
        XCTAssertTrue(createFileAccess.contains("pendingFileAccessRequestsByID[created.id] = created"))
        XCTAssertTrue(createFileAccess.contains("$0.id == request.id || $0.id == created.id"))
        XCTAssertTrue(createFileAccess.contains("removeRecentFileAccessRequests { $0.id == request.id }"))
        XCTAssertTrue(ios.contains("private func replaceRecentFileAccessRequest("))
        XCTAssertFalse(createFileAccess.contains("rebuildFileAccessLookup()"))
        XCTAssertFalse(createFileAccess.contains("rebuildRemoteLogDerivedState()"))
    ...
    func testIOSServerConnectionPasteImmediatelyRefreshesSummary() throws {
        ...
        let connectionCheck = try sourceBody(
            after: "func checkServerRelayConnection() async",
            in: ios,
            description: "iOS server relay connection check"
        )
        ...
        XCTAssertTrue(applyConnection.contains("await persistServerRelayConnection("))
        ...
        let persistConnection = try sourceBody(
            ...
            in: ios,
            description: "iOS server relay connection persistence"
        )
        let generationGuard = try XCTUnwrap(persistConnection.range(of: "persistenceCoordinator.commitIfCurrent("))
        let commitConnection = try XCTUnwrap(persistConnection.range(of: "commitServerRelayConnection("))
        XCTAssertLessThan(generationGuard.lowerBound, commitConnection.lowerBound)
        ...
        XCTAssertTrue(connectionCheck.contains("connectionCheckSequence &+= 1"))
        XCTAssertTrue(connectionCheck.contains("let checkSequence = connectionCheckSequence"))
        XCTAssertTrue(connectionCheck.contains("if connectionCheckSequence == checkSequence"))
        XCTAssertGreaterThanOrEqual(
            connectionCheck.components(separatedBy: "connectionCheckSequence == checkSequence").count - 1,
            3
        )
    ...
    func testIOSRefreshesServerSummaryWhenAppBecomesActive() throws {
        ...
        XCTAssertTrue(rootView.contains("guard !model.isUsingUITestFixture else { return }"))
        XCTAssertTrue(rootView.contains("guard newPhase == .active, !model.isUsingUITestFixture else { return }"))
        XCTAssertTrue(rootView.contains("await model.bootstrapServerRelayFromLaunch()"))
        XCTAssertTrue(rootView.contains("model.ensureServerRelayRealtimeConnection()"))
        XCTAssertFalse(rootView.contains("bootstrapServerRelayFromLaunch(silentInitialErrors: true"))
        ...
        XCTAssertTrue(ios.contains("let scope: RelayRefreshScope = includeSyncData == true ? .syncData : .state"))
        XCTAssertTrue(ios.contains("await refreshRecent(silentErrors: silentInitialErrors, includeSyncData: includeSyncData, showsActivity: false, scope: scope)"))
        ...
        XCTAssertTrue(refreshRecent.contains("await consumeRelayRefreshEndpoints("))
        XCTAssertFalse(refreshRecent.contains("let syncDataResult = await syncDataTask"))
        XCTAssertFalse(refreshRecent.contains("let responseResult = await responseTask"))
        XCTAssertTrue(ios.contains("RelayEndpointCompletionConsumer.consume(operations)"))
    ...
    func testIOSServerConnectionPersistenceDoesNotBlockTypingAndAppliesAtomically() throws {
        ...
        XCTAssertFalse(applyConnection.contains("commitServerRelayConnection("))
        let generationGuard = try XCTUnwrap(persistConnection.range(of: "persistenceCoordinator.commitIfCurrent("))
        let commitIndex = try XCTUnwrap(persistConnection.range(of: "commitServerRelayConnection(serverURL: serverURL, serverToken: serverToken)"))
        XCTAssertLessThan(generationGuard.lowerBound, commitIndex.lowerBound)
        XCTAssertTrue(credentialPersistence.contains("public final class CredentialPersistenceCoordinator: @unchecked Sendable"))
        XCTAssertTrue(credentialPersistence.contains("public func commitIfCurrent("))
    ...
    func testIOSServerConnectionUsesOneVerifiedKeychainRecordAndFailClosedMigration() throws {
        ...
        XCTAssertTrue(loadMigratingConnection.contains("account: klmsLegacyServerRelayTokenKeychainAccount"))
        ...
        XCTAssertTrue(loadMigratingConnection.contains("case let .current(storedConnection), let .migrated(storedConnection):"))
        XCTAssertTrue(loadMigratingConnection.contains("case .readFailed:"))
        XCTAssertTrue(loadMigratingConnection.contains("case .migrationFailed:"))
        XCTAssertTrue(loadMigratingConnection.contains("recoveryMessage:"))
        ...
        XCTAssertTrue(persistConnection.contains("case let .current(storedEnvelopeText)"))
    ...
    func testMacServerConnectionRejectsUnboundLegacyAndAppliesCompositeAtomically() throws {
        ...
        XCTAssertTrue(initializer.contains("persistedConnectionLoad.requiresRecovery"))
        XCTAssertTrue(initializer.contains("contains(where: \\.requiresRecovery)"))
        ...
        XCTAssertTrue(persistConnection.contains("case let .current(storedPayload)"))
    ...
    func testLogClearPreservesActiveCancellationAndFileRequests() throws {
        ...
        let iosEventLocalClear = try sourceBody(
            after: "private func applyRelayEventLocalClear",
            in: ios,
            description: "iOS relay event local clear"
        )
        let iosEventHandler = try sourceBody(
            after: "private func handleRelayEvent(",
            in: ios,
            description: "iOS relay event handler"
        )
        let iosSharedRunLogClearState = try sourceBody(
            after: "private func clearSharedRunLogDisplayState()",
            in: ios,
            description: "iOS shared run log display clear"
        )
        XCTAssertTrue(iosClearRemoteLogs.contains("applyLogClear(scope: scope)"))
        XCTAssertTrue(iosClearRemoteLogs.contains("clearSharedRunLogDisplayState()"))
        ...
        XCTAssertFalse(iosClearRemoteLogs.contains("UserAlert(\n            title: remoteClearError == nil"))
        XCTAssertFalse(iosClearRemoteLogs.contains("화면 기록을 지웠습니다."))
        XCTAssertFalse(iosClearRemoteLogs.contains("화면 기록을 지우는 중입니다."))
        ...
        XCTAssertTrue(iosClearSharedRunLogs.contains("clearSharedRunLogDisplayState()"))
        XCTAssertFalse(iosClearSharedRunLogs.contains("공유 실행 로그를 지웠습니다."))
        XCTAssertFalse(iosClearSharedRunLogs.contains("공유 실행 로그 지움"))
        let localSharedClearIndex = try XCTUnwrap(iosClearSharedRunLogs.range(of: "clearSharedRunLogDisplayState()")?.lowerBound)
        ...
        XCTAssertTrue(iosSharedRunLogClearState.contains("sharedRunLogs = []"))
        XCTAssertTrue(iosSharedRunLogClearState.contains("sharedRunLogsSignature = nil"))
        XCTAssertTrue(iosSharedRunLogClearState.contains("rebuildRemoteLogDerivedState()"))
        XCTAssertTrue(ios.contains("applyRelayEventLocalClear(reason: reason)"))
        let iosDecisionGuardIndex = try XCTUnwrap(iosEventHandler.range(of: "guard decision != .ignore else { return }")?.lowerBound)
        let iosEventClearIndex = try XCTUnwrap(iosEventHandler.range(of: "applyRelayEventLocalClear(reason: reason)")?.lowerBound)
        XCTAssertLessThan(iosDecisionGuardIndex, iosEventClearIndex)
        XCTAssertTrue(iosEventLocalClear.contains("reason == \"sync-data:run-logs-clear\""))
        XCTAssertTrue(iosEventLocalClear.contains("clearSharedRunLogDisplayState()"))
        XCTAssertTrue(iosEventLocalClear.contains("applyLogClear(scope: scope)"))
        ...
        let macHasClearableLogs = try sourceBody(
            after: "var hasClearableVisibleLogs: Bool",
            in: macModel,
            description: "Mac clearable log state"
        )
        let macEventHandler = try sourceBody(
            after: "private func handleServerRelayEvent",
            in: macModel,
            description: "Mac relay event handler"
        )
        let macSharedRunLogClearState = try sourceBody(
            after: "private func clearSharedRunLogDisplayState()",
            in: macModel,
            description: "Mac shared run log display clear"
        )
        XCTAssertTrue(macClearLogs.contains("clearTransientRunState()"))
        XCTAssertTrue(macClearLogs.contains("clearLocalStoredLogs()"))
        XCTAssertTrue(macClearLogs.contains("applyServerRelayLogClear(scope: .all)"))
        XCTAssertTrue(macClearLogs.contains("await clearServerRelayLogs(scope: .all)"))
        XCTAssertTrue(macModel.contains("_ = try await store.clearDisplayLogs(scope: scope)"))
        XCTAssertFalse(macModel.contains("serverRelayLogClearMessage("))
        XCTAssertFalse(macModel.contains("로그를 지웠습니다."))
        XCTAssertFalse(macModel.contains("화면 로그를 지웠습니다."))
        XCTAssertFalse(macModel.contains("파일 요청이 끝난 뒤 파일 요청 기록을 지울 수 있습니다."))
        XCTAssertTrue(macModel.contains("CommandRunHistoryStore(url: paths.appHistoryURL).clear()"))
        XCTAssertTrue(macModel.contains("private var localRequestLogClearedAt: Date?"))
        XCTAssertTrue(macModel.contains("private var localFileAccessLogClearedAt: Date?"))
        XCTAssertTrue(macModel.contains("private var localSharedRunLogClearedAt: Date?"))
        XCTAssertTrue(macModel.contains("private var locallyHiddenCommandIDs = Set<UUID>()"))
        XCTAssertTrue(macModel.contains("private var locallyHiddenRequestLogIDs = Set<UUID>()"))
        XCTAssertTrue(macModel.contains("private var locallyHiddenFileAccessRequestIDs = Set<UUID>()"))
        XCTAssertTrue(macModel.contains("private var locallyHiddenSharedRunLogIDs = Set<String>()"))
        XCTAssertTrue(macModel.contains("private func visibleServerRelayRequestLog"))
        XCTAssertTrue(macModel.contains("private func visibleServerRelayFileAccessRequests"))
        XCTAssertTrue(macModel.contains("private func visibleServerRelayRunLogs"))
        XCTAssertTrue(macModel.contains("locallyHiddenRequestLogIDs.formUnion(serverRelayRecentRequestLog.map(\\.id))"))
        XCTAssertTrue(macModel.contains("locallyHiddenFileAccessRequestIDs.formUnion(serverRelayRecentFileAccessRequests.filter { !$0.status.isInFlight }.map(\\.id))"))
        XCTAssertTrue(macModel.contains("locallyHiddenSharedRunLogIDs.formUnion(serverRelaySharedRunLogs.map(\\.id))"))
        XCTAssertTrue(macModel.contains("logs.filter { !locallyHiddenSharedRunLogIDs.contains($0.id) }"))
        XCTAssertTrue(macModel.contains("reason == \"sync-data:run-logs-clear\""))
        XCTAssertTrue(macEventHandler.contains("clearSharedRunLogDisplayState()"))
        let macDecisionGuardIndex = try XCTUnwrap(macEventHandler.range(of: "guard decision != .ignore else { return }")?.lowerBound)
        let macEventClearIndex = try XCTUnwrap(macEventHandler.range(of: "clearSharedRunLogDisplayState()")?.lowerBound)
        XCTAssertLessThan(macDecisionGuardIndex, macEventClearIndex)
        XCTAssertTrue(macSharedRunLogClearState.contains("serverRelaySharedRunLogs = []"))
        XCTAssertTrue(macSharedRunLogClearState.contains("sharedRunLogStageDurationsByID = [:]"))
        XCTAssertTrue(macHasClearableLogs.contains("serverRelayStatusMessage"))
        XCTAssertTrue(macHasClearableLogs.contains("remoteProcessingStatusMessage"))
        XCTAssertTrue(macModel.contains("remoteProcessingStatusMessage = nil\n        serverRelayStatusMessage = nil"))
        XCTAssertTrue(macModel.contains("recordLocalDisplayLogClear(scope: scope)"))
        XCTAssertTrue(macModel.contains("serverRelayRecentFileAccessRequests = serverRelayRecentFileAccessRequests.filter { $0.status.isInFlight }"))
        XCTAssertTrue(macView.contains("await model.clearVisibleLogsAndServerRelayLogs()"))
        XCTAssertTrue(macView.contains("DashboardTopBarView("))
        XCTAssertTrue(macView.contains("layoutMode: layoutMode"))
    }
    ...
    func testCalendarOpenActionIsSupportedAcrossRelaySurfaces() throws {
        ...
        XCTAssertTrue(calendarDetailRow.contains("await onCreate(change.editDefaults)"))
        XCTAssertTrue(calendarDetailRow.contains("await onEdit(edit)"))
        XCTAssertTrue(calendarDetailRow.contains("await onDelete()"))
        XCTAssertFalse(calendarDetailRow.contains("calendarSheetAction = .calendarCreate"))
        ...
        XCTAssertTrue(calendarDetailRow.contains("GridItem(.flexible(minimum: 0), spacing: KLMSSpacing.standard)"))
        XCTAssertFalse(calendarDetailRow.contains("GridItem(.adaptive(minimum: 96), spacing: KLMSSpacing.standard)"))
    ...
    func testIOSRefreshAndDisplayClearShowImmediateFeedback() throws {
        ...
        XCTAssertTrue(ios.contains("connectionMessage = refreshFailureMessage(reason: statusRefreshErrorMessage)"))
    ...
    func testIOSRelayRefreshFetchesRemotePanelsConcurrently() throws {
        ...
        XCTAssertTrue(refreshBody.contains("await consumeRelayRefreshEndpoints("))
        ...
        XCTAssertFalse(refreshBody.contains("let syncDataResult = await syncDataTask"))
        XCTAssertFalse(refreshBody.contains("let responseResult = await responseTask"))
        XCTAssertTrue(ios.contains("let operations: [@Sendable () async -> RelayRefreshEndpointCompletion]"))
        XCTAssertTrue(ios.contains("RelayEndpointCompletionConsumer.consume(operations)"))
        XCTAssertTrue(ios.contains("relayEndpointApplyEpochs.begin(for: endpoint)"))
        XCTAssertTrue(ios.contains("relayEndpointApplyEpochs.owns("))
        XCTAssertTrue(ios.contains("loadedSyncData: true"))
        XCTAssertTrue(refreshBody.contains("대시보드는 불러왔지만 현재 실행 상태 갱신은 실패했습니다."))
        XCTAssertTrue(refreshBody.contains("일부 항목을 불러오지 못했습니다. 화면에는 확인된 최신 내용만 표시합니다."))
        XCTAssertTrue(refreshBody.contains("else if refreshSucceeded"))
        XCTAssertTrue(ios.contains("application.didChange = markInitialSyncDataLoadFailure("))
        XCTAssertTrue(ios.contains("didChange: markSyncDataLoadFailure("))
        ...
        XCTAssertTrue(ios.contains("struct RelayRefreshScope: Equatable, Sendable"))
