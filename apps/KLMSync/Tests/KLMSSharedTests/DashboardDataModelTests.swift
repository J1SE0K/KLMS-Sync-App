...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testMacWorkspaceAccessibilitySmokeScriptTargetsNavigation() throws {
        ...
        XCTAssertTrue(basicActionsScript.contains("try pressControl(label, appElement: appElement)"))
    ...
    func testMacDashboardWindowFollowsApprovedWorkstationMockup() throws {
        ...
        XCTAssertTrue(app.contains("static let minWidth: CGFloat = 900"))
        ...
        XCTAssertTrue(macModel.contains("if let runningCommand {"))
        XCTAssertTrue(macModel.contains("실행 중입니다. 끝난 뒤 다시 시도해 주세요."))
        ...
        XCTAssertTrue(workstationLayout.contains("MacDashboardCommandCenterView(model: model, openRunLog: openRunLog)"))
        ...
        XCTAssertTrue(macCommandCenter.contains(".frame(width: 320, alignment: .topLeading)"))
        ...
        XCTAssertTrue(commandPanel.contains(".contentShape(RoundedRectangle(cornerRadius: 12))"))
        ...
        XCTAssertTrue(commandPanel.contains(".accessibilityIdentifier(\"command-cancel\")"))
        XCTAssertTrue(commandPanel.contains(".accessibilityIdentifier(isRunning ? \"command-cancel\" : \"command-\\(command.rawValue)\")"))
    ...
    func testMacAndIOSDirectInteractionAnimationsStayFast() throws {
        ...
        XCTAssertTrue(sources.contains(".onTapGesture {"))
        ...
        XCTAssertFalse(ios.contains(".scaleEffect(configuration.isPressed"))
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(ios.contains(".frame(minHeight: 30)"))
        ...
        XCTAssertTrue(immediateSettingsPanel.contains(".accessibilityValue(effectiveNoticeNotesEnabled ? \"켜짐\" : \"꺼짐\")"))
    ...
    func testMacAndIOSUseDedicatedLogScreensForRequestHistory() throws {
        ...
        XCTAssertTrue(macRemoteActivityPanel.contains(".buttonStyle(KLMSMacCompactMutedIconButtonStyle())"))
        ...
        XCTAssertTrue(macWorkstationLayoutView.contains("MacDashboardCommandCenterView(model: model, openRunLog: openRunLog)"))
        ...
        XCTAssertEqual(mac.components(separatedBy: "@ObservedObject var model: KLMSMacModel").count - 1, 9)
        ...
        XCTAssertTrue(workstationBody.contains("MacDashboardCommandCenterView(model: model, openRunLog: openRunLog)"))
        ...
        XCTAssertTrue(ios.contains("return ServerRelaySyncItem.dashboardDefaultSort(lhs, rhs)"))
        ...
        XCTAssertTrue(iosRemoteLogSummaryPanel.contains(".buttonStyle(KLMSCompactTrashButtonStyle())"))
        ...
        XCTAssertTrue(iosSharedRunLogsView.contains(".buttonStyle(KLMSCompactTrashButtonStyle())"))
        ...
        XCTAssertTrue(iosSidebarButton.contains(".font(.system(size: isCompact ? 12 : 13, weight: .semibold, design: .rounded))"))
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertTrue(ios.contains(".frame(width: 34, height: 34)"))
        XCTAssertTrue(recentFileRequests.contains(".buttonStyle(KLMSCompactTrashButtonStyle())"))
        XCTAssertTrue(recentServerRequests.contains(".buttonStyle(KLMSCompactTrashButtonStyle())"))
        XCTAssertTrue(recentRemoteCommands.contains(".buttonStyle(KLMSCompactTrashButtonStyle())"))
        XCTAssertTrue(ios.contains("private struct KLMSCardButtonStyle: ButtonStyle"))
        ...
        XCTAssertTrue(ios.contains("func shouldDebounceComparedTo(_ previous: CompanionItemListInputKey?) -> Bool"))
        XCTAssertTrue(ios.contains("false"))
        ...
        XCTAssertFalse(syncDataPanel.contains("currentKey.shouldDebounceComparedTo(cachedListInputKey)"))
        XCTAssertFalse(syncDataPanel.contains("try? await Task.sleep(nanoseconds: CompanionLargeList.filterRebuildDelayNanoseconds)"))
        XCTAssertFalse(syncDataPanel.contains("guard !Task.isCancelled, currentKey == listInputKey else { return }"))
        ...
        XCTAssertFalse(inlineDetail.contains("currentKey.shouldDebounceComparedTo(cachedListInputKey)"))
    ...
    func testIOSCalendarDetailHasMailPasteAnalyzerAndSharedCalendarActionLabels() throws {
        ...
        XCTAssertTrue(ios.contains("private func publishCompletedCalendarAction(_ actionKind: ServerRelayItemActionKind, change: CalendarChange) async"))
        XCTAssertTrue(ios.contains("status: .completed"))
        XCTAssertTrue(ios.contains("completedAction: .calendarCreate"))
        XCTAssertTrue(ios.contains("await publishCompletedCalendarAction(completedAction, change: change)"))
        XCTAssertTrue(ios.contains("await publishCompletedCalendarAction(.calendarEdit, change: change)"))
        XCTAssertTrue(ios.contains("await publishCompletedCalendarAction(.calendarDelete, change: change)"))
    ...
    func testMacAndIOSServerRelayUseWebSocketInsteadOfPeriodicPolling() throws {
        ...
        XCTAssertTrue(macModel.contains("serverRelayForceSyncDataFetchOnNextWorkerRefresh = true"))
        XCTAssertTrue(macModel.contains("await processServerRelayCommands(silent: true, forceSyncDataFetch: true)"))
        XCTAssertTrue(macModel.contains("func processServerRelayCommands(silent: Bool = false, forceSyncDataFetch: Bool = false) async"))
    ...
    func testIOSServerDisplayItemActionsUpdateDashboardImmediately() throws {
        ...
        XCTAssertTrue(ios.contains("let actions = recentItemActions"))
    ...
    func testIOSServerActionRefreshesUseNarrowScopes() throws {
        ...
        XCTAssertTrue(createFileAccess.contains("replaceRecentFileAccessRequest(created) { $0.id == request.id }"))
    ...
    func testIOSCachesServerSummaryForFirstLaunchDashboard() throws {
        ...
        XCTAssertTrue(applySharedSettings.contains("next = visibleIncomingSettings.sorted"))
    ...
    func testLogClearPreservesActiveCancellationAndFileRequests() throws {
        ...
        let iosEventStream = try sourceBody(
            after: "private func runServerRelayEventStream",
            in: ios,
            description: "iOS relay event stream"
        )
        ...
        XCTAssertTrue(iosEventStream.contains("applyRelayEventLocalClear(message)"))
