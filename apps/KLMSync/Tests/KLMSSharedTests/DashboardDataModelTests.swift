...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(companionItemListControls.contains("CompanionUIKitMenuButton("))
        XCTAssertTrue(companionItemListControls.contains("private struct CompanionUIKitMenuButton<Option: Hashable>: UIViewRepresentable"))
        XCTAssertTrue(companionItemListControls.contains("button.showsMenuAsPrimaryAction = true"))
        XCTAssertTrue(companionItemListControls.contains("button.menu = UIMenu(children: options.map"))
