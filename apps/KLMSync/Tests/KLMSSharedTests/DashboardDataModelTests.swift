...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        XCTAssertTrue(sheetItemDetail.contains("Text(\"항목 처리\")"))
        XCTAssertTrue(sheetItemDetail.contains("Text(\"동기화\")"))
        XCTAssertTrue(sheetItemDetail.contains("Label(\"\\(relevantCommand.displayName) 다시 실행\""))
        XCTAssertFalse(sheetItemDetail.contains("Text(\"수정/삭제 선택\")"))
        XCTAssertFalse(sheetItemDetail.contains("Text(\"반영\")"))
    ...
    func testIOSLargeDashboardListsRenderAllItemsLazily() throws {
        ...
        XCTAssertFalse(categoryDetail.contains("Section(\"검색과 필터\")"))
        XCTAssertFalse(categoryDetail.contains("TextField(\"\\(category.title) 검색\", text: $query)"))
        XCTAssertTrue(categoryDetail.contains("CompanionSearchFilterPanel(title: \"검색과 필터\", fieldPrompt: \"\\(category.title) 검색\", query: $query)"))
