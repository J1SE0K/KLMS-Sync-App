...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testIOSDashboardAndSettingsFollowDesignNavigation() throws {
        ...
        let iosSortFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "CompanionMenuPickerField(\n                    title: \"정렬 기준\"")?.lowerBound)
        let iosYearFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "CompanionMenuPickerField(\n                                title: \"연도\"")?.lowerBound)
        let iosSemesterFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "CompanionMenuPickerField(\n                                title: \"학기\"")?.lowerBound)
        let iosCourseFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "CompanionMenuPickerField(\n                            title: \"과목\"")?.lowerBound)
        let iosStatusFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "CompanionMenuPickerField(\n                        title: \"상태\"")?.lowerBound)
        let iosVisibilityFieldIndex = try XCTUnwrap(companionItemListControls.range(of: "CompanionMenuPickerField(\n                        title: \"표시 범위\"")?.lowerBound)
