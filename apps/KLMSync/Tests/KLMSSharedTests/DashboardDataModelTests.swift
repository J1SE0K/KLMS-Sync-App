...
final class DashboardDataModelTests: XCTestCase {
    ...
    func testIOSServerActionRefreshesUseNarrowScopes() throws {
        ...
        XCTAssertTrue(createManualCalendarAction.contains("replaceRecentItemAction(action) { $0.id == action.id }"))
        XCTAssertFalse(createManualCalendarAction.contains("schedulePostActionRefresh(scope: .itemActions)"))
        XCTAssertTrue(createManualCalendarAction.contains("removeRecentItemActions { $0.itemID == requestItemID && $0.status == .pending }"))
    ...
    func testCalendarOpenActionIsSupportedAcrossRelaySurfaces() throws {
        ...
        XCTAssertTrue(calendarDetailRow.contains("await onAction(.calendarCreate, change.editDefaults)"))
