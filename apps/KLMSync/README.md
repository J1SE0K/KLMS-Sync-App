...
The device must be unlocked for the launch step. If the helper says `installed; launch was skipped because the device is locked or not ready for developer launch` or `installed; launch was denied because the device is locked`, the app is already installed; unlock the device and open KLMS Sync manually, or rerun with `IOS_DEVICE_BUILD_FIRST=0 IOS_APP_PATH="<built app path>"`. To install without launching, use `IOS_DEVICE_LAUNCH=0`. When installing to `all`, the helper waits up to 45 seconds for paired iPhone/iPad devices to become available, prints a generic `iPhone` or `iPad` label for each result, skips devices that CoreDevice still reports as unavailable, continues with the remaining devices, then exits non-zero so the missed device is visible. Set `IOS_DEVICE_WAIT_FOR_AVAILABLE_SECONDS=0` to fail immediately. If a device is skipped, unlock it, reconnect USB, accept the Trust prompt, and confirm Developer Mode is enabled. If installation says the app is not signed, rebuild without `CODE_SIGNING_ALLOWED=NO`.
