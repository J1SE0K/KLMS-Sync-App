...
The readiness helper runs the Swift tests, Mac app build, Mac accessibility smoke, Mac basic-actions smoke, Mac tab-response probe, signed iOS build, and iPhone/iPad launch verification. The Mac basic-actions smoke checks that the dashboard action, log clear controls, Command-Q, and reopen flow are reachable. The iOS launch gate requires both an iPhone and an iPad to be present and launch-verified. Its summary marks omitted gates as `skipped`, so a run that disables iOS launch verification is not treated as full device readiness. It redacts local paths and signing-related values in its output. If the devices are locked or the developer app is not trusted yet, the iOS launch step fails with the same `launch-check pending` or `launch-check blocked` wording as the install helpers. Xcode account login and iOS device trust are separate: a signed build can succeed while each device still needs Settings > General > VPN & Device Management approval before launch verification passes.
...
The device must be unlocked for the launch step. If iOS is still refreshing app registration after installation, the helper retries launch verification briefly before giving up. Tune that with `IOS_DEVICE_LAUNCH_RETRIES` and `IOS_DEVICE_LAUNCH_RETRY_DELAY_SECONDS`. If CoreDevice has not opened the device tunnel yet, the helper first asks the device for details to wake the connection; tune that with `IOS_DEVICE_TUNNEL_WARMUP_SECONDS`. If the helper says `installed; launch-check pending`, the app is already installed but macOS cannot launch it yet; unlock the device, keep USB connected, accept Trust if shown, and open KLMS Sync manually or rerun the same install command after the device is ready. If the helper says `installed; launch-check blocked`, it first tries to open Settings on that iPhone/iPad for you; trust the developer app in Settings > General > VPN & Device Management while the helper waits, and it will retry launch automatically. Disable the Settings shortcut with `IOS_DEVICE_OPEN_SETTINGS_ON_BLOCKED=0`, tune its short wait with `IOS_DEVICE_OPEN_SETTINGS_TIMEOUT_SECONDS`, or tune the trust retry window with `IOS_DEVICE_TRUST_RETRY_SECONDS` and `IOS_DEVICE_TRUST_RETRY_POLL_SECONDS`. To install without launching, use `IOS_DEVICE_LAUNCH=0`. When installing to `all`, the helper waits up to 45 seconds for paired iPhone/iPad devices to become available, prints a generic `iPhone` or `iPad` label for each result, skips devices that CoreDevice still reports as unavailable, continues with the remaining devices, and ends with `install-summary installed=... launched=... installed_only=... pending=... blocked=... manual_launch_needed=... failed=...`. That line separates installed devices, launched devices, install-only devices, devices waiting for unlock/app registration, devices blocked by developer trust/signing, and other failures. It still exits non-zero when a requested launch could not be verified. Set `IOS_DEVICE_WAIT_FOR_AVAILABLE_SECONDS=0` to fail immediately. If a device is skipped, unlock it, reconnect USB, accept the Trust prompt, and confirm Developer Mode is enabled. If installation says the app is not signed, rebuild without `CODE_SIGNING_ALLOWED=NO`.
