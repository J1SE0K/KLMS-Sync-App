...
class LoginAssistPerformanceTests(unittest.TestCase):
    def test_safari_step_advances_multiple_login_states_per_process(self) -> None:
        ...
        self.assertIn("createSafariTab(windowRef, targetUrl)", text)
        self.assertIn("closeTab(checkTab)", text)
        self.assertIn("let keepCheckTab = false", text)
        self.assertIn("keepCheckTab = true", text)
        ...
        self.assertIn('method: "dashboard-check-tab"', text)
        self.assertIn("twofactorPending(sourceUrl, pendingReason)", text)
        self.assertNotIn("checkWindow = createSafariWindow", text)
        self.assertIn("looksLikeAuthenticatedKlmsUrl", text)
        self.assertIn("function isExactKlmsHttpsUrl(url)", text)
        self.assertIn('error: "invalid-klms-url"', text)
        self.assertNotIn('lower.includes("klms.kaist.ac.kr")', text)
        ...
        self.assertIn("inputSelectors", text)
        self.assertIn("buttonSelectors", text)
        self.assertIn("form.requestSubmit", text)
        self.assertIn('markSubmitted("button")', text)
        self.assertIn('reason: "login-submit-cooling-down"', text)
        self.assertNotIn("klmsLoginAssistMfaSubmitted === \"1\"", text)
        self.assertNotIn('reason: "login-already-submitted"', text)
        ...
        self.assertIn("findReusableAuthWindow(windows, backgroundWindowEnabled)", text)
        self.assertIn("const tabs = safeList(() => windowRef.tabs())", text)
        self.assertIn("windowRef.currentTab = tab", text)
        self.assertIn("extractTwofactorDigits", text)
        self.assertIn('".auth_number .number_wrap"', text)
        self.assertIn("[class*='auth'][class*='number']", text)
        self.assertIn('method: "span-digits"', text)
        self.assertIn('method: "body-auth-text"', text)
        self.assertIn('reason: "digits-not-ready"', text)
        self.assertIn("readyState: document.readyState", text)
        self.assertNotIn("looksLikeKaistAuthUrl(url) && (!backgroundWindowEnabled || isBackgroundWindow(windows[i]))", text)
    ...
    def test_shell_login_assist_is_manual_digits_only(self) -> None:
        ...
        self.assertIn("reason_suffix", text)
