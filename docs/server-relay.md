...
Mac 앱은 켜져 있어야 하며, 서버 요청을 polling해서 한 번에 하나씩 실행한다. Windows와 iPhone은 Mac과 같은 네트워크에 있을 필요가 없다. 대신 모든 앱이 같은 HTTPS 서버 릴레이 주소를 쓰고, Windows/iPhone은 클라이언트 토큰, Mac은 worker 토큰을 사용해야 한다.
...
- Xcode에서 Mac 앱을 빌드하고 실행한다.
- 설정에서 서버 주소, 클라이언트 토큰, worker 토큰을 저장한다.
- `서버 연결 정보 복사`로 Windows/iPhone에 붙여넣을 값을 만든다.
- `상태 검사`, `요약 갱신`, `파일 동기화` 중 하나를 직접 실행해서 서버 상태가 갱신되는지 확인한다.

Windows 앱:

- 설정 탭에서 Mac 앱이 복사한 서버 연결 정보를 붙여넣고 `붙여넣기 읽기`, `저장`, `연결 확인`을 차례로 누른다.
- 대시보드가 과제, 시험, 공지, 파일 목록을 불러오는지 확인한다.
- `상태 검사`, `전체 동기화`, `파일 동기화` 요청을 만들고 Mac 앱이 처리하는지 확인한다.
- 파일 탭에서 `파일 목록`, `새 파일`, `격리`, `삭제 예정` 전환과 `과목`, `종류`, `파일명`, `경로`, `최근` 정렬을 확인한다.
- 년도, 학기, 과목 필터와 다크모드에서 긴 과목명/파일명이 깨지지 않는지 확인한다.

완료 기준:

- Windows/iPhone은 클라이언트 토큰만 사용하고, Mac 앱은 worker 토큰으로 상태 게시와 대기 요청 처리를 수행한다.
- Windows에서 만든 명령의 최근 요청 상태가 `completed` 또는 명확한 실패 메시지로 돌아온다.
- 서버 DB에는 sanitized 상태와 항목만 올라가고, 원본 로그, `config.env`, Kaikey state, 내부 파일 경로는 노출되지 않는다.
...
원격 실행 `kind` 값은 `fullSync`, `coreSync`, `noticeSync`, `filesSync`, `verify`, `doctor`, `report`, `v2BuildState`를 쓴다. `verify`와 `v2BuildState`는 Mac 앱이 처리하며, 실제 원본 로그나 config.env 내용은 서버에 올리지 않는다.
